...
def index():
    # Page Title
    title=db(db.module.name=='dvr').select()[0].name_nice
	#title=T('Disaster Victim Registry')
    # List Modules (from which to build Menu of Modules)
    modules=db(db.module.enabled=='Yes').select(db.module.ALL,orderby=db.module.menu_priority)
    # List Options (from which to build Menu for this Module)
    options=db(db.dvr_menu_option.enabled=='Yes').select(db.dvr_menu_option.ALL,orderby=db.dvr_menu_option.priority)
    return dict(title=title,modules=modules,options=options)

# Select Option
def open():
    id=request.vars.id
    options=db(db.dvr_menu_option.id==id).select()
    if not len(options):
