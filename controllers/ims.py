...
def index():
    # Page Title
    title=db(db.module.name=='ims').select()[0].name_nice
    ...
    options=db(db.ims_menu_option.enabled=='Yes').select(db.ims_menu_option.ALL,orderby=db.ims_menu_option.priority)
    return dict(title=title,modules=modules,options=options)

# Select Option
def open():
    id=request.vars.id
    options=db(db.ims_menu_option.id==id).select()
    if not len(options):
