...
def contact():
    ...
    if auth.is_logged_in() or auth.basic():
        ...
        response.s3.filter = (db.pr_contact.pe_id == person)
    else:
        redirect(URL(r=request, c="default", f="user", args="login",
            vars={"_next":URL(r=request, c="msg", f="contact")}))
    ...
    s3xrc.model.configure(table,
            onvalidation=lambda form: msg_contact_onvalidation(form))

    def msg_contact_restrict_access(r):
        """ The following restricts update and delete access to contacts not owned by the user """
        if r.id :
            person = db(db.pr_person.uuid == auth.user.person_uuid).select(db.pr_person.pe_id, limitby=(0, 1)).first().pe_id
            if person == db(db.pr_contact.id == r.id).select(db.pr_contact.pe_id, limitby=(0, 1)).first().pe_id :
                return True
            else:
