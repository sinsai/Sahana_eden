...
marker_id = SQLTable(None, 'marker_id',
            Field('marker', db.gis_marker,
                ...
                label = T('Marker'),
                comment = DIV(A(T('Add Marker'), _class='thickbox', _href=URL(r=request, c='gis', f='marker', args='create', vars=dict(format='popup')), _target='top'), A(SPAN("[Help]"), _class="tooltip", _title=T("Marker|Defines the icon used for display of features."))),
                ondelete = 'RESTRICT'
                ))
...
feature_class_id = SQLTable(None, 'feature_class_id',
            Field('feature_class', db.gis_feature_class,
                ...
                comment = DIV(A(T('Add Feature Class'), _class='thickbox', _href=URL(r=request, c='gis', f='feature_class', args='create', vars=dict(format='popup')), _target='top'), A(SPAN("[Help]"), _class="tooltip", _title=T("Feature Class|Defines the marker used for display & the attributes visible in the popup."))),
                ondelete = 'RESTRICT'
                ))
# Populate table with Default options
if not len(db().select(db[table].ALL)):
...
db[table].metadata.comment = DIV(A(T('Add Metadata'), _class='thickbox', _href=URL(r=request, c='gis', f='feature_metadata', args='create', vars=dict(format='popup')), _target='top'), A(SPAN("[Help]"), _class="tooltip", _title=T("Metadata|Additional attributes associated with this Feature.")))
...
feature_id = SQLTable(None, 'feature_id',
            Field('feature', db.gis_feature,
                ...
                label = T('Feature'),
                comment = DIV(A(T('Add Feature'), _class='thickbox', _href=URL(r=request, c='gis', f='feature', args='create', vars=dict(format='popup')), _target='top'), A(SPAN("[Help]"), _class="tooltip", _title=T("Feature|The centre Point or Polygon used to display this Location on a Map."))),
                ondelete = 'RESTRICT'
                ))
...
location_id = SQLTable(None, 'location_id',
            Field('location', db.gis_location,
                ...
                label = T('Location'),
                comment = DIV(A(s3.crud_strings.gis_location.label_create_button, _class='thickbox', _href=URL(r=request, c='gis', f='location', args='create', vars=dict(format='popup')), _target='top'), A(SPAN("[Help]"), _class="tooltip", _title=T("Location|The Location of this Site, which can be general (for Reporting) or precise (for displaying on a Map)."))),
                ondelete = 'RESTRICT'
                ))
