...
db.define_table(table, timestamp, uuidstamp, deletion_status,
                ...
                migrate=migrate)
db[table].kit_id.requires = IS_ONE_OF(db, 'budget_kit.id', 'budget_kit.code')
db[table].kit_id.label = T('Kit')
db[table].kit_id.represent = lambda kit_id: db(db.budget_kit.id==kit_id).select()[0].code
db[table].item_id.requires = IS_ONE_OF(db, 'budget_item.id', 'budget_item.description')
...
db.define_table(table, timestamp, deletion_status,
                ...
                migrate=migrate)
db[table].bundle_id.requires = IS_ONE_OF(db, 'budget_bundle.id', 'budget_bundle.description')
...
db.define_table(table, timestamp, deletion_status,
                ...
                migrate=migrate)
db[table].budget_id.requires = IS_ONE_OF(db, 'budget_budget.id', 'budget_budget.name')
...
db[table].project_id.requires = IS_ONE_OF(db,'budget_project.id', 'budget_project.code')
...
db[table].location_id.requires = IS_ONE_OF(db, 'budget_location.id', 'budget_location.code')
...
db[table].bundle_id.requires = IS_ONE_OF(db, 'budget_bundle.id', 'budget_bundle.name')
db[table].bundle_id.label = T('Bundle')
...
db[table].location_id.label = T('Location')
db[table].location_id.represent = lambda location_id: db(db.budget_location.id==location_id).select()[0].code
db[table].staff_id.requires = IS_ONE_OF(db, 'budget_staff.id', 'budget_staff.name')
