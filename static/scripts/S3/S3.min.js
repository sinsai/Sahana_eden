...
url+=$('[name = "inv_item_id"]').val();$.getJSON(url,function(data){var InvQuantity=data.inv_inv_item.quantity;var InvPackQuantity=data.supply_item_pack.quantity;var PackName=$('[name = "item_pack_id"] option:selected').text();var re=/\(([0-9]*)/;var PackQuantity=re.exec(PackName)[1];var Quantity=(InvQuantity*InvPackQuantity)/PackQuantity;TotalQuantity='<span id = "TotalQuantity"> / '+Quantity.toFixed(2)+' '+PackName+' in inv.</span>';$('#inv_quantity_ajax_throbber').remove();$('[name = "quantity"]').after(TotalQuantity);});};$('#inv_send_item_item_pack_id').change(InvItemPackIDChange);function ItemIDChange(){var selField=$('[name $= "item_id"]');var selSubField=$('[name = "item_pack_id"]');$('[id$="item_pack_id__row1"]').show();$('[id$="item_pack_id__row"]').show();selSubField.after('<div id="item_pack_ajax_throbber" class="ajax_throbber style="display:inline;"/>').hide();var url;if(selField.length!=0){url=S3.Ap.concat('/supply/item_pack.json?item_pack.item_id=',selField.val());}else{url=S3.Ap.concat('/inv/inv_item_packs/',selField.val());}
