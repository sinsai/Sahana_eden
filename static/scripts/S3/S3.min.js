...
function s3_gis_dropdown_select(level,force){$('#gis_location_lat').val('');$('#gis_location_lon').val('');$('#gis_location_addr_street').val('');$('#gis_location_postcode').val('');var new_id=$('#gis_location_L'+level).val();if(new_id||force){if(level==s3_gis_maxlevel){var this_url=s3_gis_url+'/search.json?filter=%3D&field=level&value=nullnone&parent='+new_id;}else{var this_url=s3_gis_url+'/search.json?filter=%3D&field=level&value=L'+(level+1)+'&parent='+new_id;}
var s3_gis_load_locations=function(data,status){var options;var v='';if(data.length==0){options=s3_gis_empty_set;}else{options=s3_gis_select_location;for(var i=0;i<data.length;i++){v=data[i].id;options+='<option value="'+data[i].id+'">'+data[i].name+'</option>';}}
...
s3_gis_dropdown_hide(level+2);if(''==$('#gis_location_name').val()){$('#'+s3_gis_location_id).val(new_id);}}else{s3_gis_dropdown_hide(level+1);if((0==level)&&(''==$('#gis_location_name').val())){$('#'+s3_gis_location_id).val('');}}}
