Calendar=function(a,f,g,h){this.timeout=this.getDateText=this.getDateToolTip=this.getDateStatus=this.currentDateEl=this.activeDiv=null;this.onSelected=g||null;this.onClose=h||null;this.hidden=this.dragging=false;this.minYear=1970;this.maxYear=2050;this.dateFormat=Calendar._TT.DEF_DATE_FORMAT;this.ttDateFormat=Calendar._TT.TT_DATE_FORMAT;this.weekNumbers=this.isPopup=true;this.firstDayOfWeek=typeof a=="number"?a:Calendar._FD;this.showsOtherMonths=false;this.dateStr=f;this.ar_days=null;this.showsTime=
false;this.time24=true;this.yearStep=2;this.hiliteToday=true;this.activeYear=this.hilitedYear=this.activeMonth=this.hilitedMonth=this.yearsCombo=this.monthsCombo=this.firstdayname=this.tbody=this.element=this.table=this.multiple=null;this.dateClicked=false;if(typeof Calendar._SDN=="undefined"){if(typeof Calendar._SDN_len=="undefined")Calendar._SDN_len=3;a=[];for(f=8;f>0;)a[--f]=Calendar._DN[f].substr(0,Calendar._SDN_len);Calendar._SDN=a;if(typeof Calendar._SMN_len=="undefined")Calendar._SMN_len=3;
a=[];for(f=12;f>0;)a[--f]=Calendar._MN[f].substr(0,Calendar._SMN_len);Calendar._SMN=a}};Calendar._C=null;Calendar.is_ie=/msie/i.test(navigator.userAgent)&&!/opera/i.test(navigator.userAgent);Calendar.is_ie5=Calendar.is_ie&&/msie 5\.0/i.test(navigator.userAgent);Calendar.is_opera=/opera/i.test(navigator.userAgent);Calendar.is_khtml=/Konqueror|Safari|KHTML/i.test(navigator.userAgent);
Calendar.getAbsolutePos=function(a){var f=0,g=0,h=/^div$/i.test(a.tagName);if(h&&a.scrollLeft)f=a.scrollLeft;if(h&&a.scrollTop)g=a.scrollTop;f={x:a.offsetLeft-f,y:a.offsetTop-g};if(a.offsetParent){a=this.getAbsolutePos(a.offsetParent);f.x+=a.x;f.y+=a.y}return f};Calendar.isRelated=function(a,f){var g=f.relatedTarget;if(!g){var h=f.type;if(h=="mouseover")g=f.fromElement;else if(h=="mouseout")g=f.toElement}for(;g;){if(g==a)return true;g=g.parentNode}return false};
Calendar.removeClass=function(a,f){if(a&&a.className){for(var g=a.className.split(" "),h=[],j=g.length;j>0;)if(g[--j]!=f)h[h.length]=g[j];a.className=h.join(" ")}};Calendar.addClass=function(a,f){Calendar.removeClass(a,f);a.className+=" "+f};Calendar.getElement=function(a){for(a=Calendar.is_ie?window.event.srcElement:a.currentTarget;a.nodeType!=1||/^div$/i.test(a.tagName);)a=a.parentNode;return a};
Calendar.getTargetElement=function(a){for(a=Calendar.is_ie?window.event.srcElement:a.target;a.nodeType!=1;)a=a.parentNode;return a};Calendar.stopEvent=function(a){a||(a=window.event);if(Calendar.is_ie){a.cancelBubble=true;a.returnValue=false}else{a.preventDefault();a.stopPropagation()}return false};Calendar.addEvent=function(a,f,g){if(a.attachEvent)a.attachEvent("on"+f,g);else if(a.addEventListener)a.addEventListener(f,g,true);else a["on"+f]=g};
Calendar.removeEvent=function(a,f,g){if(a.detachEvent)a.detachEvent("on"+f,g);else if(a.removeEventListener)a.removeEventListener(f,g,true);else a["on"+f]=null};Calendar.createElement=function(a,f){var g=null;g=document.createElementNS?document.createElementNS("http://www.w3.org/1999/xhtml",a):document.createElement(a);typeof f!="undefined"&&f.appendChild(g);return g};
...
Calendar.showMonthsCombo=function(){var a=Calendar._C;if(!a)return false;var f=a.activeDiv,g=a.monthsCombo;a.hilitedMonth&&Calendar.removeClass(a.hilitedMonth,"hilite");a.activeMonth&&Calendar.removeClass(a.activeMonth,"active");var h=a.monthsCombo.getElementsByTagName("div")[a.date.getMonth()];Calendar.addClass(h,"active");a.activeMonth=h;a=g.style;a.display="block";if(f.navtype<0)a.left=f.offsetLeft+"px";else{g=g.offsetWidth;if(typeof g=="undefined")g=50;a.left=f.offsetLeft+f.offsetWidth-g+"px"}a.top=
f.offsetTop+f.offsetHeight+"px"};
Calendar.showYearsCombo=function(a){var f=Calendar._C;if(!f)return false;var g=f.activeDiv,h=f.yearsCombo;f.hilitedYear&&Calendar.removeClass(f.hilitedYear,"hilite");f.activeYear&&Calendar.removeClass(f.activeYear,"active");f.activeYear=null;for(var j=f.date.getFullYear()+(a?1:-1),b=h.firstChild,c=false,e=12;e>0;--e){if(j>=f.minYear&&j<=f.maxYear){b.innerHTML=j;b.year=j;b.style.display="block";c=true}else b.style.display="none";b=b.nextSibling;j+=a?f.yearStep:-f.yearStep}if(c){a=h.style;a.display=
"block";if(g.navtype<0)a.left=g.offsetLeft+"px";else{h=h.offsetWidth;if(typeof h=="undefined")h=50;a.left=g.offsetLeft+g.offsetWidth-h+"px"}a.top=g.offsetTop+g.offsetHeight+"px"}};
Calendar.tableMouseUp=function(a){var f=Calendar._C;if(!f)return false;f.timeout&&clearTimeout(f.timeout);var g=f.activeDiv;if(!g)return false;var h=Calendar.getTargetElement(a);a||(a=window.event);Calendar.removeClass(g,"active");if(h==g||h.parentNode==g)Calendar.cellClick(g,a);var j=Calendar.findMonth(h);g=null;if(j){g=new Date(f.date);if(j.month!=g.getMonth()){g.setMonth(j.month);f.setDate(g);f.dateClicked=false;f.callHandler()}}else if(h=Calendar.findYear(h)){g=new Date(f.date);if(h.year!=g.getFullYear()){g.setFullYear(h.year);
f.setDate(g);f.dateClicked=false;f.callHandler()}}with(Calendar){removeEvent(document,"mouseup",tableMouseUp);removeEvent(document,"mouseover",tableMouseOver);removeEvent(document,"mousemove",tableMouseOver);f._hideCombos();_C=null;return stopEvent(a)}};
Calendar.tableMouseOver=function(a){var f=Calendar._C;if(f){var g=f.activeDiv,h=Calendar.getTargetElement(a);if(h==g||h.parentNode==g){Calendar.addClass(g,"hilite active");Calendar.addClass(g.parentNode,"rowhilite")}else{if(typeof g.navtype=="undefined"||g.navtype!=50&&(g.navtype==0||Math.abs(g.navtype)>2))Calendar.removeClass(g,"active");Calendar.removeClass(g,"hilite");Calendar.removeClass(g.parentNode,"rowhilite")}a||(a=window.event);if(g.navtype==50&&h!=g){var j=Calendar.getAbsolutePos(g),b=g.offsetWidth,
c=a.clientX,e=true;if(c>j.x+b){c=c-j.x-b;e=false}else c=j.x-c;if(c<0)c=0;j=g._range;b=g._current;c=Math.floor(c/10)%j.length;for(var k=j.length;--k>=0;)if(j[k]==b)break;for(;c-- >0;)if(e){if(--k<0)k=j.length-1}else if(++k>=j.length)k=0;g.innerHTML=j[k];f.onUpdateTime()}if(g=Calendar.findMonth(h))if(g.month!=f.date.getMonth()){f.hilitedMonth&&Calendar.removeClass(f.hilitedMonth,"hilite");Calendar.addClass(g,"hilite");f.hilitedMonth=g}else f.hilitedMonth&&Calendar.removeClass(f.hilitedMonth,"hilite");
else{f.hilitedMonth&&Calendar.removeClass(f.hilitedMonth,"hilite");if(h=Calendar.findYear(h))if(h.year!=f.date.getFullYear()){f.hilitedYear&&Calendar.removeClass(f.hilitedYear,"hilite");Calendar.addClass(h,"hilite");f.hilitedYear=h}else f.hilitedYear&&Calendar.removeClass(f.hilitedYear,"hilite");else f.hilitedYear&&Calendar.removeClass(f.hilitedYear,"hilite")}return Calendar.stopEvent(a)}};Calendar.tableMouseDown=function(a){if(Calendar.getTargetElement(a)==Calendar.getElement(a))return Calendar.stopEvent(a)};
Calendar.calDragIt=function(a){var f=Calendar._C;if(!(f&&f.dragging))return false;var g,h;if(Calendar.is_ie){h=window.event.clientY+document.body.scrollTop;g=window.event.clientX+document.body.scrollLeft}else{g=a.pageX;h=a.pageY}f.hideShowCovered();var j=f.element.style;j.left=g-f.xOffs+"px";j.top=h-f.yOffs+"px";return Calendar.stopEvent(a)};
Calendar.calDragEnd=function(a){var f=Calendar._C;if(!f)return false;f.dragging=false;with(Calendar){removeEvent(document,"mousemove",calDragIt);removeEvent(document,"mouseup",calDragEnd);tableMouseUp(a)}f.hideShowCovered()};
Calendar.dayMouseDown=function(a){var f=Calendar.getElement(a);if(f.disabled)return false;var g=f.calendar;g.activeDiv=f;Calendar._C=g;if(f.navtype!=300)with(Calendar){if(f.navtype==50){f._current=f.innerHTML;addEvent(document,"mousemove",tableMouseOver)}else addEvent(document,Calendar.is_ie5?"mousemove":"mouseover",tableMouseOver);addClass(f,"hilite active");addEvent(document,"mouseup",tableMouseUp)}else g.isPopup&&g._dragStart(a);if(f.navtype==-1||f.navtype==1){g.timeout&&clearTimeout(g.timeout);
g.timeout=setTimeout("Calendar.showMonthsCombo()",250)}else if(f.navtype==-2||f.navtype==2){g.timeout&&clearTimeout(g.timeout);g.timeout=setTimeout(f.navtype>0?"Calendar.showYearsCombo(true)":"Calendar.showYearsCombo(false)",250)}else g.timeout=null;return Calendar.stopEvent(a)};Calendar.dayMouseDblClick=function(a){Calendar.cellClick(Calendar.getElement(a),a||window.event);Calendar.is_ie&&document.selection.empty()};
Calendar.dayMouseOver=function(a){var f=Calendar.getElement(a);if(Calendar.isRelated(f,a)||Calendar._C||f.disabled)return false;if(f.ttip){if(f.ttip.substr(0,1)=="_")f.ttip=f.caldate.print(f.calendar.ttDateFormat)+f.ttip.substr(1);f.calendar.tooltips.innerHTML=f.ttip}if(f.navtype!=300){Calendar.addClass(f,"hilite");f.caldate&&Calendar.addClass(f.parentNode,"rowhilite")}return Calendar.stopEvent(a)};
Calendar.dayMouseOut=function(a){with(Calendar){var f=getElement(a);if(isRelated(f,a)||_C||f.disabled)return false;removeClass(f,"hilite");f.caldate&&removeClass(f.parentNode,"rowhilite");if(f.calendar)f.calendar.tooltips.innerHTML=_TT.SEL_DATE;return stopEvent(a)}};
Calendar.cellClick=function(a,f){var g=a.calendar,h=false,j=false,b=null;if(typeof a.navtype=="undefined"){if(g.currentDateEl){Calendar.removeClass(g.currentDateEl,"selected");Calendar.addClass(a,"selected");h=g.currentDateEl==a;if(!h)g.currentDateEl=a}g.date.setDateOnly(a.caldate);b=g.date;var c=!(g.dateClicked=!a.otherMonth);if(!c&&!g.currentDateEl)g._toggleMultipleDate(new Date(b));else j=!a.disabled;c&&g._init(g.firstDayOfWeek,b)}else{if(a.navtype==200){Calendar.removeClass(a,"hilite");g.callCloseHandler();
return}b=new Date(g.date);a.navtype==0&&b.setDateOnly(new Date);g.dateClicked=false;c=b.getFullYear();var e=b.getMonth(),k=function(m){var o=b.getDate(),n=b.getMonthDays(m);o>n&&b.setDate(n);b.setMonth(m)};switch(a.navtype){case 400:Calendar.removeClass(a,"hilite");h=Calendar._TT.ABOUT;if(typeof h!="undefined")h+=g.showsTime?Calendar._TT.ABOUT_TIME:"";else h='Help and about box text is not translated into this language.\nIf you know this language and you feel generous please update\nthe corresponding file in "lang" subdir to match calendar-en.js\nand send it back to <mihai_bazon@yahoo.com> to get it into the distribution  ;-)\n\nThank you!\nhttp://dynarch.com/mishoo/calendar.epl\n';
alert(h);return;case -2:c>g.minYear&&b.setFullYear(c-1);break;case -1:if(e>0)k(e-1);else if(c-- >g.minYear){b.setFullYear(c);k(11)}break;case 1:if(e<11)k(e+1);else if(c<g.maxYear){b.setFullYear(c+1);k(0)}break;case 2:c<g.maxYear&&b.setFullYear(c+1);break;case 100:g.setFirstDayOfWeek(a.fdow);return;case 50:h=a._range;j=a.innerHTML;for(c=h.length;--c>=0;)if(h[c]==j)break;if(f&&f.shiftKey){if(--c<0)c=h.length-1}else if(++c>=h.length)c=0;a.innerHTML=h[c];g.onUpdateTime();return;case 0:if(typeof g.getDateStatus==
"function"&&g.getDateStatus(b,b.getFullYear(),b.getMonth(),b.getDate()))return false}if(b.equalsTo(g.date)){if(a.navtype==0)j=h=true}else{g.setDate(b);j=true}}j&&f&&g.callHandler();if(h){Calendar.removeClass(a,"hilite");f&&g.callCloseHandler()}};
Calendar.prototype.create=function(a){var f=null;if(a){f=a;this.isPopup=false}else{f=document.getElementsByTagName("body")[0];this.isPopup=true}this.date=this.dateStr?new Date(this.dateStr):new Date;this.table=a=Calendar.createElement("table");a.cellSpacing=0;a.cellPadding=0;a.calendar=this;Calendar.addEvent(a,"mousedown",Calendar.tableMouseDown);var g=Calendar.createElement("div");this.element=g;g.className="calendar";if(this.isPopup){g.style.position="absolute";g.style.display="none"}g.appendChild(a);
var h=Calendar.createElement("thead",a),j=null,b=null,c=this;g=function(m,o,n){j=Calendar.createElement("td",b);j.colSpan=o;j.className="button";if(n!=0&&Math.abs(n)<=2)j.className+=" nav";Calendar._add_evs(j);j.calendar=c;j.navtype=n;j.innerHTML="<div unselectable='on'>"+m+"</div>";return j};b=Calendar.createElement("tr",h);var e=6;this.isPopup&&--e;this.weekNumbers&&++e;g("?",1,400).ttip=Calendar._TT.INFO;this.title=g("",e,300);this.title.className="title";if(this.isPopup){this.title.ttip=Calendar._TT.DRAG_TO_MOVE;
this.title.style.cursor="move";g("&#x00d7;",1,200).ttip=Calendar._TT.CLOSE}b=Calendar.createElement("tr",h);b.className="headrow";this._nav_py=g("&#x00ab;",1,-2);this._nav_py.ttip=Calendar._TT.PREV_YEAR;this._nav_pm=g("&#x2039;",1,-1);this._nav_pm.ttip=Calendar._TT.PREV_MONTH;this._nav_now=g(Calendar._TT.TODAY,this.weekNumbers?4:3,0);this._nav_now.ttip=Calendar._TT.GO_TODAY;this._nav_nm=g("&#x203a;",1,1);this._nav_nm.ttip=Calendar._TT.NEXT_MONTH;this._nav_ny=g("&#x00bb;",1,2);this._nav_ny.ttip=Calendar._TT.NEXT_YEAR;
...
this.callHandler()}})()}else this.onSetTime=this.onUpdateTime=function(){};a=Calendar.createElement("tfoot",a);b=Calendar.createElement("tr",a);b.className="footrow";j=g(Calendar._TT.SEL_DATE,this.weekNumbers?8:7,300);j.className="ttip";if(this.isPopup){j.ttip=Calendar._TT.DRAG_TO_MOVE;j.style.cursor="move"}this.tooltips=j;this.monthsCombo=g=Calendar.createElement("div",this.element);g.className="combo";for(h=0;h<Calendar._MN.length;++h){a=Calendar.createElement("div");a.className=Calendar.is_ie?
"label-IEfix":"label";a.month=h;a.innerHTML=Calendar._SMN[h];g.appendChild(a)}this.yearsCombo=g=Calendar.createElement("div",this.element);g.className="combo";for(h=12;h>0;--h){a=Calendar.createElement("div");a.className=Calendar.is_ie?"label-IEfix":"label";g.appendChild(a)}this._init(this.firstDayOfWeek,this.date);f.appendChild(this.element)};
Calendar._keyEvent=function(a){var f=window._dynarch_popupCalendar;if(!f||f.multiple)return false;Calendar.is_ie&&(a=window.event);var g=Calendar.is_ie||a.type=="keypress",h=a.keyCode;if(a.ctrlKey)switch(h){case 37:g&&Calendar.cellClick(f._nav_pm);break;case 38:g&&Calendar.cellClick(f._nav_py);break;case 39:g&&Calendar.cellClick(f._nav_nm);break;case 40:g&&Calendar.cellClick(f._nav_ny);break;default:return false}else switch(h){case 32:Calendar.cellClick(f._nav_now);break;case 27:g&&f.callCloseHandler();
break;case 37:case 38:case 39:case 40:if(g){var j,b,c,e,k;g=h==37||h==38;k=h==37||h==39?1:7;var m=function(){e=f.currentDateEl;var p=e.pos;j=p&15;b=p>>4;c=f.ar_days[b][j]};m();for(var o=function(){var p=new Date(f.date);p.setDate(p.getDate()-k);f.setDate(p)},n=function(){var p=new Date(f.date);p.setDate(p.getDate()+k);f.setDate(p)};;){switch(h){case 37:if(--j>=0)c=f.ar_days[b][j];else{j=6;h=38;continue}break;case 38:if(--b>=0)c=f.ar_days[b][j];else{o();m()}break;case 39:if(++j<7)c=f.ar_days[b][j];
else{j=0;h=40;continue}break;case 40:if(++b<f.ar_days.length)c=f.ar_days[b][j];else{n();m()}}break}if(c)if(c.disabled)g?o():n();else Calendar.cellClick(c)}break;case 13:g&&Calendar.cellClick(f.currentDateEl,a);break;default:return false}return Calendar.stopEvent(a)};
Calendar.prototype._init=function(a,f){var g=new Date,h=g.getFullYear(),j=g.getMonth();g=g.getDate();this.table.style.visibility="hidden";var b=f.getFullYear();if(b<this.minYear){b=this.minYear;f.setFullYear(b)}else if(b>this.maxYear){b=this.maxYear;f.setFullYear(b)}this.firstDayOfWeek=a;this.date=new Date(f);var c=f.getMonth(),e=f.getDate();f.getMonthDays();f.setDate(1);var k=(f.getDay()-this.firstDayOfWeek)%7;if(k<0)k+=7;f.setDate(0-k);f.setDate(f.getDate()+1);k=this.tbody.firstChild;for(var m=
this.ar_days=[],o=Calendar._TT.WEEKEND,n=this.multiple?this.datesCells={}:null,p=0;p<6;++p,k=k.nextSibling){var t=k.firstChild;if(this.weekNumbers){t.className="day wn";t.innerHTML=f.getWeekNumber();t=t.nextSibling}k.className="daysrow";for(var x=false,w,u=m[p]=[],r=0;r<7;++r,t=t.nextSibling,f.setDate(w+1)){w=f.getDate();var y=f.getDay();t.className="day";t.pos=p<<4|r;u[r]=t;var z=f.getMonth()==c;if(z){t.otherMonth=false;x=true}else if(this.showsOtherMonths){t.className+=" othermonth";t.otherMonth=
true}else{t.className="emptycell";t.innerHTML="&nbsp;";t.disabled=true;continue}t.disabled=false;t.innerHTML=this.getDateText?this.getDateText(f,w):w;if(n)n[f.print("%Y%m%d")]=t;if(this.getDateStatus){var B=this.getDateStatus(f,b,c,w);if(this.getDateToolTip){var C=this.getDateToolTip(f,b,c,w);if(C)t.title=C}if(B===true){t.className+=" disabled";t.disabled=true}else{if(/disabled/i.test(B))t.disabled=true;t.className+=" "+B}}if(!t.disabled){t.caldate=new Date(f);t.ttip="_";if(!this.multiple&&z&&w==
e&&this.hiliteToday){t.className+=" selected";this.currentDateEl=t}if(f.getFullYear()==h&&f.getMonth()==j&&w==g){t.className+=" today";t.ttip+=Calendar._TT.PART_TODAY}if(o.indexOf(y.toString())!=-1)t.className+=t.otherMonth?" oweekend":" weekend"}}if(!(x||this.showsOtherMonths))k.className="emptyrow"}this.title.innerHTML=Calendar._MN[c]+", "+b;this.onSetTime();this.table.style.visibility="visible";this._initMultipleDates()};
Calendar.prototype._initMultipleDates=function(){if(this.multiple)for(var a in this.multiple){var f=this.datesCells[a];if(this.multiple[a])if(f)f.className+=" selected"}};Calendar.prototype._toggleMultipleDate=function(a){if(this.multiple){var f=a.print("%Y%m%d"),g=this.datesCells[f];if(g)if(this.multiple[f]){Calendar.removeClass(g,"selected");delete this.multiple[f]}else{Calendar.addClass(g,"selected");this.multiple[f]=a}}};
...
Calendar.prototype.setRange=function(a,f){this.minYear=a;this.maxYear=f};Calendar.prototype.callHandler=function(){if(this.onSelected)this.onSelected(this,this.date.print(this.dateFormat))};Calendar.prototype.callCloseHandler=function(){if(this.onClose)this.onClose(this);this.hideShowCovered()};Calendar.prototype.destroy=function(){this.element.parentNode.removeChild(this.element);Calendar._C=null;window._dynarch_popupCalendar=null};
Calendar.prototype.reparent=function(a){var f=this.element;f.parentNode.removeChild(f);a.appendChild(f)};Calendar._checkCalendar=function(a){var f=window._dynarch_popupCalendar;if(!f)return false;for(var g=Calendar.is_ie?Calendar.getElement(a):Calendar.getTargetElement(a);g!=null&&g!=f.element;g=g.parentNode);if(g==null){window._dynarch_popupCalendar.callCloseHandler();return Calendar.stopEvent(a)}};
Calendar.prototype.show=function(){for(var a=this.table.getElementsByTagName("tr"),f=a.length;f>0;){var g=a[--f];Calendar.removeClass(g,"rowhilite");g=g.getElementsByTagName("td");for(var h=g.length;h>0;){var j=g[--h];Calendar.removeClass(j,"hilite");Calendar.removeClass(j,"active")}}this.element.style.display="block";this.hidden=false;if(this.isPopup){window._dynarch_popupCalendar=this;Calendar.addEvent(document,"keydown",Calendar._keyEvent);Calendar.addEvent(document,"keypress",Calendar._keyEvent);
Calendar.addEvent(document,"mousedown",Calendar._checkCalendar)}this.hideShowCovered()};Calendar.prototype.hide=function(){if(this.isPopup){Calendar.removeEvent(document,"keydown",Calendar._keyEvent);Calendar.removeEvent(document,"keypress",Calendar._keyEvent);Calendar.removeEvent(document,"mousedown",Calendar._checkCalendar)}this.element.style.display="none";this.hidden=true;this.hideShowCovered()};Calendar.prototype.showAt=function(a,f){var g=this.element.style;g.left=a+"px";g.top=f+"px";this.show()};
Calendar.prototype.showAtElement=function(a,f){var g=this,h=Calendar.getAbsolutePos(a);if(!f||typeof f!="string"){this.showAt(h.x,h.y+a.offsetHeight);return true}this.element.style.display="block";Calendar.continuation_for_the_fucking_khtml_browser=function(){var j=g.element.offsetWidth,b=g.element.offsetHeight;g.element.style.display="none";var c=f.substr(0,1),e="l";if(f.length>1)e=f.substr(1,1);switch(c){case "T":h.y-=b;break;case "B":h.y+=a.offsetHeight;break;case "C":h.y+=(a.offsetHeight-b)/2;
break;case "t":h.y+=a.offsetHeight-b}switch(e){case "L":h.x-=j;break;case "R":h.x+=a.offsetWidth;break;case "C":h.x+=(a.offsetWidth-j)/2;break;case "l":h.x+=a.offsetWidth-j}h.width=j;h.height=b+40;g.monthsCombo.style.display="none";if(h.x<0)h.x=0;if(h.y<0)h.y=0;j=document.createElement("div");b=j.style;b.position="absolute";b.right=b.bottom=b.width=b.height="0px";document.body.appendChild(j);b=Calendar.getAbsolutePos(j);document.body.removeChild(j);if(Calendar.is_ie){b.y+=document.body.scrollTop;
b.x+=document.body.scrollLeft}else{b.y+=window.scrollY;b.x+=window.scrollX}j=h.x+h.width-b.x;if(j>0)h.x-=j;j=h.y+h.height-b.y;if(j>0)h.y-=j;g.showAt(h.x,h.y)};Calendar.is_khtml?setTimeout("Calendar.continuation_for_the_fucking_khtml_browser()",10):Calendar.continuation_for_the_fucking_khtml_browser()};Calendar.prototype.setDateFormat=function(a){this.dateFormat=a};Calendar.prototype.setTtDateFormat=function(a){this.ttDateFormat=a};
Calendar.prototype.parseDate=function(a,f){if(!f)f=this.dateFormat;this.setDate(Date.parseDate(a,f))};
Calendar.prototype.hideShowCovered=function(){function a(x){var w=x.style.visibility;w||(w=document.defaultView&&typeof document.defaultView.getComputedStyle=="function"?Calendar.is_khtml?"":document.defaultView.getComputedStyle(x,"").getPropertyValue("visibility"):x.currentStyle?x.currentStyle.visibility:"");return w}if(Calendar.is_ie||Calendar.is_opera){var f=["applet","iframe","select"],g=this.element,h=Calendar.getAbsolutePos(g),j=h.x,b=g.offsetWidth+j,c=h.y;g=g.offsetHeight+c;for(var e=f.length;e>
0;)for(var k=document.getElementsByTagName(f[--e]),m=null,o=k.length;o>0;){m=k[--o];h=Calendar.getAbsolutePos(m);var n=h.x,p=m.offsetWidth+n;h=h.y;var t=m.offsetHeight+h;if(this.hidden||n>b||p<j||h>g||t<c){if(!m.__msh_save_visibility)m.__msh_save_visibility=a(m);m.style.visibility=m.__msh_save_visibility}else{if(!m.__msh_save_visibility)m.__msh_save_visibility=a(m);m.style.visibility="hidden"}}}};
Calendar.prototype._displayWeekdays=function(){for(var a=this.firstDayOfWeek,f=this.firstdayname,g=Calendar._TT.WEEKEND,h=0;h<7;++h){f.className="day name";var j=(h+a)%7;if(h){f.ttip=Calendar._TT.DAY_FIRST.replace("%s",Calendar._DN[j]);f.navtype=100;f.calendar=this;f.fdow=j;Calendar._add_evs(f)}g.indexOf(j.toString())!=-1&&Calendar.addClass(f,"weekend");f.innerHTML=Calendar._SDN[(h+a)%7];f=f.nextSibling}};
Calendar.prototype._hideCombos=function(){this.monthsCombo.style.display="none";this.yearsCombo.style.display="none"};
Calendar.prototype._dragStart=function(a){if(!this.dragging){this.dragging=true;var f;if(Calendar.is_ie){f=window.event.clientY+document.body.scrollTop;a=window.event.clientX+document.body.scrollLeft}else{f=a.clientY+window.scrollY;a=a.clientX+window.scrollX}var g=this.element.style;this.xOffs=a-parseInt(g.left);this.yOffs=f-parseInt(g.top);with(Calendar){addEvent(document,"mousemove",calDragIt);addEvent(document,"mouseup",calDragEnd)}}};Date._MD=[31,28,31,30,31,30,31,31,30,31,30,31];
Date.SECOND=1E3;Date.MINUTE=60*Date.SECOND;Date.HOUR=60*Date.MINUTE;Date.DAY=24*Date.HOUR;Date.WEEK=7*Date.DAY;
Date.parseDate=function(a,f){var g=new Date,h=0,j=-1,b=0,c=a.split(/\W+/),e=f.match(/%./g),k=0,m=0,o=0,n=0;for(k=0;k<c.length;++k)if(c[k])switch(e[k]){case "%d":case "%e":b=parseInt(c[k],10);break;case "%m":j=parseInt(c[k],10)-1;break;case "%Y":case "%y":h=parseInt(c[k],10);h<100&&(h+=h>29?1900:2E3);break;case "%b":case "%B":for(m=0;m<12;++m)if(Calendar._MN[m].substr(0,c[k].length).toLowerCase()==c[k].toLowerCase()){j=m;break}break;case "%H":case "%I":case "%k":case "%l":o=parseInt(c[k],10);break;
case "%P":case "%p":if(/pm/i.test(c[k])&&o<12)o+=12;else if(/am/i.test(c[k])&&o>=12)o-=12;break;case "%M":n=parseInt(c[k],10)}if(isNaN(h))h=g.getFullYear();if(isNaN(j))j=g.getMonth();if(isNaN(b))b=g.getDate();if(isNaN(o))o=g.getHours();if(isNaN(n))n=g.getMinutes();if(h!=0&&j!=-1&&b!=0)return new Date(h,j,b,o,n,0);h=0;j=-1;for(k=b=0;k<c.length;++k)if(c[k].search(/[a-zA-Z]+/)!=-1){e=-1;for(m=0;m<12;++m)if(Calendar._MN[m].substr(0,c[k].length).toLowerCase()==c[k].toLowerCase()){e=m;break}if(e!=-1){if(j!=
-1)b=j+1;j=e}}else if(parseInt(c[k],10)<=12&&j==-1)j=c[k]-1;else if(parseInt(c[k],10)>31&&h==0){h=parseInt(c[k],10);h<100&&(h+=h>29?1900:2E3)}else if(b==0)b=c[k];if(h==0)h=g.getFullYear();if(j!=-1&&b!=0)return new Date(h,j,b,o,n,0);return g};Date.prototype.getMonthDays=function(a){var f=this.getFullYear();if(typeof a=="undefined")a=this.getMonth();return 0==f%4&&(0!=f%100||0==f%400)&&a==1?29:Date._MD[a]};
...
Date.prototype.print=function(a){var f=this.getMonth(),g=this.getDate(),h=this.getFullYear(),j=this.getWeekNumber(),b=this.getDay(),c={},e=this.getHours(),k=e>=12,m=k?e-12:e,o=this.getDayOfYear();if(m==0)m=12;var n=this.getMinutes(),p=this.getSeconds();c["%a"]=Calendar._SDN[b];c["%A"]=Calendar._DN[b];c["%b"]=Calendar._SMN[f];c["%B"]=Calendar._MN[f];c["%C"]=1+Math.floor(h/100);c["%d"]=g<10?"0"+g:g;c["%e"]=g;c["%H"]=e<10?"0"+e:e;c["%I"]=m<10?"0"+m:m;c["%j"]=o<100?o<10?"00"+o:"0"+o:o;c["%k"]=e;c["%l"]=
m;c["%m"]=f<9?"0"+(1+f):1+f;c["%M"]=n<10?"0"+n:n;c["%n"]="\n";c["%p"]=k?"PM":"AM";c["%P"]=k?"pm":"am";c["%s"]=Math.floor(this.getTime()/1E3);c["%S"]=p<10?"0"+p:p;c["%t"]="\t";c["%U"]=c["%W"]=c["%V"]=j<10?"0"+j:j;c["%u"]=b+1;c["%w"]=b;c["%y"]=(""+h).substr(2,2);c["%Y"]=h;c["%%"]="%";f=/%./g;if(!Calendar.is_ie5&&!Calendar.is_khtml)return a.replace(f,function(t){return c[t]||t});g=a.match(f);for(h=0;h<g.length;h++)if(j=c[g[h]]){f=RegExp(g[h],"g");a=a.replace(f,j)}return a};
...
$(document).ready(function(){function a(){try{g.abort()}catch(j){}$("#TotalQuantity").remove();$("#inv_quantity_ajax_throbber").length==0&&$('[name = "quantity"]').after('<div id="inv_quantity_ajax_throbber" class="ajax_throbber" style="float:right"/>');url="/eden/inv/inv_item_quantity/";url+=$('[name = "inv_item_id"]').val();g=$.getJSON(url,function(b){var c=b.inv_inv_item.quantity;b=b.supply_item_pack.quantity;var e=$('[name = "item_pack_id"] option:selected').text(),k=/\(([0-9]*)\sx/.exec(e);TotalQuantity=
'<span id = "TotalQuantity"> / '+(c*b/(k==null?1:k[1])).toFixed(2)+" "+e+" ("+S3.i18n.in_inv+")</span>";$("#inv_quantity_ajax_throbber").remove();$('[name = "quantity"]').after(TotalQuantity)})}function f(){try{g.abort()}catch(j){}var b=$('[name $= "item_id"]'),c=$('[name = "item_pack_id"]');$('[id$="item_pack_id__row1"]').show();$('[id$="item_pack_id__row"]').show();$("#item_pack_ajax_throbber").length==0&&c.after('<div id="item_pack_ajax_throbber" class="ajax_throbber style="display:inline;"/>');
c.hide();var e;e=$('[name = "item_id"]').length!=0?S3.Ap.concat("/supply/item_pack.json?item_pack.item_id=",b.val()):S3.Ap.concat("/inv/inv_item_packs/",b.val());g=$.ajax({url:e,dataType:"json",context:b,success:function(k){var m="",o="";if(k.length==0)m+='<option value="">'+S3.i18n.no_packs+"</options>";else{for(var n=0;n<k.length;n++)if(k[n].quantity==1){o=k[n].name;break}for(n=0;n<k.length;n++){m+='<option value="'+k[n].id+'">';m+=k[n].quantity==1?k[n].name:k[n].name+" ("+k[n].quantity+" x "+o+
")";m+="</option>"}}c.html(m).val(1).show();href=$("#item_pack_add").attr("href")+"&item_id="+$('[name = "item_id"]').val();$("#item_pack_add").attr("href",href).show();$("#item_pack_ajax_throbber").remove();this.attr("name")=="inv_item_id"&&a()}})}var g;$("#inv_send_item_item_pack_id").change(a);var h=$('[name $= "item_id"]').val();if(h==""|h==undefined){$('[id$="item_pack_id__row1"]').hide();$('[id$="item_pack_id__row"]').hide()}else f();$('[name $= "item_id"]').change(f);$(".quantity.ajax_more").live("click",
...
A.init=function(){O=a(f);H=g().attr({id:o,"class":z?n+"IE":""});P=g("Overlay",B?"position:absolute":"").hide();F=g("Wrapper");J=g("Content").append(N=g("LoadedContent","width:0; height:0; overflow:hidden"),T=g("LoadingOverlay").add(g("LoadingGraphic")),ca=g("Title"),L=g("Current"),ga=g("Next"),Z=g("Previous"),W=g("Slideshow").bind(p,e),R=g("Close"));F.append(g().append(g("TopLeft"),aa=g("TopCenter"),g("TopRight")),g(false,"clear:left").append(ha=g("MiddleLeft"),J,da=g("MiddleRight")),g(false,"clear:left").append(g("BottomLeft"),
ja=g("BottomCenter"),g("BottomRight"))).children().children().css({"float":"left"});I=g(false,"position:absolute; width:9999px; visibility:hidden; display:none");a("body").prepend(P,H.append(F,I));J.children().hover(function(){a(this).addClass("hover")},function(){a(this).removeClass("hover")}).addClass("hover");U=aa.height()+ja.height()+J.outerHeight(true)-J.height();ia=ha.width()+da.width()+J.outerWidth(true)-J.width();fa=N.outerHeight(true);Y=N.outerWidth(true);H.css({"padding-bottom":U,"padding-right":ia}).hide();
...
h(s.maxHeight,"y")-fa-U;s.mh=s.h&&s.h<s.mh?s.h:s.mh}G=s.href;T.show();if(s.inline){g().hide().insertBefore(a(G)[0]).one(r,function(){a(this).replaceWith(N.children())});V(a(G))}else if(s.iframe){H.one(y,function(){var Q=a("<iframe frameborder='0' style='width:100%; height:100%; border:0; display:block'/>")[0];Q.name=n+ +new Date;Q.src=s.href;if(!s.scrolling)Q.scrolling="no";if(z)Q.allowtransparency="true";a(Q).appendTo(N).one(r,function(){Q.src="//about:blank"})});V(" ")}else if(s.html)V(s.html);
...
(function(a,f){function g(){this.debug=true;this._curInst=null;this._isInline=false;this._disabledInputs=[];this._inDialog=this._timepickerShowing=false;this._dialogClass="ui-timepicker-dialog";this._mainDivId="ui-timepicker-div";this._inlineClass="ui-timepicker-inline";this._currentClass="ui-timepicker-current";this._dayOverClass="ui-timepicker-days-cell-over";this.regional=[];this.regional[""]={hourText:"Hour",minuteText:"Minute",amPmText:["AM","PM"]};this._defaults={showOn:"focus",showAnim:"fadeIn",
...
b+"Datepicker"].apply(a.timepicker,[this[0]].concat(c));if(b=="option"&&arguments.length==2&&typeof arguments[1]=="string")return a.timepicker["_"+b+"Datepicker"].apply(a.timepicker,[this[0]].concat(c));return this.each(function(){typeof b=="string"?a.timepicker["_"+b+"Datepicker"].apply(a.timepicker,[this].concat(c)):a.timepicker._attachTimepicker(this,b)})};a.timepicker=new g;a.timepicker.initialized=false;a.timepicker.uuid=(new Date).getTime();a.timepicker.version="1.8.6";window["TP_jQuery_"+j]=
a})(jQuery);
(function(a){a.cluetip={version:"1.0.7"};var f,g,h,j,b,c,e,k;a.fn.cluetip=function(n,p){if(typeof n=="object"){p=n;n=null}if(n=="destroy")return this.removeData("thisInfo").unbind(".cluetip");return this.each(function(t){function x(){return false}var w=this,u=a(this),r=a.extend(true,{},a.fn.cluetip.defaults,p||{},a.metadata?u.metadata():a.meta?u.data():{}),y=false,z=+r.cluezIndex;u.data("thisInfo",{title:w.title,zIndex:z});var B=false,C=0;if(!a("#cluetip").length){a('<div id="cluetip"><div id="cluetip-outer"><h3 id="cluetip-title"></h3><div id="cluetip-inner"></div></div><div id="cluetip-extra"></div><div id="cluetip-arrows" class="cluetip-arrows"></div></div>')[m](o).hide();f=
a("#cluetip").css({position:"absolute"});h=a("#cluetip-outer").css({position:"relative",zIndex:z});g=a("#cluetip-inner");j=a("#cluetip-title");b=a("#cluetip-arrows");c=a('<div id="cluetip-waitimage"></div>').css({position:"absolute"}).insertBefore(f).hide()}var P=r.dropShadow?+r.dropShadowSteps:0;if(!e){e=a([]);for(var H=0;H<P;H++)e=e.add(a("<div></div>").css({zIndex:z-1,opacity:0.1,top:1+H,left:1+H}));e.css({position:"absolute",backgroundColor:"#000"}).prependTo(f)}var F=u.attr(r.attribute),J=r.cluetipClass;
if(!F&&!r.splitTitle&&!n)return true;if(r.local&&r.localPrefix)F=r.localPrefix+F;r.local&&r.hideLocal&&a(F+":first").hide();var aa=parseInt(r.topOffset,10),ha=parseInt(r.leftOffset,10),da,ja,M=isNaN(parseInt(r.height,10))?"auto":/\D/g.test(r.height)?r.height:r.height+"px",O,N,I,T,ca,L=parseInt(r.width,10)||275,W=L+(parseInt(f.css("paddingLeft"),10)||0)+(parseInt(f.css("paddingRight"),10)||0)+P,ga=this.offsetWidth,Z,R,U,ia,fa,Y=r.attribute!="title"?u.attr(r.titleAttribute):"";if(r.splitTitle){if(Y==
undefined)Y="";fa=Y.split(r.splitTitle);Y=fa.shift()}if(r.escapeTitle)Y=Y.replace(/&/g,"&amp;").replace(/>/g,"&gt;").replace(/</g,"&lt;");var X=function(E){if(!r.onActivate(u))return false;B=true;f.removeClass().css({width:L});F==u.attr("href")&&u.css("cursor",r.cursor);r.hoverClass&&u.addClass(r.hoverClass);N=u.offset().top;Z=u.offset().left;U=E.pageX;T=E.pageY;if(w.tagName.toLowerCase()!="area"){O=a(document).scrollTop();ia=a(window).width()}if(r.positionBy=="fixed"){R=ga+Z+ha;f.css({left:R})}else{R=
ga>Z&&Z>W||Z+ga+W+ha>ia?Z-W-ha:ga+Z+ha;if(w.tagName.toLowerCase()=="area"||r.positionBy=="mouse"||ga+W>ia)if(U+20+W>ia){f.addClass(" cluetip-"+J);R=U-W-ha>=0?U-W-ha-parseInt(f.css("marginLeft"),10)+parseInt(g.css("marginRight"),10):U-W/2}else R=U+ha;var A=R<0?E.pageY+aa:E.pageY;f.css({left:R>0&&r.positionBy!="bottomTop"?R:U+W/2>ia?ia/2-W/2:Math.max(U-W/2,0),zIndex:u.data("thisInfo").zIndex});b.css({zIndex:u.data("thisInfo").zIndex+1})}ja=a(window).height();if(n){if(typeof n=="function")n=n.call(w);
g.html(n);K(A)}else if(fa){E=fa.length;g.html(E?fa[0]:"");if(E>1)for(var ka=1;ka<E;ka++)g.append('<div class="split-body">'+fa[ka]+"</div>");K(A)}else if(!r.local&&F.indexOf("#")!==0)if(/\.(jpe?g|tiff?|gif|png)$/i.test(F)){g.html('<img src="'+F+'" alt="'+Y+'" />');K(A)}else if(y&&r.ajaxCache){g.html(y);K(A)}else{var v=r.ajaxSettings.beforeSend,G=r.ajaxSettings.error,D=r.ajaxSettings.success,S=r.ajaxSettings.complete;E=a.extend(true,{},r.ajaxSettings,{cache:false,url:F,beforeSend:function(V){v&&v.call(w,
V,f,g);h.children().empty();r.waitImage&&c.css({top:T+20,left:U+20,zIndex:u.data("thisInfo").zIndex-1}).show()},error:function(V,Q){if(B)G?G.call(w,V,Q,f,g):g.html("<i>sorry, the contents could not be loaded</i>")},success:function(V,Q){y=r.ajaxProcess.call(w,V);if(B){D&&D.call(w,V,Q,f,g);g.html(y)}},complete:function(V,Q){S&&S.call(w,V,Q,f,g);var ba=g[0].getElementsByTagName("img");k=ba.length;for(var la=0,na=ba.length;la<na;la++)ba[la].complete&&k--;if(k&&!a.browser.opera)a(ba).bind("load error",
function(){k--;if(k<1){c.hide();B&&K(A)}});else{c.hide();B&&K(A)}}});a.ajax(E)}else if(r.local){E=a(F+(/#\S+$/.test(F)?"":":eq("+t+")")).clone(true).show();g.html(E);K(A)}},K=function(E){f.addClass("cluetip-"+J);if(r.truncate){var A=g.text().slice(0,r.truncate)+"...";g.html(A)}Y?j.show().html(Y):r.showTitle?j.show().html("&nbsp;"):j.hide();if(r.sticky){A=a('<div id="cluetip-close"><a href="#">'+r.closeText+"</a></div>");r.closePosition=="bottom"?A.appendTo(g):r.closePosition=="title"?A.prependTo(j):
A.prependTo(g);A.bind("click.cluetip",function(){ea();return false});r.mouseOutClose?f.bind("mouseleave.cluetip",function(){ea()}):f.unbind("mouseleave.cluetip")}A="";h.css({zIndex:u.data("thisInfo").zIndex,overflow:M=="auto"?"visible":"auto",height:M});da=M=="auto"?Math.max(f.outerHeight(),f.height()):parseInt(M,10);I=N;ca=O+ja;if(r.positionBy=="fixed")I=N-r.dropShadowSteps+aa;else if(R<U&&Math.max(R,0)+W>U||r.positionBy=="bottomTop")if(N+da+aa>ca&&T-O>da+aa){I=T-da-aa;A="top"}else{I=T+aa;A="bottom"}else I=
N+da+aa>ca?da>=ja?O:ca-da-aa:u.css("display")=="block"||w.tagName.toLowerCase()=="area"||r.positionBy=="mouse"?E-aa:N-r.dropShadowSteps;if(A=="")R<Z?A="left":A="right";f.css({top:I+"px"}).removeClass().addClass("clue-"+A+"-"+J).addClass(" cluetip-"+J);if(r.arrows){E=N-I-r.dropShadowSteps;b.css({top:/(left|right)/.test(A)&&R>=0&&E>0?E+"px":/(left|right)/.test(A)?0:""}).show()}else b.hide();e.hide();f.hide()[r.fx.open](r.fx.openSpeed||0);r.dropShadow&&e.css({height:da,width:L,zIndex:u.data("thisInfo").zIndex-
1}).show();a.fn.bgiframe&&f.bgiframe();if(r.delayedClose>0)C=setTimeout(ea,r.delayedClose);r.onShow.call(w,f,g)},s=function(){B=false;c.hide();if(!r.sticky||/click|toggle/.test(r.activation)){ea();clearTimeout(C)}r.hoverClass&&u.removeClass(r.hoverClass)},ea=function(){h.parent().hide().removeClass();r.onHide.call(w,f,g);u.removeClass("cluetip-clicked");Y&&u.attr(r.titleAttribute,Y);u.css("cursor","");r.arrows&&b.css({top:""})};a(document).bind("hideCluetip",function(){ea()});if(/click|toggle/.test(r.activation))u.bind("click.cluetip",
function(E){if(f.is(":hidden")||!u.is(".cluetip-clicked")){X(E);a(".cluetip-clicked").removeClass("cluetip-clicked");u.addClass("cluetip-clicked")}else s(E);this.blur();return false});else if(r.activation=="focus"){u.bind("focus.cluetip",function(E){X(E)});u.bind("blur.cluetip",function(E){s(E)})}else{u[r.clickThrough?"unbind":"bind"]("click",x);var ma=function(E){if(r.tracking==true){var A=R-E.pageX,ka=I?I-E.pageY:N-E.pageY;u.bind("mousemove.cluetip",function(v){f.css({left:v.pageX+A,top:v.pageY+
...
jQuery&&function(a){function f(c,e,k){for(var m="",o=0;o<e.length;o++)if(e[o].optgroup){m+='<label class="optGroup">';m+=k.optGroupSelectable?'<input type="checkbox" class="optGroup" />'+e[o].optgroup:e[o].optgroup;m+='</label><div class="optGroupContainer">';m+=f(c,e[o].options,k);m+="</div>"}else{var n=e[o],p='<label><input type="checkbox" name="'+c+'[]" value="'+n.value+'"';if(n.selected)p+=' checked="checked"';p+=" />"+n.text+"</label>";m+=p}return m}function g(c){var e=a(this),k=e.next(".multiSelectOptions"),
m=e.data("config"),o=e.data("callback");k.html("");var n="";if(m.selectAll)n+='<label class="selectAll"><input type="checkbox" class="selectAll" />'+m.selectAllText+"</label>";n+=f(e.attr("id"),c,m);k.html(n);c=k.width();n=false;if(k.height()>m.listHeight){k.css("height",m.listHeight+"px");n=true}else k.css("height","");c=n&&c==k.width()?17:0;k.width()+c<e.outerWidth()?k.css("width",e.outerWidth()-2+"px"):k.css("width",k.width()+c+"px");a.fn.bgiframe&&e.next(".multiSelectOptions").bgiframe({width:k.width(),
...
'"');c="'"+DisplayValue+"'"}if(DisplayValue!="")e=false;k=k.replace(g[i],DisplayValue);k=k.replace(h[i],c);b=b+"'"+a.ColumnFields[i]+"':"+c+","}b=b.slice(0,-1)+"}";if(!e&&!j.match(b)){if(j!="")j+=",";j+=b;$("#"+f+"_json").val(j);$(k).insertBefore("#"+a.WidgetID+"_input_row");if(m!=""){$(m).attr("id",o);$("#"+o).attr("name",o);$("#"+o).hide();$('<input class="upload" id="drrpp_framework_file_dummy_file" type="file" />').insertAfter("#"+o)}}}
function S3MultiselectWidgetDeleteClick(a,f){var g=f.WidgetID,h=$(a).attr("row_id");$("#"+f.DummyName).val();var j=$("#"+g+"_json").val();$(a).parent().parent().remove();if(h!="New"){if(j!="")j+=",";j=j+"{'id':'"+h+"','deleted':True},"}else{h="{";for(i=0;i<f.ColumnFields.length;i++)h=h+"'"+f.ColumnFields[i]+"':'"+$(a).attr(f.ColumnFields[i])+"',";h=h.slice(0,-1)+"}";j=j.replace(h,"")}j=j.replace(",,",",");if(j.slice(-1)==",")j=j.slice(0,-1);$("#"+g+"_json").val(j)}
(function(a,f){function g(h){return!a(h).parents().andSelf().filter(function(){return a.curCSS(this,"visibility")==="hidden"||a.expr.filters.hidden(this)}).length}a.ui=a.ui||{};if(!a.ui.version){a.extend(a.ui,{version:"1.8.11",keyCode:{ALT:18,BACKSPACE:8,CAPS_LOCK:20,COMMA:188,COMMAND:91,COMMAND_LEFT:91,COMMAND_RIGHT:93,CONTROL:17,DELETE:46,DOWN:40,END:35,ENTER:13,ESCAPE:27,HOME:36,INSERT:45,LEFT:37,MENU:93,NUMPAD_ADD:107,NUMPAD_DECIMAL:110,NUMPAD_DIVIDE:111,NUMPAD_ENTER:108,NUMPAD_MULTIPLY:106,NUMPAD_SUBTRACT:109,
...
"border"+this+"Width",true))||0;if(p)o-=parseFloat(a.curCSS(m,"margin"+this,true))||0});return o}var c=j==="Width"?["Left","Right"]:["Top","Bottom"],e=j.toLowerCase(),k={innerWidth:a.fn.innerWidth,innerHeight:a.fn.innerHeight,outerWidth:a.fn.outerWidth,outerHeight:a.fn.outerHeight};a.fn["inner"+j]=function(m){if(m===f)return k["inner"+j].call(this);return this.each(function(){a(this).css(e,b(this,m)+"px")})};a.fn["outer"+j]=function(m,o){if(typeof m!=="number")return k["outer"+j].call(this,m);return this.each(function(){a(this).css(e,
b(this,m,true,o)+"px")})}});a.extend(a.expr[":"],{data:function(h,j,b){return!!a.data(h,b[3])},focusable:function(h){var j=h.nodeName.toLowerCase(),b=a.attr(h,"tabindex");if("area"===j){j=h.parentNode;b=j.name;if(!h.href||!b||j.nodeName.toLowerCase()!=="map")return false;h=a("img[usemap=#"+b+"]")[0];return!!h&&g(h)}return(/input|select|textarea|button|object/.test(j)?!h.disabled:"a"==j?h.href||!isNaN(b):!isNaN(b))&&g(h)},tabbable:function(h){var j=a.attr(h,"tabindex");return(isNaN(j)||j>=0)&&a(h).is(":focusable")}});
...
function s3_gis_dropdown_select(a,f){s3_gis_pullNext=false;var g=$("#gis_location_name").val(),h=$("#gis_location_lat").val(),j=$("#gis_location_lon").val(),b=$("#gis_location_addr_street").val(),c=$("#gis_location_postcode").val();g=g||h||j||b||c?true:false;if((h=$("#gis_location_L"+a).val())||f){var e;$.getJSONS3(a==s3_gis_maxlevel?s3_gis_url+"/search.json?filter=%3D&field=level&value=nullnone&parent="+h:s3_gis_url+"/search.json?filter=%3D&field=level&value=L"+(a+1)+"&parent="+h,function(k){if(k.length==
0)e=s3_gis_empty_set;else if(!f&&a!=s3_gis_maxlevel&&k.length==1){e=s3_gis_select_location;e+='<option selected="" value="'+k[0].id+'">'+k[0].name+"</option>";s3_gis_pullNext=true}else{e=s3_gis_select_location;for(var m=0;m<k.length;m++)e+='<option value="'+k[m].id+'">'+k[m].name+"</option>"}a==s3_gis_maxlevel?$("#gis_location_").html(e):$("#gis_location_L"+(a+1)).html(e)},false,true);if(a==s3_gis_maxlevel){if(e!=s3_gis_empty_set){$("#gis_location__label__row").removeClass("hidden").show();$("#gis_location___row").removeClass("hidden").show();
$("#gis_location_").removeClass("hidden").show()}}else{$("#gis_location_L"+(a+1)+"_label__row").removeClass("hidden").show();$("#gis_location_L"+(a+1)+"__row").removeClass("hidden").show();s3_gis_hide_dropdown(a+2)}if(s3_gis_pullNext){h=$("#gis_location_L"+(a+1)).val();g||$("#"+s3_gis_location_id).val(h);s3_gis_dropdown_select(a+1)}else g||$("#"+s3_gis_location_id).val(h)}else if(a!=s3_gis_maxlevel){s3_gis_hide_dropdown(a+1);0==a&&!g&&$("#"+s3_gis_location_id).val("")}}
...
$("#gis_location_advanced_div").removeClass("hidden").show()}function s3_gis_update_details(a){$.getJSONS3(s3_gis_url+"/search.json?filter=%3D&field=id&value="+a,function(f){if(f.length!=0){$("#gis_location_name").val(f[0].name);$("#gis_location_addr_street").val(f[0].addr_street);$("#gis_location_postcode").val(f[0].addr_postcode);$("#gis_location_lat").val(f[0].lat);$("#gis_location_lon").val(f[0].lon)}},false,true)}
function s3_gis_save_location(a,f,g,h,j){var b=$("#gis_location_L5").val();if(undefined==b||""==b){b=$("#gis_location_L4").val();if(undefined==b||""==b){b=$("#gis_location_L3").val();if(undefined==b||""==b){b=$("#gis_location_L2").val();if(undefined==b||""==b){b=$("#gis_location_L1").val();if(undefined==b||""==b)b=$("#gis_location_L0").val()}}}}a=""==S3.gis.uuid?s3_gis_url+"/create.url?name="+a:s3_gis_url+"/update.url?uuid="+S3.gis.uuid+"&name="+a;""==f||""==g||(a=a+"&lat="+f+"&lon="+g);if(""!=h)a=
a+"&addr_street="+h.replace(/\n/g,"%0d");if(""!=j)a=a+"&addr_postcode="+j;undefined==b||""==b||(a=a+"&parent="+b);$.ajax({async:false,url:a,dataType:"json",success:function(c){if(""==S3.gis.uuid&&c.status=="success"){c=c.message.split("=")[1];$("#"+s3_gis_location_id).val(c)}}})}function s3_gis_geolocate(a){var f=a.coords.latitude;a=a.coords.longitude;$("#gis_location_lat").val(f);$("#gis_location_lon").val(a)}var s3_gis_calcDone=false,s3_gis_calcAnswer;
function s3_gis_convertDD(){var a=$("#DDMMSS_deg").val(),f=$("#DDMMSS_min").val(),g=$("#DDMMSS_sec").val(),h=false;if(a=="")a=0;else if(isNaN(a)||a>180||a<-180){alert(s3_gis_degrees_validation_error);h=true}if(f=="")f=0;else if(isNaN(f)||f>60||f<0){alert(s3_gis_minutes_validation_error);h=true}if(g=="")g=0;else if(isNaN(g)||g>60||g<0){alert(s3_gis_seconds_validation_error);h=true}if(h==false){f=f*1+g/60;s3_gis_calcAnswer=a>0?a*1+f/60:a*1-f/60;$("#DDMMSS_dec").val(s3_gis_calcAnswer);s3_gis_calcDone=
true}}function s3_gis_convertGps(){var a=0,f=0;a=$("#gps_deg").val();f=$("#gps_min").val();var g=false;if(a=="")a=0;else if(isNaN(a)||a>180||a<-180){alert(s3_gis_degrees_validation_error);g=true}if(f=="")f=0;else if(isNaN(f)||f>60||f<0){alert(s3_gis_minutes_validation_error);g=true}if(g==false){s3_gis_calcAnswer=a>0?a*1+f/60:a*1-f/60;$("#gps_dec").val(s3_gis_calcAnswer);s3_gis_calcDone=true}}
...
$(function(){if(typeof s3_gis_location_id!="undefined"){$("#gis_location_L0").change(function(){s3_gis_dropdown_select(0)});$("#gis_location_L1").change(function(){s3_gis_dropdown_select(1)});$("#gis_location_L2").change(function(){s3_gis_dropdown_select(2)});$("#gis_location_L3").change(function(){s3_gis_dropdown_select(3)});$("#gis_location_L4").change(function(){s3_gis_dropdown_select(4)});$("#gis_location_").change(function(){var f=$(this).val();$("#"+s3_gis_location_id).val(f);s3_gis_update_details(f);
$("#gis_location__label__row").hide();$("#gis_location___row").hide();$("#gis_location_add-btn").hide();s3_gis_show_details()});$("#gis_location_add-btn").click(function(f){if(""!=S3.gis.uuid){$("body").data("uuid",S3.gis.uuid);S3.gis.uuid=""}var g=$("#gis_location_name").val(),h=$("#gis_location_addr_street").val(),j=$("#gis_location_postcode").val(),b=$("#gis_location_lat").val(),c=$("#gis_location_lat").val();$("body").data("name",g);$("body").data("addr_street",h);$("body").data("postcode",j);
$("body").data("lat",b);$("body").data("lon",c);$("#gis_location_name").val("");$("#gis_location_addr_street").val("");$("#gis_location_postcode").val("");$("#gis_location_lat").val("");$("#gis_location_lon").val("");$("#gis_location_add-btn").hide();$("#gis_location_cancel-btn").removeClass("hidden").show();$("#gis_location__label__row").hide();$("#gis_location___row").hide();navigator.geolocation&&$("#gis_location_geolocate-btn").removeClass("hidden").show();s3_gis_show_details();$("#gis_location_details_hide-btn").hide();
f.preventDefault()});$("#gis_location_cancel-btn").click(function(f){S3.gis.uuid=$("body").data("uuid");$("#gis_location_name").val($("body").data("name"));$("#gis_location_addr_street").val($("body").data("addr_street"));$("#gis_location_postcode").val($("body").data("postcode"));$("#gis_location_lat").val($("body").data("lat"));$("#gis_location_lon").val($("body").data("lon"));$("#gis_location_cancel-btn").hide();$("#gis_location__label__row").show();$("#gis_location___row").show();$("#gis_location_add-btn").show();
$("#gis_location_details_hide-btn").show();s3_gis_hide_details();f.preventDefault()});$("#gis_location_advanced_checkbox").change(function(f){if($("#gis_location_advanced_checkbox").is(":checked")){$("#gis_location_lat_label").removeClass("hidden").show();$("#gis_location_lat_row").removeClass("hidden").show();$("#gis_location_lon_label").removeClass("hidden").show();$("#gis_location_lon_row").removeClass("hidden").show()}else{$("#gis_location_lat_label").hide();$("#gis_location_lat_row").hide();
$("#gis_location_lon_label").hide();$("#gis_location_lon_row").hide()}f.preventDefault()});$("#gis_location_search-btn").click(function(f){$(this).hide();$("#gis_location_autocomplete_div").removeClass("hidden").show();$("#gis_location_add-btn").removeClass("hidden").show();$("#gis_location_L0_label__row").hide();$("#gis_location_L0__row").hide();$("#gis_location_L1_label__row").hide();$("#gis_location_L1__row").hide();$("#gis_location_L2_label__row").hide();$("#gis_location_L2__row").hide();$("#gis_location_L3_label__row").hide();
$("#gis_location_L3__row").hide();$("#gis_location_L4_label__row").hide();$("#gis_location_L4__row").hide();$("#gis_location_L5_label__row").hide();$("#gis_location_L5__row").hide();$("#gis_location___row").hide();$("#gis_location__label__row").hide();$("#gis_location_details-btn").hide();s3_gis_hide_details();f.preventDefault()});$("#gis_location_details-btn").click(function(f){$("#gis_location_add-btn").hide();$("#gis_location___row").hide();$("#gis_location__label__row").hide();$("#gis_location_details-btn").hide();
$("#gis_location_details_hide-btn").removeClass("hidden").show();s3_gis_show_details();f.preventDefault()});$("#gis_location_details_hide-btn").click(function(f){$("#gis_location_add-btn").removeClass("hidden").show();$("#gis_location___row").show();$("#gis_location__label__row").show();$("#gis_location_details-btn").removeClass("hidden").show();$("#gis_location_details_hide-btn").hide();s3_gis_hide_details();f.preventDefault()});$("#gis_location_geolocate-btn").click(function(f){navigator.geolocation.getCurrentPosition(s3_gis_geolocate);
f.preventDefault()});var a={val:$("#gis_location_autocomplete").val(),accept:false};$("#gis_location_autocomplete").autocomplete({source:s3_gis_url+"/search.json?filter=~&field=name&exclude_field=level&exclude_value=XX",minLength:2,focus:function(f,g){$("#gis_location_autocomplete").val(g.item.name);return false},select:function(f,g){$("#gis_location_autocomplete").val(g.item.name);$("#"+s3_gis_location_id).val(g.item.id);a.accept=true;$("ul.ui-autocomplete").hide();$("#gis_location_autocomplete_div").hide();
$("#gis_location_search-btn").show();$("#gis_location_L0_label__row").show();$("#gis_location_L0__row").show();var h=g.item.path,j=g.item.id,b,c,e,k,m=[];switch(g.item.level){case "L0":$("#gis_location_L0").val(j);$("#"+s3_gis_location_id).val(j);s3_gis_dropdown_select(0);break;case "L1":if(h){h=h.split("/");if(h.length==2&&h[1]==j)$("#gis_location_L0").val(h[0]);else{s3_gis_dropdown_select(0,true);$("#gis_location_L1").val(j);$("#"+s3_gis_location_id).val(j);break}}s3_gis_dropdown_select(0,true);
$("#gis_location_L1").val(j);$("#"+s3_gis_location_id).val(j);s3_gis_dropdown_select(1);break;case "L2":if(h){h=h.split("/");if(h.length==3&&h[2]==j){$("#gis_location_L0").val(h[0]);b=h[1]}else if(h.length==2&&h[1]==j)if(undefined!=$("#gis_location_L0 option[value="+h[0]+"]")[0])$("#gis_location_L0").val(h[0]);else b=h[0];else{s3_gis_dropdown_select(1,true);$("#gis_location_L2").val(j);$("#"+s3_gis_location_id).val(j);break}}s3_gis_dropdown_select(0,true);$("#gis_location_L1").val(b);s3_gis_dropdown_select(1,
true);$("#gis_location_L2").val(j);$("#"+s3_gis_location_id).val(j);s3_gis_dropdown_select(2);break;case "L3":if(h){h=h.split("/");if(h.length==4&&h[3]==j){$("#gis_location_L0").val(h[0]);b=h[1];c=h[2]}else if(h.length==3&&h[2]==j)if(undefined!=$("#gis_location_L0 option[value="+h[0]+"]")[0]){$("#gis_location_L0").val(h[0]);m=[h[1]]}else{b=h[0];c=h[1]}else if(h.length==2&&h[1]==j)if(undefined!=$("#gis_location_L0 option[value="+h[0]+"]")[0])$("#gis_location_L0").val(h[0]);else m=[h[0]];else{s3_gis_dropdown_select(2,
true);$("#gis_location_L3").val(j);$("#"+s3_gis_location_id).val(j);break}}s3_gis_dropdown_select(0,true);if(m[0])if(undefined!=$("#gis_location_L1 option[value="+m[0]+"]")[0])b=m[0];$("#gis_location_L1").val(b);s3_gis_dropdown_select(1,true);if(m[0])if(undefined!=$("#gis_location_L2 option[value="+m[0]+"]")[0])c=m[0];$("#gis_location_L2").val(c);s3_gis_dropdown_select(2,true);$("#gis_location_L3").val(j);$("#"+s3_gis_location_id).val(j);s3_gis_dropdown_select(3);break;case "L4":if(h){h=h.split("/");
if(h.length==5&&h[4]==j){$("#gis_location_L0").val(h[0]);b=h[1];c=h[2];e=h[3]}else if(h.length==4&&h[3]==j)if(undefined!=$("#gis_location_L0 option[value="+h[0]+"]")[0]){$("#gis_location_L0").val(h[0]);m=[h[1],h[2]]}else m=[h[0],h[1],h[2]];else if(h.length==3&&h[2]==j)if(undefined!=$("#gis_location_L0 option[value="+h[0]+"]")[0]){$("#gis_location_L0").val(h[0]);m=[h[1]]}else m=[h[0],h[1]];else if(h.length==2&&h[1]==j)if(undefined!=$("#gis_location_L0 option[value="+h[0]+"]")[0])$("#gis_location_L0").val(h[0]);
else m=[h[0]];else{s3_gis_dropdown_select(3,true);$("#gis_location_L4").val(j);$("#"+s3_gis_location_id).val(j);break}}s3_gis_dropdown_select(0,true);if(m[0])if(undefined!=$("#gis_location_L1 option[value="+m[0]+"]")[0])b=m[0];$("#gis_location_L1").val(b);s3_gis_dropdown_select(1,true);if(m[0])if(undefined!=$("#gis_location_L2 option[value="+m[0]+"]")[0])c=m[0];else if(m[1])if(undefined!=$("#gis_location_L2 option[value="+m[1]+"]")[0])c=m[1];$("#gis_location_L2").val(c);s3_gis_dropdown_select(2,true);
if(m[0])if(undefined!=$("#gis_location_L3 option[value="+m[0]+"]")[0])e=m[0];else if(m[1])if(undefined!=$("#gis_location_L3 option[value="+m[1]+"]")[0])e=m[1];else if(m[2])if(undefined!=$("#gis_location_L3 option[value="+m[2]+"]")[0])e=m[2];$("#gis_location_L3").val(e);s3_gis_dropdown_select(3,true);$("#gis_location_L4").val(j);$("#"+s3_gis_location_id).val(j);s3_gis_dropdown_select(4);break;case "L5":break;default:if(h){h=h.split("/");if(h.length==6&&h[5]==j){$("#gis_location_L0").val(h[0]);b=h[1];
c=h[2];e=h[4]}else if(h.length==5&&h[4]==j)if(undefined!=$("#gis_location_L0 option[value="+h[0]+"]")[0]){$("#gis_location_L0").val(h[0]);m=[h[1],h[2],h[3]]}else m=[h[0],h[1],h[2],h[3]];else if(h.length==4&&h[3]==j)if(undefined!=$("#gis_location_L0 option[value="+h[0]+"]")[0]){$("#gis_location_L0").val(h[0]);m=[h[1],h[2]]}else m=[h[0],h[1],h[2]];else if(h.length==3&&h[2]==j)if(undefined!=$("#gis_location_L0 option[value="+h[0]+"]")[0]){$("#gis_location_L0").val(h[0]);m=[h[1]]}else m=[h[0],h[1]];else if(h.length==
2&&h[1]==j)if(undefined!=$("#gis_location_L0 option[value="+h[0]+"]")[0])$("#gis_location_L0").val(h[0]);else m=[h[0]];else{s3_gis_dropdown_select(4,true);$("#gis_location_").val(j);$("#"+s3_gis_location_id).val(j);break}s3_gis_dropdown_select(0,true);if(m[0])if(undefined!=$("#gis_location_L1 option[value="+m[0]+"]")[0])b=m[0];$("#gis_location_L1").val(b);s3_gis_dropdown_select(1,true);if(m[0])if(undefined!=$("#gis_location_L2 option[value="+m[0]+"]")[0])c=m[0];else if(m[1])if(undefined!=$("#gis_location_L2 option[value="+
m[1]+"]")[0])c=m[1];$("#gis_location_L2").val(c);s3_gis_dropdown_select(2,true);if(m[0])if(undefined!=$("#gis_location_L3 option[value="+m[0]+"]")[0])e=m[0];else if(m[1])if(undefined!=$("#gis_location_L3 option[value="+m[1]+"]")[0])e=m[1];else if(m[2])if(undefined!=$("#gis_location_L3 option[value="+m[2]+"]")[0])e=m[2];$("#gis_location_L3").val(e);s3_gis_dropdown_select(3,true);if(m[0])if(undefined!=$("#gis_location_L4 option[value="+m[0]+"]")[0])k=m[0];else if(m[1])if(undefined!=$("#gis_location_L4 option[value="+
m[1]+"]")[0])k=m[1];else if(m[2])if(undefined!=$("#gis_location_L4 option[value="+m[2]+"]")[0])k=m[2];else if(m[3])if(undefined!=$("#gis_location_L4 option[value="+m[4]+"]")[0])k=m[3];$("#gis_location_L4").val(k);s3_gis_dropdown_select(4,true);$("#gis_location_").val(j)}$("#"+s3_gis_location_id).val(j);s3_gis_update_details(j);$("#gis_location_details-btn").removeClass("hidden").show()}return false}}).data("autocomplete")._renderItem=function(f,g){return $("<li></li>").data("item.autocomplete",g).append("<a>"+
g.name+"</a>").appendTo(f)};$("#gis_location_autocomplete").blur(function(){if(!$("#gis_location_autocomplete").val()){$("#"+s3_gis_location_id).val("");a.accept=true}if(a.accept)a.val=$("#gis_location_autocomplete").val();else $("#gis_location_autocomplete").val(a.val);a.accept=false});$("form").submit(function(){S3ClearNavigateAwayConfirm();var f=$("#gis_location_name").val(),g=$("#gis_location_lat").val(),h=$("#gis_location_lon").val(),j=$("#gis_location_addr_street").val(),b=$("#gis_location_postcode").val();
s3_gis_save_location(f,g,h,j,b);return true})}});
Ext.onReady(function(){var a,f=Ext.get("gis_location_converter-btn"),g=Ext.get("gis_location_map-btn");if(g)g.on("click",function(){mapWin.show(this)});if(f)f.on("click",function(){a||(a=new Ext.Window({applyTo:"gis-convert-win",layout:"fit",width:400,height:250,closeAction:"hide",plain:true,items:new Ext.TabPanel({applyTo:"gis-convert-tabs",autoTabs:true,activeTab:0,deferredRender:false,border:false}),buttons:[{text:s3_gis_fill_lat,disabled:false,handler:function(){if(s3_gis_convertFillBack(1)){s3_gis_calcDone=
...
(function(a,f){function g(){this.debug=false;this._curInst=null;this._keyEvent=false;this._disabledInputs=[];this._inDialog=this._datepickerShowing=false;this._mainDivId="ui-datepicker-div";this._inlineClass="ui-datepicker-inline";this._appendClass="ui-datepicker-append";this._triggerClass="ui-datepicker-trigger";this._dialogClass="ui-datepicker-dialog";this._disableClass="ui-datepicker-disabled";this._unselectableClass="ui-datepicker-unselectable";this._currentClass="ui-datepicker-current-day";this._dayOverClass=
...
k.settings):this._get(k,c):null;var m=c||{};if(typeof c=="string"){m={};m[c]=e}if(k){this._curInst==k&&this._hideDatepicker();var o=this._getDateDatepicker(b,true),n=this._getMinMaxDate(k,"min"),p=this._getMinMaxDate(k,"max");h(k.settings,m);if(n!==null&&m.dateFormat!==f&&m.minDate===f)k.settings.minDate=this._formatDate(k,n);if(p!==null&&m.dateFormat!==f&&m.maxDate===f)k.settings.maxDate=this._formatDate(k,p);this._attachments(a(b),k);this._autoSize(k);this._setDateDatepicker(b,o);this._updateDatepicker(k)}},
...
"0123456789";break;case "D":case "M":return null;case "'":if(k("'"))c+="'";else e=true;break;default:c+=b.charAt(m)}return c},_get:function(b,c){return b.settings[c]!==f?b.settings[c]:this._defaults[c]},_setDateFromField:function(b,c){if(b.input.val()!=b.lastVal){var e=this._get(b,"dateFormat"),k=b.lastVal=b.input?b.input.val():null,m,o;m=o=this._getDefaultDate(b);var n=this._getFormatConfig(b);try{m=this.parseDate(e,k,n)||o}catch(p){this.log(p);k=c?"":k}b.selectedDay=m.getDate();b.drawMonth=b.selectedMonth=
...
"string")return a.datepicker["_"+b+"Datepicker"].apply(a.datepicker,[this[0]].concat(c));return this.each(function(){typeof b=="string"?a.datepicker["_"+b+"Datepicker"].apply(a.datepicker,[this].concat(c)):a.datepicker._attachDatepicker(this,b)})};a.datepicker=new g;a.datepicker.initialized=false;a.datepicker.uuid=(new Date).getTime();a.datepicker.version="1.8.11";window["DP_jQuery_"+j]=a})(jQuery);
(function(a,f){if(a.cleanData){var g=a.cleanData;a.cleanData=function(j){for(var b=0,c;(c=j[b])!=null;b++)a(c).triggerHandler("remove");g(j)}}else{var h=a.fn.remove;a.fn.remove=function(j,b){return this.each(function(){if(!b)if(!j||a.filter(j,[this]).length)a("*",this).add([this]).each(function(){a(this).triggerHandler("remove")});return h.call(a(this),j,b)})}}a.widget=function(j,b,c){var e=j.split(".")[0],k;j=j.split(".")[1];k=e+"-"+j;if(!c){c=b;b=a.Widget}a.expr[":"][k]=function(m){return!!a.data(m,
...
function popup(a){newwindow=window.open(a,"name","height=400,width=600");window.focus&&newwindow.focus();return false}function collapse(a){jQuery("#"+a).slideToggle()}function fade(a,f){f>0?jQuery("#"+a).hide().fadeIn("slow"):jQuery("#"+a).show().fadeOut("slow")}
function ajax(a,f,g){query="";if(typeof f=="string"){if(d=jQuery(f).serialize())query=d}else{pcs=[];for(i=0;i<f.length;i++)(q=jQuery("#"+f[i]).serialize())&&pcs.push(q);if(pcs.length>0)query=pcs.join("&")}jQuery.ajax({type:"POST",url:a,data:query,success:function(h){if(g)g==":eval"?eval(h):jQuery("#"+g).html(h)}})}String.prototype.reverse=function(){return this.split("").reverse().join("")};
...
"").reverse()})}jQuery(document).ready(function(){jQuery(".flash").hide().slideDown("slow");jQuery(".flash").html()!=""&&jQuery(".flash").slideDown("slow");web2py_ajax_init()});function web2py_trap_form(a,f){jQuery("#"+f+" form").each(function(){var g=jQuery(this);g.hasClass("no_trap")||g.submit(function(){jQuery(".flash").hide().html("");web2py_ajax_page("post",a,g.serialize(),f);return false})})}
function web2py_ajax_page(a,f,g,h){jQuery.ajax({type:a,url:f,data:g,beforeSend:function(j){j.setRequestHeader("web2py-component-location",document.location);j.setRequestHeader("web2py-component-element",h)},complete:function(j){(command=j.getResponseHeader("web2py-component-command"))&&eval(command);(flash=j.getResponseHeader("web2py-component-flash"))&&jQuery(".flash").html(flash).slideDown()},success:function(j){jQuery("#"+h).html(j);web2py_trap_form(f,h);web2py_ajax_init()}})}
...
(function(a){a.fn.isSubmitButton=function(){return jQuery(this).formElementType()==="submit"}})(jQuery);(function(a){a.fn.isResetButton=function(){return jQuery(this).formElementType()==="reset"}})(jQuery);(function(a){a.fn.isSelectBox=function(){var f=jQuery(this).formElementType(),g=this.attr("size");return f!=="select-one"?false:typeof g==="undefined"?false:parseInt(g)>1}})(jQuery);(function(a){a.fn.isMultiSelectBox=function(){return jQuery(this).formElementType()==="select-multiple"}})(jQuery);
(function(a){a.fn.isDropDownList=function(){var f=jQuery(this).formElementType(),g=this.attr("size");return f!=="select-one"?false:typeof g==="undefined"?true:parseInt(g)<=1}})(jQuery);(function(a){a.fn.isChecked=function(){return jQuery(this).is(":checked")}})(jQuery);
(function(a){a.fn.isSelected=function(){var f=jQuery(this);if(arguments.length===0)return false;var g=false,h=arguments[0],j=arguments;if(f.isRadioBox()){f=jQuery("input[type='radio'][name='"+f.attr("name")+"'][checked]");if(f.length===1)return h===f.val()}else if(f.isSelectBox()||f.isDropDownList()){f=jQuery("#"+f.attr("id")+" option:selected");if(f.length===1)return h===f.val()}else if(f.isMultiSelectBox()){f=jQuery("#"+f.attr("id")+" option:selected");jQuery.each(f,function(){var b=jQuery(this);
g=false;for(var c=0;c<j.length;c++)if(j[c]===b.val()){g=true;break}})}return g}})(jQuery);
(function(a){a.fn.selectedItem=function(f){var g=jQuery(this);if(typeof f==="undefined"||isNaN(f))f=0;if(g.isRadioBox()){var h=jQuery("input[type='radio'][name='"+g.attr("name")+"'][checked]");if(f>h.length-1)f=h.length-1;else if(f<0)f=0;if(h.length>0)return jQuery(h[f])}else if(g.isSelectBox()||g.isMultiSelectBox()||g.isDropDownList()){h=jQuery("#"+g.attr("id")+" option:selected");if(f>h.length-1)f=h.length-1;else if(f<0)f=0;if(h.length>0)return jQuery(h[f])}return g}})(jQuery);
...
(function(a){a.fn.selectedValues=function(){var f=[],g=jQuery(this);if(g.isMultiSelectBox()){g=jQuery("#"+g.attr("id")+" option:selected");jQuery.each(g,function(){f.push(jQuery(this).val())})}else f.push(g.selectedValue());return f}})(jQuery);
(function(a){a.fn.itemExists=function(f){var g=jQuery(this),h=false;if(g.isRadioBox())h=jQuery("input[type='radio'][name='"+g.attr("name")+"'][value='"+f+"']").length===1;else if(g.isSelectBox()||g.isMultiSelectBox()||g.isDropDownList()){g=jQuery("#"+g.attr("id")+" option");jQuery.each(g,function(){if(jQuery(this).val()===f){h=true;return false}})}else return jQuery(this).val()===f;return h}})(jQuery);
...
c+"/closebox."+k+'" alt="Close" style="border:none; margin:0; padding:0;" />                   </a>                 </div>');$("html").click(function(n){$(n.target).parents("#zoom:visible").length==0&&g()});$(document).keyup(function(n){n.keyCode==27&&$("#zoom:visible").length>0&&g()});$("#zoom_close").click(g)}$("#zoom");$("#zoom_table");var m=$("#zoom_close"),o=$("#zoom_content");$("td.ml,td.mm,td.mr");this.each(function(){$($(this).attr("href")).hide();$(this).click(f)});return this};
(function(a){a.ui=a.ui||{};var f=/left|center|right/,g=/top|center|bottom/,h=a.fn.position,j=a.fn.offset;a.fn.position=function(b){if(!b||!b.of)return h.apply(this,arguments);b=a.extend({},b);var c=a(b.of),e=c[0],k=(b.collision||"flip").split(" "),m=b.offset?b.offset.split(" "):[0,0],o,n,p;if(e.nodeType===9){o=c.width();n=c.height();p={top:0,left:0}}else if(e.setTimeout){o=c.width();n=c.height();p={top:c.scrollTop(),left:c.scrollLeft()}}else if(e.preventDefault){b.at="left top";o=n=0;p={top:b.of.pageY,
left:b.of.pageX}}else{o=c.outerWidth();n=c.outerHeight();p=c.offset()}a.each(["my","at"],function(){var t=(b[this]||"").split(" ");if(t.length===1)t=f.test(t[0])?t.concat(["center"]):g.test(t[0])?["center"].concat(t):["center","center"];t[0]=f.test(t[0])?t[0]:"center";t[1]=g.test(t[1])?t[1]:"center";b[this]=t});if(k.length===1)k[1]=k[0];m[0]=parseInt(m[0],10)||0;if(m.length===1)m[1]=m[0];m[1]=parseInt(m[1],10)||0;if(b.at[0]==="right")p.left+=o;else if(b.at[0]==="center")p.left+=o/2;if(b.at[1]==="bottom")p.top+=
...
(function(a){var f=0;a.widget("ui.autocomplete",{options:{appendTo:"body",autoFocus:false,delay:300,minLength:1,position:{my:"left top",at:"left bottom",collision:"none"},source:null},pending:0,_create:function(){var g=this,h=this.element[0].ownerDocument,j;this.element.addClass("ui-autocomplete-input").attr("autocomplete","off").attr({role:"textbox","aria-autocomplete":"list","aria-haspopup":"true"}).bind("keydown.autocomplete",function(b){if(!(g.options.disabled||g.element.attr("readonly"))){j=
false;var c=a.ui.keyCode;switch(b.keyCode){case c.PAGE_UP:g._move("previousPage",b);break;case c.PAGE_DOWN:g._move("nextPage",b);break;case c.UP:g._move("previous",b);b.preventDefault();break;case c.DOWN:g._move("next",b);b.preventDefault();break;case c.ENTER:case c.NUMPAD_ENTER:if(g.menu.active){j=true;b.preventDefault()}case c.TAB:if(!g.menu.active)break;g.menu.select(b);break;case c.ESCAPE:g.element.val(g.term);g.close(b);break;default:clearTimeout(g.searching);g.searching=setTimeout(function(){if(g.term!=
g.element.val()){g.selectedItem=null;g.search(null,b)}},g.options.delay)}}}).bind("keypress.autocomplete",function(b){if(j){j=false;b.preventDefault()}}).bind("focus.autocomplete",function(){if(!g.options.disabled){g.selectedItem=null;g.previous=g.element.val()}}).bind("blur.autocomplete",function(b){if(!g.options.disabled){clearTimeout(g.searching);g.closing=setTimeout(function(){g.close(b);g._change(b)},150)}});this._initSource();this.response=function(){return g._response.apply(g,arguments)};this.menu=
a("<ul></ul>").addClass("ui-autocomplete").appendTo(a(this.options.appendTo||"body",h)[0]).mousedown(function(b){var c=g.menu.element[0];a(b.target).closest(".ui-menu-item").length||setTimeout(function(){a(document).one("mousedown",function(e){e.target!==g.element[0]&&e.target!==c&&!a.ui.contains(c,e.target)&&g.close()})},1);setTimeout(function(){clearTimeout(g.closing)},13)}).menu({focus:function(b,c){var e=c.item.data("item.autocomplete");false!==g._trigger("focus",b,{item:e})&&/^key/.test(b.originalEvent.type)&&
g.element.val(e.value)},selected:function(b,c){var e=c.item.data("item.autocomplete"),k=g.previous;if(g.element[0]!==h.activeElement){g.element.focus();g.previous=k;setTimeout(function(){g.previous=k;g.selectedItem=e},1)}false!==g._trigger("select",b,{item:e})&&g.element.val(e.value);g.term=g.element.val();g.close(b);g.selectedItem=e},blur:function(){g.menu.element.is(":visible")&&g.element.val()!==g.term&&g.element.val(g.term)}}).zIndex(this.element.zIndex()+1).css({top:0,left:0}).hide().data("menu");
a.fn.bgiframe&&this.menu.element.bgiframe()},destroy:function(){this.element.removeClass("ui-autocomplete-input").removeAttr("autocomplete").removeAttr("role").removeAttr("aria-autocomplete").removeAttr("aria-haspopup");this.menu.element.remove();a.Widget.prototype.destroy.call(this)},_setOption:function(g,h){a.Widget.prototype._setOption.apply(this,arguments);g==="source"&&this._initSource();if(g==="appendTo")this.menu.element.appendTo(a(h||"body",this.element[0].ownerDocument)[0]);g==="disabled"&&
h&&this.xhr&&this.xhr.abort()},_initSource:function(){var g=this,h,j;if(a.isArray(this.options.source)){h=this.options.source;this.source=function(b,c){c(a.ui.autocomplete.filter(h,b.term))}}else if(typeof this.options.source==="string"){j=this.options.source;this.source=function(b,c){g.xhr&&g.xhr.abort();g.xhr=a.ajax({url:j,data:b,dataType:"json",autocompleteRequest:++f,success:function(e){this.autocompleteRequest===f&&c(e)},error:function(){this.autocompleteRequest===f&&c([])}})}}else this.source=
this.options.source},search:function(g,h){g=g!=null?g:this.element.val();this.term=this.element.val();if(g.length<this.options.minLength)return this.close(h);clearTimeout(this.closing);if(this._trigger("search",h)!==false)return this._search(g)},_search:function(g){this.pending++;this.element.addClass("ui-autocomplete-loading");this.source({term:g},this.response)},_response:function(g){if(!this.options.disabled&&g&&g.length){g=this._normalize(g);this._suggest(g);this._trigger("open")}else this.close();
this.pending--;this.pending||this.element.removeClass("ui-autocomplete-loading")},close:function(g){clearTimeout(this.closing);if(this.menu.element.is(":visible")){this.menu.element.hide();this.menu.deactivate();this._trigger("close",g)}},_change:function(g){this.previous!==this.element.val()&&this._trigger("change",g,{item:this.selectedItem})},_normalize:function(g){if(g.length&&g[0].label&&g[0].value)return g;return a.map(g,function(h){if(typeof h==="string")return{label:h,value:h};return a.extend({label:h.label||
h.value,value:h.value||h.label},h)})},_suggest:function(g){var h=this.menu.element.empty().zIndex(this.element.zIndex()+1);this._renderMenu(h,g);this.menu.deactivate();this.menu.refresh();h.show();this._resizeMenu();h.position(a.extend({of:this.element},this.options.position));this.options.autoFocus&&this.menu.next(new a.Event("mouseover"))},_resizeMenu:function(){var g=this.menu.element;g.outerWidth(Math.max(g.width("").outerWidth(),this.element.outerWidth()))},_renderMenu:function(g,h){var j=this;
a.each(h,function(b,c){j._renderItem(g,c)})},_renderItem:function(g,h){return a("<li></li>").data("item.autocomplete",h).append(a("<a></a>").text(h.label)).appendTo(g)},_move:function(g,h){if(this.menu.element.is(":visible"))if(this.menu.first()&&/^previous/.test(g)||this.menu.last()&&/^next/.test(g)){this.element.val(this.term);this.menu.deactivate()}else this.menu[g](h);else this.search(null,h)},widget:function(){return this.menu.element}});a.extend(a.ui.autocomplete,{escapeRegex:function(g){return g.replace(/[-[\]{}()*+?.,\\^$|#\s]/g,
"\\$&")},filter:function(g,h){var j=RegExp(a.ui.autocomplete.escapeRegex(h),"i");return a.grep(g,function(b){return j.test(b.label||b.value||b)})}})})(jQuery);
(function(a){a.widget("ui.menu",{_create:function(){var f=this;this.element.addClass("ui-menu ui-widget ui-widget-content ui-corner-all").attr({role:"listbox","aria-activedescendant":"ui-active-menuitem"}).click(function(g){if(a(g.target).closest(".ui-menu-item a").length){g.preventDefault();f.select(g)}});this.refresh()},refresh:function(){var f=this;this.element.children("li:not(.ui-menu-item):has(a)").addClass("ui-menu-item").attr("role","menuitem").children("a").addClass("ui-corner-all").attr("tabindex",
-1).mouseenter(function(g){f.activate(g,a(this).parent())}).mouseleave(function(){f.deactivate()})},activate:function(f,g){this.deactivate();if(this.hasScroll()){var h=g.offset().top-this.element.offset().top,j=this.element.attr("scrollTop"),b=this.element.height();if(h<0)this.element.attr("scrollTop",j+h);else h>=b&&this.element.attr("scrollTop",j+h-b+g.height())}this.active=g.eq(0).children("a").addClass("ui-state-hover").attr("id","ui-active-menuitem").end();this._trigger("focus",f,{item:g})},
deactivate:function(){if(this.active){this.active.children("a").removeClass("ui-state-hover").removeAttr("id");this._trigger("blur");this.active=null}},next:function(f){this.move("next",".ui-menu-item:first",f)},previous:function(f){this.move("prev",".ui-menu-item:last",f)},first:function(){return this.active&&!this.active.prevAll(".ui-menu-item").length},last:function(){return this.active&&!this.active.nextAll(".ui-menu-item").length},move:function(f,g,h){if(this.active){f=this.active[f+"All"](".ui-menu-item").eq(0);
f.length?this.activate(h,f):this.activate(h,this.element.children(g))}else this.activate(h,this.element.children(g))},nextPage:function(f){if(this.hasScroll())if(!this.active||this.last())this.activate(f,this.element.children(".ui-menu-item:first"));else{var g=this.active.offset().top,h=this.element.height(),j=this.element.children(".ui-menu-item").filter(function(){var b=a(this).offset().top-g-h+a(this).height();return b<10&&b>-10});j.length||(j=this.element.children(".ui-menu-item:last"));this.activate(f,
j)}else this.activate(f,this.element.children(".ui-menu-item").filter(!this.active||this.last()?":first":":last"))},previousPage:function(f){if(this.hasScroll())if(!this.active||this.first())this.activate(f,this.element.children(".ui-menu-item:last"));else{var g=this.active.offset().top,h=this.element.height();result=this.element.children(".ui-menu-item").filter(function(){var j=a(this).offset().top-g+h-a(this).height();return j<10&&j>-10});result.length||(result=this.element.children(".ui-menu-item:first"));
this.activate(f,result)}else this.activate(f,this.element.children(".ui-menu-item").filter(!this.active||this.first()?":last":":first"))},hasScroll:function(){return this.element.height()<this.element.attr("scrollHeight")},select:function(f){this._trigger("selected",f,{item:this.active})}})})(jQuery);$(document).ready(function(){S3.rtl?$('div.label label[id!="delete_record__label"]').prepend(":"):$('div.label label[id!="delete_record__label"]').append(":")});var S3={};S3.gis={};var popupWin=null;
...
return false});$(".information").hide().slideDown("slow");$(".information").click(function(){$(this).fadeOut("slow");return false});$(".confirmation").hide().slideDown("slow");$(".confirmation").click(function(){$(this).fadeOut("slow");return false});$(".hoverable").hover(function(){$(this).addClass("hovered")},function(){$(this).removeClass("hovered")});$("#modulenav li").hover(function(){var g=$(this).width(),h=$("ul",this).width();h!=null&&h<g&&$("ul",this).css({width:g.toString()+"px"});$("ul",
this).css("display","block")},function(){$("ul",this).css("display","none")});$("#subnav li").hover(function(){var g=$(this).width()-2;$("ul",this).css({display:"block",width:g.toString()+"px"})},function(){$("ul",this).css("display","none")});$("a.colorbox").attr("href",function(g,h){var j="";try{j=$(this).parents("tr").attr("id").replace(/__row/,"")}catch(b){if(j=="")return h}var c=h;if(h.indexOf("&caller=")==-1)c=h+"&caller="+j;return c});$(".colorbox").click(function(){$.fn.colorbox({iframe:true,
width:"99%",height:"99%",href:this.href,title:this.title});return false});$(".tooltip").cluetip({activation:"hover",sticky:false,splitTitle:"|"});var f='<img src="'+S3.Ap.concat("/static/img/cross2.png")+'" alt="close" />';$(".stickytip").cluetip({activation:"hover",sticky:true,closePosition:"title",closeText:f,splitTitle:"|"});$(".ajaxtip").cluetip({activation:"click",sticky:true,closePosition:"title",closeText:f,width:380});now=new Date;$("form").append("<input type='hidden' value="+now.getTimezoneOffset()+
" name='_utc_offset'/>")});function s3_tb_remove(){$.fn.colorbox.close()}
(function(a){a.ajaxS3=function(f){var g=a.extend({},a.ajaxS3Settings,f);g.tryCount=0;if(f.message)s3_showStatus(S3.i18n.ajax_get+" "+(f.message?f.message:S3.i18n.ajax_fmd)+"...",this.ajaxS3Settings.msgTimeout);g.success=function(h,j){s3_hideStatus();f.success&&f.success(h,j)};g.error=function(h,j){if(j=="timeout"){this.tryCount++;if(this.tryCount<=this.retryLimit){s3_showStatus(S3.i18n.ajax_get+" "+(f.message?f.message:S3.i18n.ajax_fmd)+"... "+S3.i18n.ajax_rtr+" "+this.tryCount,a.ajaxS3Settings.msgTimeout);
a.ajax(this)}else s3_showStatus(S3.i18n.ajax_wht+" "+(this.retryLimit+1)+" "+S3.i18n.ajax_gvn,a.ajaxS3Settings.msgTimeout,false,true)}else h.status==500?s3_showStatus(S3.i18n.ajax_500,a.ajaxS3Settings.msgTimeout,false,true):s3_showStatus(S3.i18n.ajax_dwn,a.ajaxS3Settings.msgTimeout,false,true)};a.ajax(g)};a.postS3=function(f,g,h,j){return a.ajaxS3({type:"POST",url:f,data:g,success:h,dataType:j})};a.getS3=function(f,g,h,j,b,c){if(a.isFunction(g)){c=b;b=j;j=h;h=g;g=null}if(c)var e=false;return a.ajaxS3({type:"GET",
url:f,async:e,data:g,success:h,dataType:j,message:b})};a.getJSONS3=function(f,g,h,j,b){if(a.isFunction(g)){b=j;j=h;h=g;g=null}b||(b=false);return a.getS3(f,g,h,"json",j,b)};a.ajaxS3Settings={timeout:1E4,msgTimeout:2E3,retryLimit:10,dataType:"json",async:true,type:"GET"};a.ajaxS3Setup=function(f){a.extend(a.ajaxS3Settings,f)}})($);
function S3StatusBar(a,f){var g=this,h=null;this.elementId="_showstatus";this.prependMultiline=true;this.showCloseButton=false;this.afterTimeoutText=null;this.cssClass="statusbar";this.highlightClass="statusbarhighlight";this.errorClass="statusbarerror";this.closeButtonClass="statusbarclose";this.additive=false;$.extend(this,f);if(a)h=$(a);h||(h=$("<div id='_statusbar' class='"+g.cssClass+"'><div class='"+g.closeButtonClass+"'>"+(g.showCloseButton?" X </div></div>":"")).appendTo(document.body).show());
$("."+g.cssClass).click(function(){$(h).hide()});this.show=function(j,b,c,e){if(c||c==undefined&&g.additive){j="<div style='margin-bottom: 2px;' >"+j+"</div>";g.prependMultiline?h.prepend(j):h.append(j)}else if(g.showCloseButton){c=h.find("div.statusbarclose");h.text(j).prepend(c)}else h.text(j);h.show();if(b){e?h.addClass(g.errorClass):h.addClass(g.highlightClass);setTimeout(function(){h.removeClass(g.highlightClass);g.afterTimeoutText&&g.show(g.afterTimeoutText)},b)}};this.release=function(){if(_statusbar){$("#_statusbar").remove();
_statusbar=undefined}}}var _statusbar=null;function s3_showStatus(a,f,g,h){_statusbar||(_statusbar=new S3StatusBar);_statusbar.show(a,f,g,h)}function s3_hideStatus(){_statusbar&&_statusbar.release()}function S3SetNavigateAwayConfirm(){window.onbeforeunload=function(){return S3.i18n.unsaved_changes}}function S3ClearNavigateAwayConfirm(){window.onbeforeunload=function(){}}
function S3EnableNavigateAwayConfirm(){$(document).ready(function(){$("[class=error]").length>0&&S3SetNavigateAwayConfirm();$(":input:not(input[id=gis_location_advanced_checkbox])").keypress(S3SetNavigateAwayConfirm);$(":input:not(input[id=gis_location_advanced_checkbox])").change(S3SetNavigateAwayConfirm);$("form").submit(S3ClearNavigateAwayConfirm)})}function S3ConfirmClick(a,f){$(a).click(function(g){if(confirm(f))return true;else{g.preventDefault();return false}})}
function s3_viewMap(a){a=S3.Ap.concat("/gis/display_feature/")+a;var f=$("#map").html();a="<iframe width='640' height='480' src='"+a+"'></iframe>";var g=$('<a href="#">'+S3.i18n.close_map+"</a>");g.bind("click",function(h){$("#map").html(f);h.preventDefault()});$("#map").html(a);$("#map").append($("<div style='margin-bottom: 10px' />").append(g))}
function s3_viewMapMulti(a,f,g,h){a=S3.Ap.concat("/gis/display_feature//?module=")+a+"&resource="+f+"&instance="+g+"&jresource="+h;var j=$("#map").html();a='<iframe width="640" height="480" src="'+a+'"></iframe>';f=$('<a href="#">'+S3.i18n.close_map+"</a>");f.bind("click",function(b){$("#map").html(j);b.preventDefault()});$("#map").html(a);$("#map").append($("<div style='margin-bottom: 10px' />").append(f))};
