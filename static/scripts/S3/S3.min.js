...
$(document).ready(function(){try{$('.zoom').fancyZoom({scaleImg:true,closeOnClick:true,directory:S3Ap.concat("/static/media")});}catch(e){};$('input.date').datepicker({changeMonth:true,changeYear:true,showOn:'both',buttonImage:S3Ap.concat('/static/img/jquery-ui/calendar.gif'),buttonImageOnly:true,dateFormat:'yy-mm-dd',isRTL:S3.rtl});$('.error').hide().slideDown('slow')
...
return url_out;});$('.colorbox').click(function(){$.fn.colorbox({iframe:true,width:'99%',height:'99%',href:this.href,title:this.title});return false;});$('.tooltip').cluetip({activation:'hover',sticky:false,splitTitle:'|'});var tipCloseText='<img src="'+S3Ap.concat('/static/img/cross2.png')+'" alt="close" />';$('.stickytip').cluetip({activation:'hover',sticky:true,closePosition:'title',closeText:tipCloseText,splitTitle:'|'});$('.ajaxtip').cluetip({activation:'click',sticky:true,closePosition:'title',closeText:tipCloseText,width:380});now=new Date();$('form').append("<input type='hidden' value="+now.getTimezoneOffset()+" name='_utc_offset'/>");});function s3_tb_remove(){$.fn.colorbox.close();}
(function($){jQuery.ajaxS3=function(s){var options=jQuery.extend({},jQuery.ajaxS3Settings,s);options.tryCount=0;if(s.message){s3_showStatus(_ajaxS3_get_+' '+(s.message?s.message:_ajaxS3_fmd_)+'...',this.ajaxS3Settings.msgTimeout);}
options.success=function(data,status){s3_hideStatus();if(s.success)
s.success(data,status);}
options.error=function(xhr,textStatus,errorThrown){if(textStatus=='timeout'){this.tryCount++;if(this.tryCount<=this.retryLimit){s3_showStatus(_ajaxS3_get_+' '+(s.message?s.message:_ajaxS3_fmd_)+'... '+_ajaxS3_rtr_+' '+this.tryCount,$.ajaxS3Settings.msgTimeout);$.ajax(this);return;}
s3_showStatus(_ajaxS3_wht_+' '+(this.retryLimit+1)+' '+_ajaxS3_gvn_,$.ajaxS3Settings.msgTimeout,false,true);return;}
if(xhr.status==500){s3_showStatus(_ajaxS3_500_,$.ajaxS3Settings.msgTimeout,false,true);}else{s3_showStatus(_ajaxS3_dwn_,$.ajaxS3Settings.msgTimeout,false,true);}};jQuery.ajax(options);};jQuery.postS3=function(url,data,callback,type){return jQuery.ajaxS3({type:"POST",url:url,data:data,success:callback,dataType:type});};jQuery.getS3=function(url,data,callback,type,message,sync){if(jQuery.isFunction(data)){sync=message;message=type;type=callback;callback=data;data=null;}
