...
s3_gis_ac_data.accept=false;});$('form').submit(function(){S3ClearNavigateAwayConfirm();var name=$('#gis_location_name').val();var lat=$('#gis_location_lat').val();var lon=$('#gis_location_lon').val();var addr_street=$('#gis_location_addr_street').val();var addr_postcode=$('#gis_location_postcode').val();if(''==name){if((''==lat||''==lon)&&(''==addr_street)&&(''==addr_postcode)){return true;}else{name=$('#gis_location_ :selected').text();if(s3_gis_loading_locations.replace('<option value="">','').replace('</option>','')==name){$('#gis_location_name').after('<div id="type__error" class="error" style="display: block;">Name field is required!</div>');$('#gis_location_name').focus();if(s3_navigate_away_confirm){S3SetNavigateAwayConfirm()}
