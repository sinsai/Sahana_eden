...
function s3_gis_dropdown_select(a,f){s3_gis_pullNext=false;if(a==s3_gis_maxlevel){$("#gis_location__label__row").removeClass("hidden").show();$("#gis_location___row").removeClass("hidden").show();$("#gis_location_").removeClass("hidden").show()}else{$("#gis_location_L"+(a+1)+"_label__row").removeClass("hidden").show();$("#gis_location_L"+(a+1)+"__row").removeClass("hidden").show();s3_gis_hide_dropdown(a+2)}var g=$("#gis_location_name").val(),h=$("#gis_location_lat").val(),j=$("#gis_location_lon").val(),
b=$("#gis_location_addr_street").val(),c=$("#gis_location_postcode").val();g=g||h||j||b||c?true:false;if((h=$("#gis_location_L"+a).val())||f){var e;$.getJSONS3(a==s3_gis_maxlevel?s3_gis_url+"/search.json?filter=%3D&field=level&value=nullnone&parent="+h:s3_gis_url+"/search.json?filter=%3D&field=level&value=L"+(a+1)+"&parent="+h,function(k){if(k.length==0)e=s3_gis_empty_set;else if(!f&&a!=s3_gis_maxlevel&&k.length==1){e=s3_gis_select_location;e+='<option selected="" value="'+k[0].id+'">'+k[0].name+
"</option>";s3_gis_pullNext=true}else{e=s3_gis_select_location;for(var m=0;m<k.length;m++)e+='<option value="'+k[m].id+'">'+k[m].name+"</option>"}a==s3_gis_maxlevel?$("#gis_location_").html(e):$("#gis_location_L"+(a+1)).html(e)},false,true);if(s3_gis_pullNext){h=$("#gis_location_L"+(a+1)).val();g||$("#"+s3_gis_location_id).val(h);s3_gis_dropdown_select(a+1)}else g||$("#"+s3_gis_location_id).val(h)}else if(a!=s3_gis_maxlevel){s3_gis_hide_dropdown(a+1);0==a&&!g&&$("#"+s3_gis_location_id).val("")}}
...
navigator.geolocation&&$("#gis_location_geolocate-btn").removeClass("hidden").show();if(""!=S3.gis.uuid){$("body").data("uuid",S3.gis.uuid);S3.gis.uuid=""}var a=$("#gis_location_name").val(),f=$("#gis_location_addr_street").val(),g=$("#gis_location_postcode").val(),h=$("#gis_location_lat").val(),j=$("#gis_location_lat").val();$("body").data("name",a);$("body").data("addr_street",f);$("body").data("postcode",g);$("body").data("lat",h);$("body").data("lon",j);$("#gis_location_name").val("");$("#gis_location_addr_street").val("");
...
$("#gis_location__label__row").hide();$("#gis_location___row").hide();$("#gis_location_add-btn").hide();s3_gis_show_details()});$("#gis_location_search-btn").click(function(f){$(this).hide();s3_gis_search();f.preventDefault()});$("#gis_location_add-btn").click(function(f){$(this).hide();s3_gis_add_new();f.preventDefault()});$("#gis_location_cancel-btn").click(function(f){$(this).hide();s3_gis_cancel();f.preventDefault()});$("#gis_location_advanced_checkbox").change(function(f){if($("#gis_location_advanced_checkbox").is(":checked")){$("#gis_location_lat_label").removeClass("hidden").show();
$("#gis_location_lat_row").removeClass("hidden").show();$("#gis_location_lon_label").removeClass("hidden").show();$("#gis_location_lon_row").removeClass("hidden").show()}else{$("#gis_location_lat_label").hide();$("#gis_location_lat_row").hide();$("#gis_location_lon_label").hide();$("#gis_location_lon_row").hide()}f.preventDefault()});$("#gis_location_details-btn").click(function(f){$("#gis_location_add-btn").hide();$("#gis_location___row").hide();$("#gis_location__label__row").hide();$("#gis_location_details-btn").hide();
