...
function s3_gis_dropdown_select(a,e){s3_gis_pullNext=false;$("#gis_location_lat").val("");$("#gis_location_lon").val("");$("#gis_location_addr_street").val("");$("#gis_location_postcode").val("");var h=$("#gis_location_L"+a).val();if(h||e){$.getJSONS3(a==s3_gis_maxlevel?s3_gis_url+"/search.json?filter=%3D&field=level&value=nullnone&parent="+h:s3_gis_url+"/search.json?filter=%3D&field=level&value=L"+(a+1)+"&parent="+h,function(g){var j;if(g.length==0)j=s3_gis_empty_set;else if(g.length==1){j='<option value="'+
g[0].id+'">'+g[0].name+"</option>";s3_gis_pullNext=true}else{j=s3_gis_select_location;for(var b=0;b<g.length;b++)j+='<option value="'+g[b].id+'">'+g[b].name+"</option>"}a==s3_gis_maxlevel?$("#gis_location_").html(j):$("#gis_location_L"+(a+1)).html(j)},false,true);if(a==s3_gis_maxlevel){if(options!=s3_gis_empty_set){$("#gis_location_").removeClass("hidden").show();$("#gis_location_label_").removeClass("hidden").show();$("#gis_location_details-btn").removeClass("hidden").show()}}else{$("#gis_location_L"+
...
function popup(a){newwindow=window.open(a,"name","height=400,width=600");window.focus&&newwindow.focus();return false}function collapse(a){jQuery("#"+a).slideToggle()}function fade(a,e){e>0?jQuery("#"+a).hide().fadeIn("slow"):jQuery("#"+a).show().fadeOut("slow")}
