...
if(options&&options.insertionElement){insertionElement=options.insertionElement;}};})(jQuery);;(function($){$.fn.ajaxSubmit=function(options){if(!this.length){log('ajaxSubmit: skipping submit process - no element selected');return this;}
...
field.focus();};})(jQuery);(function($){$.fn.dataTableSettings=[];$.fn.dataTableExt={};var _oExt=$.fn.dataTableExt;_oExt.sVersion="1.5.0";_oExt.iApiIndex=0;_oExt.oApi={};_oExt.afnFiltering=[];_oExt.aoFeatures=[];_oExt.ofnSearch={};_oExt.oPagination={"two_button":{"fnInit":function(oSettings,fnCallbackDraw)
{var nPaging=oSettings.anFeatures.p;oSettings.nPrevious=document.createElement('div');oSettings.nNext=document.createElement('div');if(oSettings.sTableId!=='')
{nPaging.setAttribute('id',oSettings.sTableId+'_paginate');oSettings.nPrevious.setAttribute('id',oSettings.sTableId+'_previous');oSettings.nNext.setAttribute('id',oSettings.sTableId+'_next');}
...
fnCallbackDraw(oSettings);});$(oSettings.nPrevious).bind('selectstart',function(){return false;});$(oSettings.nNext).bind('selectstart',function(){return false;});},"fnUpdate":function(oSettings,fnCallbackDraw)
...
{if(!oSettings.anFeatures.p)
{return;}
var iPageCount=jQuery.fn.dataTableExt.oPagination.iFullNumbersShowPages;var iPageCountHalf=Math.floor(iPageCount/2);var iPages=Math.ceil((oSettings.fnRecordsDisplay()-1)/oSettings._iDisplayLength)+1;var iCurrentPage=Math.ceil(oSettings._iDisplayStart/oSettings._iDisplayLength)+1;var sList="";var iStartButton;var iEndButton;if(iPages<iPageCount)
...
{_fnReDraw(_fnSettingsFromNode(this[_oExt.iApiIndex]));};this.fnFilter=function(sInput,iColumn,bEscapeRegex)
...
{return _fnSettingsFromNode(this[_oExt.iApiIndex]);};this.fnSort=function(aaSort)
{var oSettings=_fnSettingsFromNode(this[_oExt.iApiIndex]);oSettings.aaSorting=aaSort;_fnSort(oSettings);};this.fnAddData=function(mData,bRedraw)
{var aiReturn=[];var iTest;if(typeof bRedraw=='undefined')
{bRedraw=true;}
var oSettings=_fnSettingsFromNode(this[_oExt.iApiIndex]);if(typeof mData[0]=="object")
...
{_fnReDraw(oSettings);}
return aiReturn;};this.fnDeleteRow=function(iAODataIndex,fnCallBack,bNullRow)
{var oSettings=_fnSettingsFromNode(this[_oExt.iApiIndex]);var i;for(i=0;i<oSettings.aiDisplayMaster.length;i++)
...
{var oSettings=_fnSettingsFromNode(this[_oExt.iApiIndex]);var iVisibleColumn;var sDisplay;if(typeof bRedraw=='undefined')
{bRedraw=true;}
...
{var oSettings=_fnSettingsFromNode(this[_oExt.iApiIndex]);var i,iLen;var iColumns=oSettings.aoColumns.length;var nTd;if(oSettings.aoColumns[iCol].bVisible==bShow)
{return;}
...
_fnSaveState(oSettings);};function _fnExternApiFunc(sFunc)
...
{alert("Warning - added data does not match known column length");return-1;}
...
{return;}
var fnInnerSorting=function(){if(e.shiftKey)
...
$('thead th',oSettings.nTable).mousedown(function(e){if(e.shiftKey)
...
{var i;var anRows=[];var iRowCount=0;var bRowError=false;var iStrips=oSettings.asStripClasses.length;var iOpenRows=oSettings.aoOpenRows.length;if(oSettings.oFeatures.bServerSide&&!_fnAjaxUpdate(oSettings))
{return;}
...
{var nRow=oSettings.aoData[oSettings.aiDisplay[j]].nTr;if(iStrips!==0)
...
{_oExt.oPagination[oSettings.sPaginationType].fnUpdate(oSettings,function(oSettings){_fnCalculateEnd(oSettings);_fnDraw(oSettings);});}
...
{oSettings.anFeatures.i.innerHTML=oSettings.oLanguage.sInfo.replace('_START_',oSettings._iDisplayStart+1).replace('_END_',oSettings.fnDisplayEnd()).replace('_TOTAL_',oSettings.fnRecordsDisplay())+
...
_fnClearTable(oSettings);oSettings._iRecordsTotal=json.iTotalRecords;oSettings._iRecordsDisplay=json.iTotalDisplayRecords;var sOrdering=_fnColumnOrdering(oSettings);var bReOrder=(json.sColumns!='undefined'&&sOrdering!==""&&json.sColumns!=sOrdering);if(bReOrder)
...
var nInsertNode=nWrapper;var sDom=oSettings.sDomPositioning.split('');var nTmp;for(var i=0;i<sDom.length;i++)
...
{nTmp=_fnFeatureHtmlLength(oSettings);oSettings.anFeatures[cOption]=nTmp;nInsertNode.appendChild(nTmp);}
else if(cOption=='f'&&oSettings.oFeatures.bFilter)
{nTmp=_fnFeatureHtmlFilter(oSettings);oSettings.anFeatures[cOption]=nTmp;nInsertNode.appendChild(nTmp);}
else if(cOption=='r'&&oSettings.oFeatures.bProcessing)
{nTmp=_fnFeatureHtmlProcessing(oSettings);oSettings.anFeatures[cOption]=nTmp;nInsertNode.appendChild(nTmp);}
else if(cOption=='t')
{oSettings.anFeatures[cOption]=oSettings.nTable;nInsertNode.appendChild(oSettings.nTable);}
else if(cOption=='i'&&oSettings.oFeatures.bInfo)
{nTmp=_fnFeatureHtmlInfo(oSettings);oSettings.anFeatures[cOption]=nTmp;nInsertNode.appendChild(nTmp);}
else if(cOption=='p'&&oSettings.oFeatures.bPaginate)
{nTmp=_fnFeatureHtmlPaginate(oSettings);oSettings.anFeatures[cOption]=nTmp;nInsertNode.appendChild(nTmp);}
else if(_oExt.aoFeatures.length!==0)
{var aoFeatures=_oExt.aoFeatures;for(var k=0,kLen=aoFeatures.length;k<kLen;k++)
{if(cOption==aoFeatures[k].cFeature)
{nTmp=aoFeatures[k].fnInit(oSettings);oSettings.anFeatures[cOption]=nTmp;nInsertNode.appendChild(nTmp);break;}}}}
...
nFilter.className="dataTables_filter";var sSpace=oSettings.oLanguage.sSearch===""?"":" ";nFilter.innerHTML=oSettings.oLanguage.sSearch+sSpace+'<input type="text" />';var jqFilter=$("input",nFilter);jqFilter.val(oSettings.oPreviousSearch.sSearch.replace('"','&quot;'));jqFilter.keyup(function(e){_fnFilterComplete(oSettings,{"sSearch":this.value,"bEscapeRegex":oSettings.oPreviousSearch.bEscapeRegex});});return nFilter;}
function _fnFeatureHtmlInfo(oSettings)
{var nInfo=document.createElement('div');if(oSettings.sTableId!=='')
{nInfo.setAttribute('id',oSettings.sTableId+'_info');}
nInfo.className="dataTables_info";return nInfo;}
function _fnFeatureHtmlPaginate(oSettings)
{var nPaginate=document.createElement('div');nPaginate.className="dataTables_paginate paging_"+oSettings.sPaginationType;oSettings.anFeatures.p=nPaginate;_oExt.oPagination[oSettings.sPaginationType].fnInit(oSettings,function(oSettings){_fnCalculateEnd(oSettings);_fnDraw(oSettings);});return nPaginate;}
...
{var nProcessing=document.createElement('div');if(oSettings.sTableId!=='')
{nProcessing.setAttribute('id',oSettings.sTableId+'_processing');}
nProcessing.innerHTML=oSettings.oLanguage.sProcessing;nProcessing.className="dataTables_processing";oSettings.nTable.parentNode.insertBefore(nProcessing,oSettings.nTable);return nProcessing;}
...
{var aaSort=[];var oSort=_oExt.oSort;var aoData=oSettings.aoData;var iDataSort;var iDataType;var i;if(oSettings.aaSorting.length!==0||oSettings.aaSortingFixed!==null)
...
{oSettings.oFeatures.bStateSave=oInit.bStateSave;_fnLoadState(oSettings,oInit);}
...
$('.confirmation').click(function(){$(this).fadeOut('slow');return false;});$('a.popup').click(function(){var url=$(this).attr('href');var caller=$(this).parents('tr').attr('id').replace(/__row/,'');openPopup(url.replace(/format=plain/,'format=popup')+'&caller='+caller);return false;});});