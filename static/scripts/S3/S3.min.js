...
if(DIV.hasClass("collapsed")){if(DIV.hasClass("fulfil")){ShipmentType="recv"}else if(DIV.hasClass("transit")){ShipmentType="send"}else if(DIV.hasClass("commit")){ShipmentType="commit"}
...
function s3_gis_dropdown_select(level,force){var pullNext=false;$('#gis_location_lat').val('');$('#gis_location_lon').val('');$('#gis_location_addr_street').val('');$('#gis_location_postcode').val('');var new_id=$('#gis_location_L'+level).val();if(new_id||force){if(level==s3_gis_maxlevel){var this_url=s3_gis_url+'/search.json?filter=%3D&field=level&value=nullnone&parent='+new_id;}else{var this_url=s3_gis_url+'/search.json?filter=%3D&field=level&value=L'+(level+1)+'&parent='+new_id;}
