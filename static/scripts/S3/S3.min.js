...
var pullNext;function s3_gis_dropdown_select(level,force){pullNext=false;$('#gis_location_lat').val('');$('#gis_location_lon').val('');$('#gis_location_addr_street').val('');$('#gis_location_postcode').val('');var new_id=$('#gis_location_L'+level).val();if(new_id||force){if(level==s3_gis_maxlevel){var this_url=s3_gis_url+'/search.json?filter=%3D&field=level&value=nullnone&parent='+new_id;}else{var this_url=s3_gis_url+'/search.json?filter=%3D&field=level&value=L'+(level+1)+'&parent='+new_id;}
var s3_gis_load_locations=function(data,status){var options;var v='';if(data.length==0){options=s3_gis_empty_set;}else if(data.length==1){options='<option value="'+data[0].id+'">'+data[0].name+'</option>';pullNext=true;}else{options=s3_gis_select_location;for(var i=0;i<data.length;i++){v=data[i].id;options+='<option value="'+data[i].id+'">'+data[i].name+'</option>';}}
if(level==s3_gis_maxlevel){$('#gis_location_').html(options);}else{$('#gis_location_L'+(level+1)).html(options);}}
$.getJSONS3(this_url,s3_gis_load_locations,false,force);if(level==s3_gis_maxlevel){if(options==s3_gis_empty_set){}else{$('#gis_location_').removeClass('hidden').show();$('#gis_location_label_').removeClass('hidden').show();$('#gis_location_details-btn').removeClass('hidden').show();}}else{$('#gis_location_L'+(level+1)).removeClass('hidden').show();$('#gis_location_label_L'+(level+1)).removeClass('hidden').show();$('#gis_location_details-btn').removeClass('hidden').show();}
s3_gis_dropdown_hide(level+2);if(''==$('#gis_location_name').val()){$('#'+s3_gis_location_id).val(new_id);}
if(pullNext){s3_gis_dropdown_select(level+1);}}else{s3_gis_dropdown_hide(level+1);if((0==level)&&(''==$('#gis_location_name').val())){$('#'+s3_gis_location_id).val('');}}}
...
(function($){jQuery.ajaxS3=function(s){var options=jQuery.extend({},jQuery.ajaxS3Settings,s);options.tryCount=0;if(s.message){s3_showStatus(S3.i18n.ajax_get+' '+(s.message?s.message:S3.i18n.ajax_fmd)+'...',this.ajaxS3Settings.msgTimeout);}
...
if(xhr.status==500){s3_showStatus(S3.i18n.ajax_500,$.ajaxS3Settings.msgTimeout,false,true);}else{s3_showStatus(S3.i18n.ajax_dwn,$.ajaxS3Settings.msgTimeout,false,true);}};jQuery.ajax(options);};jQuery.postS3=function(url,data,callback,type){return jQuery.ajaxS3({type:"POST",url:url,data:data,success:callback,dataType:type});};jQuery.getS3=function(url,data,callback,type,message,sync){if(jQuery.isFunction(data)){sync=message;message=type;type=callback;callback=data;data=null;}
if(sync){var async=false;}
return jQuery.ajaxS3({type:'GET',url:url,async:async,data:data,success:callback,dataType:type,message:message});};jQuery.getJSONS3=function(url,data,callback,message,sync){if(jQuery.isFunction(data)){sync=message;message=callback;callback=data;data=null;}
if(!sync){var sync=false;}
return jQuery.getS3(url,data,callback,'json',message,sync);};jQuery.ajaxS3Settings={timeout:10000,msgTimeout:2000,retryLimit:10,dataType:'json',async:true,type:'GET'};jQuery.ajaxS3Setup=function(settings){jQuery.extend(jQuery.ajaxS3Settings,settings);};})(jQuery);function S3StatusBar(sel,options){var _I=this;var _sb=null;this.elementId='_showstatus';this.prependMultiline=true;this.showCloseButton=false;this.afterTimeoutText=null;this.cssClass='statusbar';this.highlightClass='statusbarhighlight';this.errorClass='statusbarerror';this.closeButtonClass='statusbarclose';this.additive=false;$.extend(this,options);if(sel){_sb=$(sel);}
