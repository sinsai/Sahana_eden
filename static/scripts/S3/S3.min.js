...
url+=$('[name = "inv_item_id"]').val();$.getJSON(url,function(data){var InvQuantity=data.inv_inv_item.quantity;var InvPacketQuantity=data.supply_item_packet.quantity;var PacketName=$('[name = "item_packet_id"] option:selected').text();var re=/\(([0-9]*)/;var PacketQuantity=re.exec(PacketName)[1];var Quantity=(InvQuantity*InvPacketQuantity)/PacketQuantity;TotalQuantity='<span id = "TotalQuantity"> / '+Quantity.toFixed(2)+' '+PacketName+' in inv.</span>';$('#inv_quantity_ajax_throbber').remove();$('[name = "quantity"]').after(TotalQuantity);});};$('#inv_send_item_item_packet_id').change(InvItemPackIDChange);function ItemIDChange(){var selField=$(this);var selSubField=$('[name = "item_packet_id"]');$('[id$="item_packet_id__row1"]').show();$('[id$="item_packet_id__row"]').show();selSubField.after('<div id="item_packet_ajax_throbber" class="ajax_throbber style="display:inline;"/>').hide();var url;if(selField.length!=0){url=S3.Ap.concat('/supply/item_packet.json?item_packet.item_id=',selField.val());}else{url=S3.Ap.concat('/inv/inv_item_packets/',selField.val());}
var data;$.ajax({url:url,dataType:'json',context:selField,success:function(data){var options='';var v='';if(data.length==0){options+='<option value="">'+S3.i18n.no_packets+'</options>';}else{for(var i=0;i<data.length;i++){v=data[i].id;options+='<option value="'+data[i].id+'">'+data[i].name+' ('+data[i].quantity+')</option>';}}
selSubField.html(options).val(1).show();href=$('#item_packet_add').attr('href')+"&item_id="+$('[name = "item_id"]').val();$('#item_packet_add').attr('href',href).show();$('#item_packet_ajax_throbber').hide();if(this.attr('name')=='inv_item_id'){InvItemPackIDChange();}}});};var ItemID=$('[name $= "item_id"]').val();if(ItemID==''|ItemID==undefined){$('[id$="item_packet_id__row1"]').hide();$('[id$="item_packet_id__row"]').hide();}else{ItemIDChange();}
