...
{JSONValue="'"+InputSelector.val()+"'";DisplayValue=$(InputID+" option:selected").text();}
else if(AutoTextValue!=undefined)
{DisplayValue=AutoTextValue
JSONValue=$(InputID+"_auto_json").val();$(InputID+"_auto_text").val("");$(InputID+"_auto_json").val("");$(InputID+"_auto_edit").hide();}
...
DisplayValue;S3MultiselectWidgetFileCounter++;}}
else
...
{$("#"+NewFileInputID).replaceWith($(FileInputID).clone());$(FileInputID+":first").attr("id",NewFileInputID);$("#"+NewFileInputID).attr("name",NewFileInputID);$("#"+NewFileInputID).hide();$(FileInputID).val("");}}};function S3MultiselectWidgetDeleteClick(WidgetDelete,Args)
...
$("#"+WidgetID+"_json").val(JSONInputValue);};;jQuery.ui||(function($){$.ui={version:"1.8.1",plugin:{add:function(module,option,set){var proto=$.ui[module].prototype;for(var i in set){proto.plugins[i]=proto.plugins[i]||[];proto.plugins[i].push([option,set[i]]);}},call:function(instance,name,args){var set=instance.plugins[name];if(!set||!instance.element[0].parentNode){return;}
...
return $.datepicker['_'+options+'Datepicker'].apply($.datepicker,[this[0]].concat(otherArgs));return this.each(function(){typeof options=='string'?$.datepicker['_'+options+'Datepicker'].apply($.datepicker,[this].concat(otherArgs)):$.datepicker._attachDatepicker(this,options);});};$.datepicker=new Datepicker();$.datepicker.initialized=false;$.datepicker.uuid=new Date().getTime();$.datepicker.version="1.8.1";window['DP_jQuery_'+dpuuid]=$;})(jQuery);﻿function popup(url){newwindow=window.open(url,'name','height=400,width=600');if(window.focus)newwindow.focus();return false;}
...
{options={minChars:2,matchContains:true,dataType:'json',parse:function(data)
{var rows=new Array();for(var i=0;i<data.length;i++)
{rows[i]={data:data[i],value:data[i].id,result:data[i][AutocompleteField]};}
return rows;},formatItem:function(row,i,n)
{return row[AutocompleteField];}};return options;};function s3_auto_widget_result(event,data,formatted,AutoID,AutocompleteField,PopupURL)
{var NewJSON="{'id':'"+data.id+"'}";var selAutoJSON=$("#"+AutoID+"_auto_json");var selAutoEdit=$("#"+AutoID+"_auto_edit");var selAutoNewLabel=$("#"+AutoID+"_auto_new_label");selAutoJSON.val(NewJSON);selAutoNewLabel.hide();selAutoEdit.show();selAutoEdit.attr("href",PopupURL+"/update/"+data.id+"?format=popup"+"&auto_id="+AutoID+"&fields=['"+AutocompleteField+"']");};function s3_auto_widget_keypress(AutoID)
