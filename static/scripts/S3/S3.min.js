...
var g=Calendar.createElement("thead",a),j=null,b=null,c=this;h=function(m,o,n){j=Calendar.createElement("td",b);j.colSpan=o;j.className="button";if(n!=0&&Math.abs(n)<=2)j.className+=" nav";Calendar._add_evs(j);j.calendar=c;j.navtype=n;j.innerHTML="<div unselectable='on'>"+m+"</div>";return j};b=Calendar.createElement("tr",g);var f=6;this.isPopup&&--f;this.weekNumbers&&++f;h("?",1,400).ttip=Calendar._TT.INFO;this.title=h("",f,300);this.title.className="title";if(this.isPopup){this.title.ttip=Calendar._TT.DRAG_TO_MOVE;
...
break;case 37:case 38:case 39:case 40:if(h){var j,b,c,f,k;h=g==37||g==38;k=g==37||g==39?1:7;var m=function(){f=e.currentDateEl;var p=f.pos;j=p&15;b=p>>4;c=e.ar_days[b][j]};m();for(var o=function(){var p=new Date(e.date);p.setDate(p.getDate()-k);e.setDate(p)},n=function(){var p=new Date(e.date);p.setDate(p.getDate()+k);e.setDate(p)};;){switch(g){case 37:if(--j>=0)c=e.ar_days[b][j];else{j=6;g=38;continue}break;case 38:if(--b>=0)c=e.ar_days[b][j];else{o();m()}break;case 39:if(++j<7)c=e.ar_days[b][j];
else{j=0;g=40;continue}break;case 40:if(++b<e.ar_days.length)c=e.ar_days[b][j];else{n();m()}}break}if(c)if(c.disabled)h?o():n();else Calendar.cellClick(c)}break;case 13:h&&Calendar.cellClick(e.currentDateEl,a);break;default:return false}return Calendar.stopEvent(a)};
...
0;)for(var k=document.getElementsByTagName(e[--f]),m=null,o=k.length;o>0;){m=k[--o];g=Calendar.getAbsolutePos(m);var n=g.x,p=m.offsetWidth+n;g=g.y;var t=m.offsetHeight+g;if(this.hidden||n>b||p<j||g>h||t<c){if(!m.__msh_save_visibility)m.__msh_save_visibility=a(m);m.style.visibility=m.__msh_save_visibility}else{if(!m.__msh_save_visibility)m.__msh_save_visibility=a(m);m.style.visibility="hidden"}}}};
...
<<<<<<< TREE
Date.parseDate=function(a,e){var h=new Date,g=0,j=-1,b=0,c=a.split(/\W+/),f=e.match(/%./g),k=0,m=0,o=0,n=0;for(k=0;k<c.length;++k)if(c[k])switch(f[k]){case "%d":case "%e":b=parseInt(c[k],10);break;case "%m":j=parseInt(c[k],10)-1;break;case "%Y":case "%y":g=parseInt(c[k],10);g<100&&(g+=g>29?1900:2E3);break;case "%b":case "%B":for(m=0;m<12;++m)if(Calendar._MN[m].substr(0,c[k].length).toLowerCase()==c[k].toLowerCase()){j=m;break}break;case "%H":case "%I":case "%k":case "%l":o=parseInt(c[k],10);break;
case "%P":case "%p":if(/pm/i.test(c[k])&&o<12)o+=12;else if(/am/i.test(c[k])&&o>=12)o-=12;break;case "%M":n=parseInt(c[k],10)}if(isNaN(g))g=h.getFullYear();if(isNaN(j))j=h.getMonth();if(isNaN(b))b=h.getDate();if(isNaN(o))o=h.getHours();if(isNaN(n))n=h.getMinutes();if(g!=0&&j!=-1&&b!=0)return new Date(g,j,b,o,n,0);g=0;j=-1;for(k=b=0;k<c.length;++k)if(c[k].search(/[a-zA-Z]+/)!=-1){f=-1;for(m=0;m<12;++m)if(Calendar._MN[m].substr(0,c[k].length).toLowerCase()==c[k].toLowerCase()){f=m;break}if(f!=-1){if(j!=
-1)b=j+1;j=f}}else if(parseInt(c[k],10)<=12&&j==-1)j=c[k]-1;else if(parseInt(c[k],10)>31&&g==0){g=parseInt(c[k],10);g<100&&(g+=g>29?1900:2E3)}else if(b==0)b=c[k];if(g==0)g=h.getFullYear();if(j!=-1&&b!=0)return new Date(g,j,b,o,n,0);return h};Date.prototype.getMonthDays=function(a){var e=this.getFullYear();if(typeof a=="undefined")a=this.getMonth();return 0==e%4&&(0!=e%100||0==e%400)&&a==1?29:Date._MD[a]};
...
<<<<<<< TREE
Calendar.setup=function(a){function e(c,f){if(typeof a[c]=="undefined")a[c]=f}function h(c){var f=c.params,k=c.dateClicked||f.electric;if(k&&f.inputField){f.inputField.value=c.date.print(f.ifFormat);if(typeof f.inputField.onchange=="function")f.inputField.onchange()}if(k&&f.displayArea)f.displayArea.innerHTML=c.date.print(f.daFormat);if(k&&typeof f.onUpdate=="function")f.onUpdate(c);k&&f.flat&&typeof f.flatCallback=="function"&&f.flatCallback(c);k&&f.singleClick&&c.dateClicked&&c.callCloseHandler()}
e("inputField",null);e("displayArea",null);e("button",null);e("eventName","click");e("ifFormat","%Y/%m/%d");e("daFormat","%Y/%m/%d");e("singleClick",true);e("disableFunc",null);e("dateStatusFunc",a.disableFunc);e("dateText",null);e("firstDay",null);e("align","Br");e("range",[1900,2999]);e("weekNumbers",true);e("flat",null);e("flatCallback",null);e("onSelect",null);e("onClose",null);e("onUpdate",null);e("date",null);e("showsTime",false);e("timeFormat","24");e("electric",true);e("step",2);e("position",
null);e("cache",false);e("showOthers",false);e("multiple",null);var g=["inputField","displayArea","button"],j;for(j in g)if(typeof a[g[j]]=="string")a[g[j]]=document.getElementById(a[g[j]]);if(!(a.flat||a.multiple||a.inputField||a.displayArea||a.button)){alert("Calendar.setup:\n  Nothing to setup (no fields found).  Please check your code");return false}if(a.flat!=null){if(typeof a.flat=="string")a.flat=document.getElementById(a.flat);if(!a.flat){alert("Calendar.setup:\n  Flat specified but can't find parent.");
...
c.hide();var f;f=$('[name = "item_id"]').length!=0?S3.Ap.concat("/supply/item_pack.json?item_pack.item_id=",b.val()):S3.Ap.concat("/inv/inv_item_packs/",b.val());h=$.ajax({url:f,dataType:"json",context:b,success:function(k){var m="",o="";if(k.length==0)m+='<option value="">'+S3.i18n.no_packs+"</options>";else{for(var n=0;n<k.length;n++)if(k[n].quantity==1){o=k[n].name;break}for(n=0;n<k.length;n++){m+='<option value="'+k[n].id+'">';m+=k[n].quantity==1?k[n].name:k[n].name+" ("+k[n].quantity+" x "+o+
...
<<<<<<< TREE
(function(a,e){function h(v,G){v=v?' id="'+n+v+'"':"";return a("<div"+v+(G?' style="'+G+'"':"")+"/>")}function g(v,G){G=G==="x"?O.width():O.height();return typeof v==="string"?Math.round(/%/.test(v)?G/100*parseInt(v,10):parseInt(v,10)):v}function j(v){return s.photo||/\.(gif|png|jpg|jpeg|bmp)(?:\?([^#]*))?(?:#(\.*))?$/i.test(v)}function b(v){for(var G in v)if(a.isFunction(v[G])&&G.substring(0,2)!=="on")v[G]=v[G].call(X);v.rel=v.rel||X.rel||"nofollow";v.href=v.href||a(X).attr("href");v.title=v.title||
X.title;return v}function c(v,G){G&&G.call(X);a.event.trigger(v)}function f(){var v,G=n+"Slideshow_",D="click."+n,S,V;if(s.slideshow&&M[1]){S=function(){W.text(s.slideshowStop).unbind(D).bind(x,function(){if(K<M.length-1||s.loop)v=setTimeout(A.next,s.slideshowSpeed)}).bind(t,function(){clearTimeout(v)}).one(D+" "+w,V);H.removeClass(G+"off").addClass(G+"on");v=setTimeout(A.next,s.slideshowSpeed)};V=function(){clearTimeout(v);W.text(s.slideshowStart).unbind([x,t,w,D].join(" ")).one(D,S);H.removeClass(G+
"on").addClass(G+"off")};s.slideshowAuto?S():V()}}function k(v){if(!E){X=v;s=b(a.extend({},a.data(X,o)));M=a(X);K=0;if(s.rel!=="nofollow"){M=a("."+ka).filter(function(){return(a.data(this,o).rel||this.rel)===s.rel});K=M.index(X);if(K===-1){M=M.add(X);K=M.length-1}}if(!ea){ea=ma=true;H.show();if(s.returnFocus)try{X.blur();a(X).one(u,function(){try{this.focus()}catch(D){}})}catch(G){}P.css({opacity:+s.opacity,cursor:s.overlayClose?"pointer":"auto"}).show();s.w=g(s.initialWidth,"x");s.h=g(s.initialHeight,
=======
...
<<<<<<< TREE
A.init=function(){O=a(e);H=h().attr({id:o,"class":z?n+"IE":""});P=h("Overlay",B?"position:absolute":"").hide();F=h("Wrapper");J=h("Content").append(N=h("LoadedContent","width:0; height:0; overflow:hidden"),T=h("LoadingOverlay").add(h("LoadingGraphic")),ca=h("Title"),L=h("Current"),ga=h("Next"),Z=h("Previous"),W=h("Slideshow").bind(p,f),R=h("Close"));F.append(h().append(h("TopLeft"),aa=h("TopCenter"),h("TopRight")),h(false,"clear:left").append(ha=h("MiddleLeft"),J,da=h("MiddleRight")),h(false,"clear:left").append(h("BottomLeft"),
ja=h("BottomCenter"),h("BottomRight"))).children().children().css({"float":"left"});I=h(false,"position:absolute; width:9999px; visibility:hidden; display:none");a("body").prepend(P,H.append(F,I));J.children().hover(function(){a(this).addClass("hover")},function(){a(this).removeClass("hover")}).addClass("hover");U=aa.height()+ja.height()+J.outerHeight(true)-J.height();ia=ha.width()+da.width()+J.outerWidth(true)-J.width();fa=N.outerHeight(true);Y=N.outerWidth(true);H.css({"padding-bottom":U,"padding-right":ia}).hide();
=======
...
<<<<<<< TREE
s.w+Y&&H.height()===s.h+fa?0:v;F[0].style.width=F[0].style.height="9999px";H.dequeue().animate({width:s.w+Y,height:s.h+fa,top:V,left:Q},{duration:S,complete:function(){D(this);ma=false;F[0].style.width=s.w+Y+ia+"px";F[0].style.height=s.h+fa+U+"px";G&&G()},step:function(){D(this)}})};A.resize=function(v){if(ea){v=v||{};if(v.width)s.w=g(v.width,"x")-Y-ia;if(v.innerWidth)s.w=g(v.innerWidth,"x");N.css({width:s.w});if(v.height)s.h=g(v.height,"y")-fa-U;if(v.innerHeight)s.h=g(v.innerHeight,"y");if(!v.innerHeight&&
=======
...
<<<<<<< TREE
S=s.transition==="none"?0:s.speed;O.unbind("resize."+n);N.remove();N=h("LoadedContent").html(v);N.hide().appendTo(I.show()).css({width:function(){s.w=s.w||N.width();s.w=s.mw&&s.mw<s.w?s.mw:s.w;return s.w}(),overflow:s.scrolling?"auto":"hidden"}).css({height:function(){s.h=s.h||N.height();s.h=s.mh&&s.mh<s.h?s.mh:s.h;return s.h}()}).prependTo(J);I.hide();a("#"+n+"Photo").css({cssFloat:"none",marginLeft:"auto",marginRight:"auto"});if(B)a("select").not(H.find("select")).filter(function(){return this.style.visibility!==
"hidden"}).css({visibility:"hidden"}).one(w,function(){this.style.visibility="inherit"});s.transition==="fade"?H.fadeTo(S,0,function(){G(0)}):G(S)}};A.load=function(v){var G,D,S,V=A.prep;ma=true;X=M[K];v||(s=b(a.extend({},a.data(X,o))));c(r);c(t,s.onLoad);s.h=s.height?g(s.height,"y")-fa-U:s.innerHeight&&g(s.innerHeight,"y");s.w=s.width?g(s.width,"x")-Y-ia:s.innerWidth&&g(s.innerWidth,"x");s.mw=s.w;s.mh=s.h;if(s.maxWidth){s.mw=g(s.maxWidth,"x")-Y-ia;s.mw=s.w&&s.w<s.mw?s.w:s.mw}if(s.maxHeight){s.mh=
g(s.maxHeight,"y")-fa-U;s.mh=s.h&&s.h<s.mh?s.h:s.mh}G=s.href;T.show();if(s.inline){h().hide().insertBefore(a(G)[0]).one(r,function(){a(this).replaceWith(N.children())});V(a(G))}else if(s.iframe){H.one(y,function(){var Q=a("<iframe frameborder='0' style='width:100%; height:100%; border:0; display:block'/>")[0];Q.name=n+ +new Date;Q.src=s.href;if(!s.scrolling)Q.scrolling="no";if(z)Q.allowtransparency="true";a(Q).appendTo(N).one(r,function(){Q.src="//about:blank"})});V(" ")}else if(s.html)V(s.html);
=======
...
b[0]?a.timepicker._hideTimepicker():a.timepicker._showTimepicker(b[0]);return false})}},_inlineTimepicker:function(b,c){var f=a(b);if(!f.hasClass(this.markerClassName)){f.addClass(this.markerClassName).append(c.tpDiv).bind("setData.timepicker",function(k,m,o){c.settings[m]=o}).bind("getData.timepicker",function(k,m){return this._get(c,m)});a.data(b,"timepicker",c);this._setTimeFromField(c);this._updateTimepicker(c);c.tpDiv.show()}},_showTimepicker:function(b){b=b.target||b;if(b.nodeName.toLowerCase()!=
...
c.tpDiv.css({position:a.timepicker._inDialog&&a.blockUI?"static":k?"fixed":"absolute",display:"none",left:f.left+"px",top:f.top+"px"});if(!c.inline){f=a.timepicker._get(c,"showAnim");var m=a.timepicker._get(c,"duration"),o=function(){a.timepicker._timepickerShowing=true;var n=a.timepicker._getBorders(c.tpDiv);c.tpDiv.find("iframe.ui-timepicker-cover").css({left:-n[0],top:-n[1],width:c.tpDiv.outerWidth(),height:c.tpDiv.outerHeight()})};c.tpDiv.zIndex(a(b).zIndex()+1);if(a.effects&&a.effects[f])c.tpDiv.show(f,
a.timepicker._get(c,"showOptions"),m,o);else c.tpDiv[f||"show"](f?m:null,o);if(!f||!m)o();c.input.is(":visible")&&!c.input.is(":disabled")&&c.input.focus();a.timepicker._curInst=c}}},_updateTimepicker:function(b){var c=this,f=a.timepicker._getBorders(b.tpDiv);b.tpDiv.empty().append(this._generateHTML(b)).find("iframe.ui-timepicker-cover").css({left:-f[0],top:-f[1],width:b.tpDiv.outerWidth(),height:b.tpDiv.outerHeight()}).end().find(".ui-timepicker td a").bind("mouseout",function(){a(this).removeClass("ui-state-hover");
=======
...
this._get(b,"hourText")+'</div><table class="ui-timepicker">';f+='<tr><th rowspan="2" class="periods">'+n[0]+"</th>";for(c=0;c<=5;c++)f+=this._generateHTMLHourCell(b,c,k,m);f+="</tr><tr>";for(c=6;c<=11;c++)f+=this._generateHTMLHourCell(b,c,k,m);f+='</tr><tr><th rowspan="2" class="periods">'+n[1]+"</th>";for(c=12;c<=17;c++)f+=this._generateHTMLHourCell(b,c,k,m);f+="</tr><tr>";for(c=18;c<=23;c++)f+=this._generateHTMLHourCell(b,c,k,m);f+='</tr></table></td><td class="ui-timepicker-minutes"><div class="ui-timepicker-title ui-widget-header ui-helper-clearfix ui-corner-all">'+
this._get(b,"minuteText")+'</div><table class="ui-timepicker"><tr>';for(c=0;c<15;c+=5)f+=this._generateHTMLMinuteCell(b,c,c<10&&o?"0"+c.toString():c.toString());f+="</tr><tr>";for(c=15;c<30;c+=5)f+=this._generateHTMLMinuteCell(b,c,c.toString());f+="</tr><tr>";for(c=30;c<45;c+=5)f+=this._generateHTMLMinuteCell(b,c,c.toString());f+="</tr><tr>";for(c=45;c<60;c+=5)f+=this._generateHTMLMinuteCell(b,c,c.toString());f+="</tr></table></td></tr></table>";return f},_generateHTMLHourCell:function(b,c,f,k){var m=
...
a.timepicker._timepickerShowing&&!(a.timepicker._inDialog&&a.blockUI)&&a.timepicker._hideTimepicker()}},_hideTimepicker:function(b){var c=this._curInst;if(!(!c||b&&c!=a.data(b,"timepicker")))if(this._timepickerShowing){b=this._get(c,"showAnim");var f=this._get(c,"duration"),k=function(){a.timepicker._tidyDialog(c);this._curInst=null};if(a.effects&&a.effects[b])c.tpDiv.hide(b,a.timepicker._get(c,"showOptions"),f,k);else c.tpDiv[b=="slideDown"?"slideUp":b=="fadeIn"?"fadeOut":"hide"](b?f:null,k);b||
=======
...
0||b.hours>23)b.hours=12;if(b.minutes<0||b.minutes>59)b.minutes=0;var c="",f=this._get(b,"showPeriod")==true,k=this._get(b,"showLeadingZero")==true,m=this._get(b,"amPmText"),o=b.minutes?b.minutes:0,n=b.hours?b.hours:0;n||(displayHoyrs=0);if(f){if(b.hours==0)n=12;if(b.hours<12)c=m[0];else{c=m[1];if(n>12)n-=12}}f=n.toString();if(k&&n<10)f="0"+f;k=o.toString();if(o<10)k="0"+k;o=f+this._get(b,"timeSeparator")+k;if(c.length>0)o+=" "+c;if(b.input){b.input.val(o);b.input.trigger("change")}if(c=this._get(b,
...
a})(jQuery);
<<<<<<< TREE
(function(a){a.cluetip={version:"1.0.7"};var e,h,g,j,b,c,f,k;a.fn.cluetip=function(n,p){if(typeof n=="object"){p=n;n=null}if(n=="destroy")return this.removeData("thisInfo").unbind(".cluetip");return this.each(function(t){function x(){return false}var w=this,u=a(this),r=a.extend(true,{},a.fn.cluetip.defaults,p||{},a.metadata?u.metadata():a.meta?u.data():{}),y=false,z=+r.cluezIndex;u.data("thisInfo",{title:w.title,zIndex:z});var B=false,C=0;if(!a("#cluetip").length){a('<div id="cluetip"><div id="cluetip-outer"><h3 id="cluetip-title"></h3><div id="cluetip-inner"></div></div><div id="cluetip-extra"></div><div id="cluetip-arrows" class="cluetip-arrows"></div></div>')[m](o).hide();e=
a("#cluetip").css({position:"absolute"});g=a("#cluetip-outer").css({position:"relative",zIndex:z});h=a("#cluetip-inner");j=a("#cluetip-title");b=a("#cluetip-arrows");c=a('<div id="cluetip-waitimage"></div>').css({position:"absolute"}).insertBefore(e).hide()}var P=r.dropShadow?+r.dropShadowSteps:0;if(!f){f=a([]);for(var H=0;H<P;H++)f=f.add(a("<div></div>").css({zIndex:z-1,opacity:0.1,top:1+H,left:1+H}));f.css({position:"absolute",backgroundColor:"#000"}).prependTo(e)}var F=u.attr(r.attribute),J=r.cluetipClass;
if(!F&&!r.splitTitle&&!n)return true;if(r.local&&r.localPrefix)F=r.localPrefix+F;r.local&&r.hideLocal&&a(F+":first").hide();var aa=parseInt(r.topOffset,10),ha=parseInt(r.leftOffset,10),da,ja,M=isNaN(parseInt(r.height,10))?"auto":/\D/g.test(r.height)?r.height:r.height+"px",O,N,I,T,ca,L=parseInt(r.width,10)||275,W=L+(parseInt(e.css("paddingLeft"),10)||0)+(parseInt(e.css("paddingRight"),10)||0)+P,ga=this.offsetWidth,Z,R,U,ia,fa,Y=r.attribute!="title"?u.attr(r.titleAttribute):"";if(r.splitTitle){if(Y==
undefined)Y="";fa=Y.split(r.splitTitle);Y=fa.shift()}if(r.escapeTitle)Y=Y.replace(/&/g,"&amp;").replace(/>/g,"&gt;").replace(/</g,"&lt;");var X=function(E){if(!r.onActivate(u))return false;B=true;e.removeClass().css({width:L});F==u.attr("href")&&u.css("cursor",r.cursor);r.hoverClass&&u.addClass(r.hoverClass);N=u.offset().top;Z=u.offset().left;U=E.pageX;T=E.pageY;if(w.tagName.toLowerCase()!="area"){O=a(document).scrollTop();ia=a(window).width()}if(r.positionBy=="fixed"){R=ga+Z+ha;e.css({left:R})}else{R=
ga>Z&&Z>W||Z+ga+W+ha>ia?Z-W-ha:ga+Z+ha;if(w.tagName.toLowerCase()=="area"||r.positionBy=="mouse"||ga+W>ia)if(U+20+W>ia){e.addClass(" cluetip-"+J);R=U-W-ha>=0?U-W-ha-parseInt(e.css("marginLeft"),10)+parseInt(h.css("marginRight"),10):U-W/2}else R=U+ha;var A=R<0?E.pageY+aa:E.pageY;e.css({left:R>0&&r.positionBy!="bottomTop"?R:U+W/2>ia?ia/2-W/2:Math.max(U-W/2,0),zIndex:u.data("thisInfo").zIndex});b.css({zIndex:u.data("thisInfo").zIndex+1})}ja=a(window).height();if(n){if(typeof n=="function")n=n.call(w);
h.html(n);K(A)}else if(fa){E=fa.length;h.html(E?fa[0]:"");if(E>1)for(var ka=1;ka<E;ka++)h.append('<div class="split-body">'+fa[ka]+"</div>");K(A)}else if(!r.local&&F.indexOf("#")!==0)if(/\.(jpe?g|tiff?|gif|png)$/i.test(F)){h.html('<img src="'+F+'" alt="'+Y+'" />');K(A)}else if(y&&r.ajaxCache){h.html(y);K(A)}else{var v=r.ajaxSettings.beforeSend,G=r.ajaxSettings.error,D=r.ajaxSettings.success,S=r.ajaxSettings.complete;E=a.extend(true,{},r.ajaxSettings,{cache:false,url:F,beforeSend:function(V){v&&v.call(w,
V,e,h);g.children().empty();r.waitImage&&c.css({top:T+20,left:U+20,zIndex:u.data("thisInfo").zIndex-1}).show()},error:function(V,Q){if(B)G?G.call(w,V,Q,e,h):h.html("<i>sorry, the contents could not be loaded</i>")},success:function(V,Q){y=r.ajaxProcess.call(w,V);if(B){D&&D.call(w,V,Q,e,h);h.html(y)}},complete:function(V,Q){S&&S.call(w,V,Q,e,h);var ba=h[0].getElementsByTagName("img");k=ba.length;for(var la=0,na=ba.length;la<na;la++)ba[la].complete&&k--;if(k&&!a.browser.opera)a(ba).bind("load error",
function(){k--;if(k<1){c.hide();B&&K(A)}});else{c.hide();B&&K(A)}}});a.ajax(E)}else if(r.local){E=a(F+(/#\S+$/.test(F)?"":":eq("+t+")")).clone(true).show();h.html(E);K(A)}},K=function(E){e.addClass("cluetip-"+J);if(r.truncate){var A=h.text().slice(0,r.truncate)+"...";h.html(A)}Y?j.show().html(Y):r.showTitle?j.show().html("&nbsp;"):j.hide();if(r.sticky){A=a('<div id="cluetip-close"><a href="#">'+r.closeText+"</a></div>");r.closePosition=="bottom"?A.appendTo(h):r.closePosition=="title"?A.prependTo(j):
...
N+da+aa>ca?da>=ja?O:ca-da-aa:u.css("display")=="block"||w.tagName.toLowerCase()=="area"||r.positionBy=="mouse"?E-aa:N-r.dropShadowSteps;if(A=="")R<Z?A="left":A="right";e.css({top:I+"px"}).removeClass().addClass("clue-"+A+"-"+J).addClass(" cluetip-"+J);if(r.arrows){E=N-I-r.dropShadowSteps;b.css({top:/(left|right)/.test(A)&&R>=0&&E>0?E+"px":/(left|right)/.test(A)?0:""}).show()}else b.hide();f.hide();e.hide()[r.fx.open](r.fx.openSpeed||0);r.dropShadow&&f.css({height:da,width:L,zIndex:u.data("thisInfo").zIndex-
1}).show();a.fn.bgiframe&&e.bgiframe();if(r.delayedClose>0)C=setTimeout(ea,r.delayedClose);r.onShow.call(w,e,h)},s=function(){B=false;c.hide();if(!r.sticky||/click|toggle/.test(r.activation)){ea();clearTimeout(C)}r.hoverClass&&u.removeClass(r.hoverClass)},ea=function(){g.parent().hide().removeClass();r.onHide.call(w,e,h);u.removeClass("cluetip-clicked");Y&&u.attr(r.titleAttribute,Y);u.css("cursor","");r.arrows&&b.css({top:""})};a(document).bind("hideCluetip",function(){ea()});if(/click|toggle/.test(r.activation))u.bind("click.cluetip",
function(E){if(e.is(":hidden")||!u.is(".cluetip-clicked")){X(E);a(".cluetip-clicked").removeClass("cluetip-clicked");u.addClass("cluetip-clicked")}else s(E);this.blur();return false});else if(r.activation=="focus"){u.bind("focus.cluetip",function(E){X(E)});u.bind("blur.cluetip",function(E){s(E)})}else{u[r.clickThrough?"unbind":"bind"]("click",x);var ma=function(E){if(r.tracking==true){var A=R-E.pageX,ka=I?I-E.pageY:N-E.pageY;u.bind("mousemove.cluetip",function(v){e.css({left:v.pageX+A,top:v.pageY+
=======
...
<<<<<<< TREE
jQuery&&function(a){function e(c,f,k){for(var m="",o=0;o<f.length;o++)if(f[o].optgroup){m+='<label class="optGroup">';m+=k.optGroupSelectable?'<input type="checkbox" class="optGroup" />'+f[o].optgroup:f[o].optgroup;m+='</label><div class="optGroupContainer">';m+=e(c,f[o].options,k);m+="</div>"}else{var n=f[o],p='<label><input type="checkbox" name="'+c+'[]" value="'+n.value+'"';if(n.selected)p+=' checked="checked"';p+=" />"+n.text+"</label>";m+=p}return m}function h(c){var f=a(this),k=f.next(".multiSelectOptions"),
m=f.data("config"),o=f.data("callback");k.html("");var n="";if(m.selectAll)n+='<label class="selectAll"><input type="checkbox" class="selectAll" />'+m.selectAllText+"</label>";n+=e(f.attr("id"),c,m);k.html(n);c=k.width();n=false;if(k.height()>m.listHeight){k.css("height",m.listHeight+"px");n=true}else k.css("height","");c=n&&c==k.width()?17:0;k.width()+c<f.outerWidth()?k.css("width",f.outerWidth()-2+"px"):k.css("width",k.width()+c+"px");a.fn.bgiframe&&f.next(".multiSelectOptions").bgiframe({width:k.width(),
=======
...
w>0)u=w-1;if(u>=0){a(x.get(w)).removeClass("hover");a(x.get(u)).addClass("hover");g(t)}return false}if(p.keyCode==13||p.keyCode==32){p=t.find("LABEL.hover INPUT:checkbox");p.attr("checked",!p.attr("checked")).parent("LABEL").toggleClass("checked",p.attr("checked"));p.hasClass("selectAll")&&t.find("INPUT:checkbox").attr("checked",p.attr("checked")).parent("LABEL").addClass("checked").toggleClass("checked",p.attr("checked"));b.call(f);o&&o(a(this));return false}if(p.keyCode>=33&&p.keyCode<=126){x=t.find("LABEL:startsWith("+
String.fromCharCode(p.keyCode)+")");var r=x.index(x.filter("LABEL.hover"));w=x.filter(function(y){return y>r});x=(w.length>=1?w:x).filter("LABEL:first");if(x.length==1){t.find("LABEL.hover").removeClass("hover");x.addClass("hover");g(t)}}}else{if(p.keyCode==38||p.keyCode==40||p.keyCode==13||p.keyCode==32){a(this).removeClass("focus").trigger("click");t.find("LABEL:first").addClass("hover");return false}if(p.keyCode==9){t.next(":input").focus();return true}}if(p.keyCode==13)return false})}function g(c){var f=
...
if(j==="absolute"||j==="relative"||j==="fixed"){j=parseInt(g.css("zIndex"),10);if(!isNaN(j)&&j!==0)return j}g=g.parent()}}return 0},disableSelection:function(){return this.bind((a.support.selectstart?"selectstart":"mousedown")+".ui-disableSelection",function(g){g.preventDefault()})},enableSelection:function(){return this.unbind(".ui-disableSelection")}});a.each(["Width","Height"],function(g,j){function b(m,o,n,p){a.each(c,function(){o-=parseFloat(a.curCSS(m,"padding"+this,true))||0;if(n)o-=parseFloat(a.curCSS(m,
"border"+this+"Width",true))||0;if(p)o-=parseFloat(a.curCSS(m,"margin"+this,true))||0});return o}var c=j==="Width"?["Left","Right"]:["Top","Bottom"],f=j.toLowerCase(),k={innerWidth:a.fn.innerWidth,innerHeight:a.fn.innerHeight,outerWidth:a.fn.outerWidth,outerHeight:a.fn.outerHeight};a.fn["inner"+j]=function(m){if(m===e)return k["inner"+j].call(this);return this.each(function(){a(this).css(f,b(this,m)+"px")})};a.fn["outer"+j]=function(m,o){if(typeof m!=="number")return k["outer"+j].call(this,m);return this.each(function(){a(this).css(f,
b(this,m,true,o)+"px")})}});a.extend(a.expr[":"],{data:function(g,j,b){return!!a.data(g,b[3])},focusable:function(g){var j=g.nodeName.toLowerCase(),b=a.attr(g,"tabindex");if("area"===j){j=g.parentNode;b=j.name;if(!g.href||!b||j.nodeName.toLowerCase()!=="map")return false;g=a("img[usemap=#"+b+"]")[0];return!!g&&h(g)}return(/input|select|textarea|button|object/.test(j)?!g.disabled:"a"==j?g.href||!isNaN(b):!isNaN(b))&&h(g)},tabbable:function(g){var j=a.attr(g,"tabindex");return(isNaN(j)||j>=0)&&a(g).is(":focusable")}});
...
function s3_gis_dropdown_select(a,g){s3_gis_pullNext=false;var f=$("#gis_location_name").val(),h=$("#gis_location_lat").val(),j=$("#gis_location_lon").val(),b=$("#gis_location_addr_street").val(),c=$("#gis_location_postcode").val();f=f&&h&&j&&b&&c;if((h=$("#gis_location_L"+a).val())||g){var e;$.getJSONS3(a==s3_gis_maxlevel?s3_gis_url+"/search.json?filter=%3D&field=level&value=nullnone&parent="+h:s3_gis_url+"/search.json?filter=%3D&field=level&value=L"+(a+1)+"&parent="+h,function(k){if(k.length==0)e=
s3_gis_empty_set;else if(k.length==1){e='<option value="'+k[0].id+'">'+k[0].name+"</option>";s3_gis_pullNext=true}else{e=s3_gis_select_location;for(var m=0;m<k.length;m++)e+='<option value="'+k[m].id+'">'+k[m].name+"</option>"}a==s3_gis_maxlevel?$("#gis_location_").html(e):$("#gis_location_L"+(a+1)).html(e)},false,true);if(a==s3_gis_maxlevel){if(e!=s3_gis_empty_set){$("#gis_location_").removeClass("hidden").show();$("#gis_location_label_").removeClass("hidden").show();$("#gis_location_details-btn").removeClass("hidden").show()}}else{$("#gis_location_L"+
(a+1)).removeClass("hidden").show();$("#gis_location_label_L"+(a+1)).removeClass("hidden").show();$("#gis_location_details-btn").removeClass("hidden").show()}s3_gis_dropdown_hide(a+2);if(s3_gis_pullNext){h=$("#gis_location_L"+(a+1)).val();f||$("#"+s3_gis_location_id).val(h);s3_gis_dropdown_select(a+1)}else f||$("#"+s3_gis_location_id).val(h)}else{s3_gis_dropdown_hide(a+1);0==a&&!f&&$("#"+s3_gis_location_id).val("")}}
>>>>>>> MERGE-SOURCE
...
Ext.onReady(function(){var a,e=Ext.get("gis_location_converter-btn"),h=Ext.get("gis_location_map-btn");if(h)h.on("click",function(){mapWin.show(this)});if(e)e.on("click",function(){a||(a=new Ext.Window({applyTo:"gis-convert-win",layout:"fit",width:400,height:250,closeAction:"hide",plain:true,items:new Ext.TabPanel({applyTo:"gis-convert-tabs",autoTabs:true,activeTab:0,deferredRender:false,border:false}),buttons:[{text:s3_gis_fill_lat,disabled:false,handler:function(){if(s3_gis_convertFillBack(1)){s3_gis_calcDone=
=======
...
var g=$("#gis_location_name").val(),f=$("#gis_location_lat").val(),h=$("#gis_location_lon").val(),j=$("#gis_location_addr_street").val(),b=$("#gis_location_postcode").val(),c=$("#gis_location_L0").val();""==g&&(""==f||""==h)&&""==j&&""==b&&""==c||s3_gis_save_location(g,f,h,j,b);return true})}});
...
null||c[f]==e)b[f]=c[f];return b}a.extend(a.ui,{datepicker:{version:"1.8.9"}});var j=(new Date).getTime();a.extend(h.prototype,{markerClassName:"hasDatepicker",log:function(){this.debug&&console.log.apply("",arguments)},_widgetDatepicker:function(){return this.dpDiv},setDefaults:function(b){g(this._defaults,b||{});return this},_attachDatepicker:function(b,c){var f=null,k;for(k in this._defaults){var m=b.getAttribute("date:"+k);if(m){f=f||{};try{f[k]=eval(m)}catch(o){f[k]=m}}}k=b.nodeName.toLowerCase();
m=k=="div"||k=="span";if(!b.id){this.uuid+=1;b.id="dp"+this.uuid}var n=this._newInst(a(b),m);n.settings=a.extend({},c||{},f||{});if(k=="input")this._connectDatepicker(b,n);else m&&this._inlineDatepicker(b,n)},_newInst:function(b,c){return{id:b[0].id.replace(/([^A-Za-z0-9_-])/g,"\\\\$1"),input:b,selectedDay:0,selectedMonth:0,selectedYear:0,drawMonth:0,drawYear:0,inline:c,dpDiv:!c?this.dpDiv:a('<div class="'+this._inlineClass+' ui-datepicker ui-widget ui-widget-content ui-helper-clearfix ui-corner-all"></div>')}},
_connectDatepicker:function(b,c){var f=a(b);c.append=a([]);c.trigger=a([]);if(!f.hasClass(this.markerClassName)){this._attachments(f,c);f.addClass(this.markerClassName).keydown(this._doKeyDown).keypress(this._doKeyPress).keyup(this._doKeyUp).bind("setData.datepicker",function(k,m,o){c.settings[m]=o}).bind("getData.datepicker",function(k,m){return this._get(c,m)});this._autoSize(c);a.data(b,"datepicker",c)}},_attachments:function(b,c){var f=this._get(c,"appendText"),k=this._get(c,"isRTL");c.append&&
...
f.match(/MM/)?"monthNames":"monthNamesShort")));c.setDate(k(this._get(b,f.match(/DD/)?"dayNames":"dayNamesShort"))+20-c.getDay())}b.input.attr("size",this._formatDate(b,c).length)}},_inlineDatepicker:function(b,c){var f=a(b);if(!f.hasClass(this.markerClassName)){f.addClass(this.markerClassName).append(c.dpDiv).bind("setData.datepicker",function(k,m,o){c.settings[m]=o}).bind("getData.datepicker",function(k,m){return this._get(c,m)});a.data(b,"datepicker",c);this._setDate(c,this._getDefaultDate(c),
...
k.settings):this._get(k,c):null;var m=c||{};if(typeof c=="string"){m={};m[c]=f}if(k){this._curInst==k&&this._hideDatepicker();var o=this._getDateDatepicker(b,true);g(k.settings,m);this._attachments(a(b),k);this._autoSize(k);this._setDateDatepicker(b,o);this._updateDatepicker(k)}},_changeDatepicker:function(b,c,f){this._optionDatepicker(b,c,f)},_refreshDatepicker:function(b){(b=this._getInst(b))&&this._updateDatepicker(b)},_setDateDatepicker:function(b,c){var f=this._getInst(b);if(f){this._setDate(f,
...
null;c.dpDiv.empty();c.dpDiv.css({position:"absolute",display:"block",top:"-1000px"});a.datepicker._updateDatepicker(c);f=a.datepicker._checkOffset(c,f,k);c.dpDiv.css({position:a.datepicker._inDialog&&a.blockUI?"static":k?"fixed":"absolute",display:"none",left:f.left+"px",top:f.top+"px"});if(!c.inline){f=a.datepicker._get(c,"showAnim");var m=a.datepicker._get(c,"duration"),o=function(){a.datepicker._datepickerShowing=true;var n=c.dpDiv.find("iframe.ui-datepicker-cover");if(n.length){var p=a.datepicker._getBorders(c.dpDiv);
n.css({left:-p[0],top:-p[1],width:c.dpDiv.outerWidth(),height:c.dpDiv.outerHeight()})}};c.dpDiv.zIndex(a(b).zIndex()+1);if(a.effects&&a.effects[f])c.dpDiv.show(f,a.datepicker._get(c,"showOptions"),m,o);else c.dpDiv[f||"show"](f?m:null,o);if(!f||!m)o();c.input.is(":visible")&&!c.input.is(":disabled")&&c.input.focus();a.datepicker._curInst=c}}},_updateDatepicker:function(b){var c=this,f=a.datepicker._getBorders(b.dpDiv);b.dpDiv.empty().append(this._generateHTML(b));var k=b.dpDiv.find("iframe.ui-datepicker-cover");
...
m=b.yearshtml=null},0)}},_getBorders:function(b){var c=function(f){return{thin:1,medium:2,thick:3}[f]||f};return[parseFloat(c(b.css("border-left-width"))),parseFloat(c(b.css("border-top-width")))]},_checkOffset:function(b,c,f){var k=b.dpDiv.outerWidth(),m=b.dpDiv.outerHeight(),o=b.input?b.input.outerWidth():0,n=b.input?b.input.outerHeight():0,p=document.documentElement.clientWidth+a(document).scrollLeft(),t=document.documentElement.clientHeight+a(document).scrollTop();c.left-=this._get(b,"isRTL")?
k-o:0;c.left-=f&&c.left==b.input.offset().left?a(document).scrollLeft():0;c.top-=f&&c.top==b.input.offset().top+n?a(document).scrollTop():0;c.left-=Math.min(c.left,c.left+k>p&&p>k?Math.abs(c.left+k-p):0);c.top-=Math.min(c.top,c.top+m>t&&t>m?Math.abs(m+n):0);return c},_findPos:function(b){for(var c=this._get(this._getInst(b),"isRTL");b&&(b.type=="hidden"||b.nodeType!=1);)b=b[c?"previousSibling":"nextSibling"];b=a(b).offset();return[b.left,b.top]},_hideDatepicker:function(b){var c=this._curInst;if(!(!c||
b&&c!=a.data(b,"datepicker")))if(this._datepickerShowing){b=this._get(c,"showAnim");var f=this._get(c,"duration"),k=function(){a.datepicker._tidyDialog(c);this._curInst=null};if(a.effects&&a.effects[b])c.dpDiv.hide(b,a.datepicker._get(c,"showOptions"),f,k);else c.dpDiv[b=="slideDown"?"slideUp":b=="fadeIn"?"fadeOut":"hide"](b?f:null,k);b||k();if(b=this._get(c,"onClose"))b.apply(c.input?c.input[0]:null,[c.input?c.input.val():"",c]);this._datepickerShowing=false;this._lastInput=null;if(this._inDialog){this._dialogInput.css({position:"absolute",
...
100+parseInt(k,10);for(var m=(f?f.dayNamesShort:null)||this._defaults.dayNamesShort,o=(f?f.dayNames:null)||this._defaults.dayNames,n=(f?f.monthNamesShort:null)||this._defaults.monthNamesShort,p=(f?f.monthNames:null)||this._defaults.monthNames,t=f=-1,x=-1,w=-1,u=false,r=function(F){(F=P+1<b.length&&b.charAt(P+1)==F)&&P++;return F},y=function(F){var J=r(F);F=c.substring(C).match(RegExp("^\\d{1,"+(F=="@"?14:F=="!"?20:F=="y"&&J?4:F=="o"?3:2)+"}"));if(!F)throw"Missing number at position "+C;C+=F[0].length;
return parseInt(F[0],10)},z=function(F,J,aa){F=r(F)?aa:J;for(J=0;J<F.length;J++)if(c.substr(C,F[J].length).toLowerCase()==F[J].toLowerCase()){C+=F[J].length;return J+1}throw"Unknown name at position "+C;},B=function(){if(c.charAt(C)!=b.charAt(P))throw"Unexpected literal at position "+C;C++},C=0,P=0;P<b.length;P++)if(u)if(b.charAt(P)=="'"&&!r("'"))u=false;else B();else switch(b.charAt(P)){case "d":x=y("d");break;case "D":z("D",m,o);break;case "o":w=y("o");break;case "m":t=y("m");break;case "M":t=z("M",
n,p);break;case "y":f=y("y");break;case "@":var H=new Date(y("@"));f=H.getFullYear();t=H.getMonth()+1;x=H.getDate();break;case "!":H=new Date((y("!")-this._ticksTo1970)/1E4);f=H.getFullYear();t=H.getMonth()+1;x=H.getDate();break;case "'":if(r("'"))B();else u=true;break;default:B()}if(f==-1)f=(new Date).getFullYear();else if(f<100)f+=(new Date).getFullYear()-(new Date).getFullYear()%100+(f<=k?0:-100);if(w>-1){t=1;x=w;do{k=this._getDaysInMonth(f,t-1);if(x<=k)break;t++;x-=k}while(1)}H=this._daylightSavingAdjust(new Date(f,
t-1,x));if(H.getFullYear()!=f||H.getMonth()+1!=t||H.getDate()!=x)throw"Invalid date";return H},ATOM:"yy-mm-dd",COOKIE:"D, dd M yy",ISO_8601:"yy-mm-dd",RFC_822:"D, d M y",RFC_850:"DD, dd-M-y",RFC_1036:"D, d M y",RFC_1123:"D, d M yy",RFC_2822:"D, d M yy",RSS:"D, d M y",TICKS:"!",TIMESTAMP:"@",W3C:"yy-mm-dd",_ticksTo1970:(718685+Math.floor(492.5)-Math.floor(19.7)+Math.floor(4.925))*864E9,formatDate:function(b,c,f){if(!c)return"";var k=(f?f.dayNamesShort:null)||this._defaults.dayNamesShort,m=(f?f.dayNames:
null)||this._defaults.dayNames,o=(f?f.monthNamesShort:null)||this._defaults.monthNamesShort;f=(f?f.monthNames:null)||this._defaults.monthNames;var n=function(r){(r=u+1<b.length&&b.charAt(u+1)==r)&&u++;return r},p=function(r,y,z){y=""+y;if(n(r))for(;y.length<z;)y="0"+y;return y},t=function(r,y,z,B){return n(r)?B[y]:z[y]},x="",w=false;if(c)for(var u=0;u<b.length;u++)if(w)if(b.charAt(u)=="'"&&!n("'"))w=false;else x+=b.charAt(u);else switch(b.charAt(u)){case "d":x+=p("d",c.getDate(),2);break;case "D":x+=
t("D",c.getDay(),k,m);break;case "o":x+=p("o",(c.getTime()-(new Date(c.getFullYear(),0,0)).getTime())/864E5,3);break;case "m":x+=p("m",c.getMonth()+1,2);break;case "M":x+=t("M",c.getMonth(),o,f);break;case "y":x+=n("y")?c.getFullYear():(c.getYear()%100<10?"0":"")+c.getYear()%100;break;case "@":x+=c.getTime();break;case "!":x+=c.getTime()*1E4+this._ticksTo1970;break;case "'":if(n("'"))x+="'";else w=true;break;default:x+=b.charAt(u)}return x},_possibleChars:function(b){for(var c="",f=false,k=function(o){(o=
m+1<b.length&&b.charAt(m+1)==o)&&m++;return o},m=0;m<b.length;m++)if(f)if(b.charAt(m)=="'"&&!k("'"))f=false;else c+=b.charAt(m);else switch(b.charAt(m)){case "d":case "m":case "y":case "@":c+="0123456789";break;case "D":case "M":return null;case "'":if(k("'"))c+="'";else f=true;break;default:c+=b.charAt(m)}return c},_get:function(b,c){return b.settings[c]!==e?b.settings[c]:this._defaults[c]},_setDateFromField:function(b,c){if(b.input.val()!=b.lastVal){var f=this._get(b,"dateFormat"),k=b.lastVal=b.input?
b.input.val():null,m,o;m=o=this._getDefaultDate(b);var n=this._getFormatConfig(b);try{m=this.parseDate(f,k,n)||o}catch(p){this.log(p);k=c?"":k}b.selectedDay=m.getDate();b.drawMonth=b.selectedMonth=m.getMonth();b.drawYear=b.selectedYear=m.getFullYear();b.currentDay=k?m.getDate():0;b.currentMonth=k?m.getMonth():0;b.currentYear=k?m.getFullYear():0;this._adjustInstDate(b)}},_getDefaultDate:function(b){return this._restrictMinMax(b,this._determineDate(b,this._get(b,"defaultDate"),new Date))},_determineDate:function(b,
c,f){var k=function(m){var o=new Date;o.setDate(o.getDate()+m);return o};if(c=(c=c==null||c===""?f:typeof c=="string"?function(m){try{return a.datepicker.parseDate(a.datepicker._get(b,"dateFormat"),m,a.datepicker._getFormatConfig(b))}catch(o){}var n=(m.toLowerCase().match(/^c/)?a.datepicker._getDate(b):null)||new Date,p=n.getFullYear(),t=n.getMonth();n=n.getDate();for(var x=/([+-]?[0-9]+)\s*(d|D|w|W|m|M|y|Y)?/g,w=x.exec(m);w;){switch(w[2]||"d"){case "d":case "D":n+=parseInt(w[1],10);break;case "w":case "W":n+=
parseInt(w[1],10)*7;break;case "m":case "M":t+=parseInt(w[1],10);n=Math.min(n,a.datepicker._getDaysInMonth(p,t));break;case "y":case "Y":p+=parseInt(w[1],10);n=Math.min(n,a.datepicker._getDaysInMonth(p,t))}w=x.exec(m)}return new Date(p,t,n)}(c):typeof c=="number"?isNaN(c)?f:k(c):new Date(c.getTime()))&&c.toString()=="Invalid Date"?f:c){c.setHours(0);c.setMinutes(0);c.setSeconds(0);c.setMilliseconds(0)}return this._daylightSavingAdjust(c)},_daylightSavingAdjust:function(b){if(!b)return null;b.setHours(b.getHours()>
12?b.getHours()+2:0);return b},_setDate:function(b,c,f){var k=!c,m=b.selectedMonth,o=b.selectedYear;c=this._restrictMinMax(b,this._determineDate(b,c,new Date));b.selectedDay=b.currentDay=c.getDate();b.drawMonth=b.selectedMonth=b.currentMonth=c.getMonth();b.drawYear=b.selectedYear=b.currentYear=c.getFullYear();if((m!=b.selectedMonth||o!=b.selectedYear)&&!f)this._notifyChange(b);this._adjustInstDate(b);if(b.input)b.input.val(k?"":this._formatDate(b))},_getDate:function(b){return!b.currentYear||b.input&&
b.input.val()==""?null:this._daylightSavingAdjust(new Date(b.currentYear,b.currentMonth,b.currentDay))},_generateHTML:function(b){var c=new Date;c=this._daylightSavingAdjust(new Date(c.getFullYear(),c.getMonth(),c.getDate()));var f=this._get(b,"isRTL"),k=this._get(b,"showButtonPanel"),m=this._get(b,"hideIfNoPrevNext"),o=this._get(b,"navigationAsDateFormat"),n=this._getNumberOfMonths(b),p=this._get(b,"showCurrentAtPos"),t=this._get(b,"stepMonths"),x=n[0]!=1||n[1]!=1,w=this._daylightSavingAdjust(!b.currentDay?
new Date(9999,9,9):new Date(b.currentYear,b.currentMonth,b.currentDay)),u=this._getMinMaxDate(b,"min"),r=this._getMinMaxDate(b,"max");p=b.drawMonth-p;var y=b.drawYear;if(p<0){p+=12;y--}if(r){var z=this._daylightSavingAdjust(new Date(r.getFullYear(),r.getMonth()-n[0]*n[1]+1,r.getDate()));for(z=u&&z<u?u:z;this._daylightSavingAdjust(new Date(y,p,1))>z;){p--;if(p<0){p=11;y--}}}b.drawMonth=p;b.drawYear=y;z=this._get(b,"prevText");z=!o?z:this.formatDate(z,this._daylightSavingAdjust(new Date(y,p-t,1)),this._getFormatConfig(b));
...
b.currentDay?w:c;t=!o?t:this.formatDate(t,B,this._getFormatConfig(b));o=!b.inline?'<button type="button" class="ui-datepicker-close ui-state-default ui-priority-primary ui-corner-all" onclick="DP_jQuery_'+j+'.datepicker._hideDatepicker();">'+this._get(b,"closeText")+"</button>":"";k=k?'<div class="ui-datepicker-buttonpane ui-widget-content">'+(f?o:"")+(this._isInRange(b,B)?'<button type="button" class="ui-datepicker-current ui-state-default ui-priority-secondary ui-corner-all" onclick="DP_jQuery_'+
j+".datepicker._gotoToday('#"+b.id+"');\">"+t+"</button>":"")+(f?"":o)+"</div>":"";o=parseInt(this._get(b,"firstDay"),10);o=isNaN(o)?0:o;t=this._get(b,"showWeek");B=this._get(b,"dayNames");this._get(b,"dayNamesShort");var C=this._get(b,"dayNamesMin"),P=this._get(b,"monthNames"),H=this._get(b,"monthNamesShort"),F=this._get(b,"beforeShowDay"),J=this._get(b,"showOtherMonths"),aa=this._get(b,"selectOtherMonths");this._get(b,"calculateWeek");for(var ha=this._getDefaultDate(b),da="",ja=0;ja<n[0];ja++){for(var M=
"",O=0;O<n[1];O++){var N=this._daylightSavingAdjust(new Date(y,p,b.selectedDay)),I=" ui-corner-all",T="";if(x){T+='<div class="ui-datepicker-group';if(n[1]>1)switch(O){case 0:T+=" ui-datepicker-group-first";I=" ui-corner-"+(f?"right":"left");break;case n[1]-1:T+=" ui-datepicker-group-last";I=" ui-corner-"+(f?"left":"right");break;default:T+=" ui-datepicker-group-middle";I=""}T+='">'}T+='<div class="ui-datepicker-header ui-widget-header ui-helper-clearfix'+I+'">'+(/all|left/.test(I)&&ja==0?f?m:z:"")+
(/all|right/.test(I)&&ja==0?f?z:m:"")+this._generateMonthYearHeader(b,p,y,u,r,ja>0||O>0,P,H)+'</div><table class="ui-datepicker-calendar"><thead><tr>';var ca=t?'<th class="ui-datepicker-week-col">'+this._get(b,"weekHeader")+"</th>":"";for(I=0;I<7;I++){var L=(I+o)%7;ca+="<th"+((I+o+6)%7>=5?' class="ui-datepicker-week-end"':"")+'><span title="'+B[L]+'">'+C[L]+"</span></th>"}T+=ca+"</tr></thead><tbody>";ca=this._getDaysInMonth(y,p);if(y==b.selectedYear&&p==b.selectedMonth)b.selectedDay=Math.min(b.selectedDay,
ca);I=(this._getFirstDayOfMonth(y,p)-o+7)%7;ca=x?6:Math.ceil((I+ca)/7);L=this._daylightSavingAdjust(new Date(y,p,1-I));for(var W=0;W<ca;W++){T+="<tr>";var ga=!t?"":'<td class="ui-datepicker-week-col">'+this._get(b,"calculateWeek")(L)+"</td>";for(I=0;I<7;I++){var Z=F?F.apply(b.input?b.input[0]:null,[L]):[true,""],R=L.getMonth()!=p,U=R&&!aa||!Z[0]||u&&L<u||r&&L>r;ga+='<td class="'+((I+o+6)%7>=5?" ui-datepicker-week-end":"")+(R?" ui-datepicker-other-month":"")+(L.getTime()==N.getTime()&&p==b.selectedMonth&&
b._keyEvent||ha.getTime()==L.getTime()&&ha.getTime()==N.getTime()?" "+this._dayOverClass:"")+(U?" "+this._unselectableClass+" ui-state-disabled":"")+(R&&!J?"":" "+Z[1]+(L.getTime()==w.getTime()?" "+this._currentClass:"")+(L.getTime()==c.getTime()?" ui-datepicker-today":""))+'"'+((!R||J)&&Z[2]?' title="'+Z[2]+'"':"")+(U?"":' onclick="DP_jQuery_'+j+".datepicker._selectDay('#"+b.id+"',"+L.getMonth()+","+L.getFullYear()+', this);return false;"')+">"+(R&&!J?"&#xa0;":U?'<span class="ui-state-default">'+
L.getDate()+"</span>":'<a class="ui-state-default'+(L.getTime()==c.getTime()?" ui-state-highlight":"")+(L.getTime()==w.getTime()?" ui-state-active":"")+(R?" ui-priority-secondary":"")+'" href="#">'+L.getDate()+"</a>")+"</td>";L.setDate(L.getDate()+1);L=this._daylightSavingAdjust(L)}T+=ga+"</tr>"}p++;if(p>11){p=0;y++}T+="</tbody></table>"+(x?"</div>"+(n[0]>0&&O==n[1]-1?'<div class="ui-datepicker-row-break"></div>':""):"");M+=T}da+=M}da+=k+(a.browser.msie&&parseInt(a.browser.version,10)<7&&!b.inline?
'<iframe src="javascript:false;" class="ui-datepicker-cover" frameborder="0"></iframe>':"");b._keyEvent=false;return da},_generateMonthYearHeader:function(b,c,f,k,m,o,n,p){var t=this._get(b,"changeMonth"),x=this._get(b,"changeYear"),w=this._get(b,"showMonthAfterYear"),u='<div class="ui-datepicker-title">',r="";if(o||!t)r+='<span class="ui-datepicker-month">'+n[c]+"</span>";else{n=k&&k.getFullYear()==f;var y=m&&m.getFullYear()==f;r+='<select class="ui-datepicker-month" onchange="DP_jQuery_'+j+".datepicker._selectMonthYear('#"+
b.id+"', this, 'M');\" onclick=\"DP_jQuery_"+j+".datepicker._clickMonthYear('#"+b.id+"');\">";for(var z=0;z<12;z++)if((!n||z>=k.getMonth())&&(!y||z<=m.getMonth()))r+='<option value="'+z+'"'+(z==c?' selected="selected"':"")+">"+p[z]+"</option>";r+="</select>"}w||(u+=r+(o||!(t&&x)?"&#xa0;":""));b.yearshtml="";if(o||!x)u+='<span class="ui-datepicker-year">'+f+"</span>";else{p=this._get(b,"yearRange").split(":");var B=(new Date).getFullYear();n=function(C){C=C.match(/c[+-].*/)?f+parseInt(C.substring(1),
10):C.match(/[+-].*/)?B+parseInt(C,10):parseInt(C,10);return isNaN(C)?B:C};c=n(p[0]);p=Math.max(c,n(p[1]||""));c=k?Math.max(c,k.getFullYear()):c;p=m?Math.min(p,m.getFullYear()):p;for(b.yearshtml+='<select class="ui-datepicker-year" onchange="DP_jQuery_'+j+".datepicker._selectMonthYear('#"+b.id+"', this, 'Y');\" onclick=\"DP_jQuery_"+j+".datepicker._clickMonthYear('#"+b.id+"');\">";c<=p;c++)b.yearshtml+='<option value="'+c+'"'+(c==f?' selected="selected"':"")+">"+c+"</option>";b.yearshtml+="</select>";
if(a.browser.mozilla)u+='<select class="ui-datepicker-year"><option value="'+f+'" selected="selected">'+f+"</option></select>";else{u+=b.yearshtml;b.yearshtml=null}}u+=this._get(b,"yearSuffix");if(w)u+=(o||!(t&&x)?"&#xa0;":"")+r;u+="</div>";return u},_adjustInstDate:function(b,c,f){var k=b.drawYear+(f=="Y"?c:0),m=b.drawMonth+(f=="M"?c:0);c=Math.min(b.selectedDay,this._getDaysInMonth(k,m))+(f=="D"?c:0);k=this._restrictMinMax(b,this._daylightSavingAdjust(new Date(k,m,c)));b.selectedDay=k.getDate();
b.drawMonth=b.selectedMonth=k.getMonth();b.drawYear=b.selectedYear=k.getFullYear();if(f=="M"||f=="Y")this._notifyChange(b)},_restrictMinMax:function(b,c){var f=this._getMinMaxDate(b,"min"),k=this._getMinMaxDate(b,"max");f=f&&c<f?f:c;return k&&f>k?k:f},_notifyChange:function(b){var c=this._get(b,"onChangeMonthYear");if(c)c.apply(b.input?b.input[0]:null,[b.selectedYear,b.selectedMonth+1,b])},_getNumberOfMonths:function(b){b=this._get(b,"numberOfMonths");return b==null?[1,1]:typeof b=="number"?[1,b]:
...
<<<<<<< TREE
jQuery.fn.fancyZoom=function(a){function e(n){if(f)return false;f=true;var p=$($(this).attr("href")),t=a.height,x=window.innerWidth||window.document.documentElement.clientWidth||window.document.body.clientWidth,w=window.innerHeight||window.document.documentElement.clientHeight||window.document.body.clientHeight,u=window.pageYOffset||window.document.documentElement.scrollTop||window.document.body.scrollTop,r={width:x,height:w,x:window.pageXOffset||window.document.documentElement.scrollLeft||window.document.body.scrollLeft,
y:u};x=(a.width||p.width())+60;w=(t||p.height())+60;t=Math.max(r.height/2-w/2+u,0);r=r.width/2-x/2;u=n.pageY;n=n.pageX;m.attr("curTop",u);m.attr("curLeft",n);m.attr("scaleImg",a.scaleImg?"true":"false");$("#zoom").hide().css({position:"absolute",top:u+"px",left:n+"px",width:"1px",height:"1px"});j();m.hide();a.closeOnClick&&$("#zoom").click(h);if(a.scaleImg){o.html(p.html());$("#zoom_content img").css("width","100%")}else o.html("");$("#zoom").animate({top:t+"px",left:r+"px",opacity:"show",width:x,
height:w},500,null,function(){a.scaleImg!=true&&o.html(p.html());b();m.show();f=false});return false}function h(){if(f)return false;f=true;$("#zoom").unbind("click");j();m.attr("scaleImg")!="true"&&o.html("");m.hide();$("#zoom").animate({top:m.attr("curTop")+"px",left:m.attr("curLeft")+"px",opacity:"hide",width:"1px",height:"1px"},500,null,function(){m.attr("scaleImg")=="true"&&o.html("");b();f=false});return false}function g(n){$("#zoom_table td").each(function(){var x=$(this).css("background-image").replace(/\.(png|gif|none)\"\)$/,
"."+n+'")');$(this).css("background-image",x)});var p=m.children("img"),t=p.attr("src").replace(/\.(png|gif|none)$/,"."+n);p.attr("src",t)}function j(){$.browser.msie&&parseFloat($.browser.version)>=7&&g("gif")}function b(){$.browser.msie&&$.browser.version>=7&&g("png")}var c=(a=a||{})&&a.directory?a.directory:"images",f=false;if($("#zoom").length==0){var k=$.browser.msie?"gif":"png";$("body").append('<div id="zoom" style="display:none;">                   <table id="zoom_table" style="border-collapse:collapse; width:100%; height:100%;">                     <tbody>                       <tr>                         <td class="tl" style="background:url('+
=======
...
<<<<<<< TREE
c+"/closebox."+k+'" alt="Close" style="border:none; margin:0; padding:0;" />                   </a>                 </div>');$("html").click(function(n){$(n.target).parents("#zoom:visible").length==0&&h()});$(document).keyup(function(n){n.keyCode==27&&$("#zoom:visible").length>0&&h()});$("#zoom_close").click(h)}$("#zoom");$("#zoom_table");var m=$("#zoom_close"),o=$("#zoom_content");$("td.ml,td.mm,td.mr");this.each(function(){$($(this).attr("href")).hide();$(this).click(e)});return this};
(function(a){a.ui=a.ui||{};var e=/left|center|right/,h=/top|center|bottom/,g=a.fn.position,j=a.fn.offset;a.fn.position=function(b){if(!b||!b.of)return g.apply(this,arguments);b=a.extend({},b);var c=a(b.of),f=c[0],k=(b.collision||"flip").split(" "),m=b.offset?b.offset.split(" "):[0,0],o,n,p;if(f.nodeType===9){o=c.width();n=c.height();p={top:0,left:0}}else if(f.setTimeout){o=c.width();n=c.height();p={top:c.scrollTop(),left:c.scrollLeft()}}else if(f.preventDefault){b.at="left top";o=n=0;p={top:b.of.pageY,
...
f>0?b.left-f:Math.max(b.left-c.collisionPosition.left,b.left)},top:function(b,c){var f=a(window);f=c.collisionPosition.top+c.collisionHeight-f.height()-f.scrollTop();b.top=f>0?b.top-f:Math.max(b.top-c.collisionPosition.top,b.top)}},flip:{left:function(b,c){if(c.at[0]!=="center"){var f=a(window);f=c.collisionPosition.left+c.collisionWidth-f.width()-f.scrollLeft();var k=c.my[0]==="left"?-c.elemWidth:c.my[0]==="right"?c.elemWidth:0,m=c.at[0]==="left"?c.targetWidth:-c.targetWidth,o=-2*c.offset[0];b.left+=
c.collisionPosition.left<0?k+m+o:f>0?k+m+o:0}},top:function(b,c){if(c.at[1]!=="center"){var f=a(window);f=c.collisionPosition.top+c.collisionHeight-f.height()-f.scrollTop();var k=c.my[1]==="top"?-c.elemHeight:c.my[1]==="bottom"?c.elemHeight:0,m=c.at[1]==="top"?c.targetHeight:-c.targetHeight,o=-2*c.offset[1];b.top+=c.collisionPosition.top<0?k+m+o:f>0?k+m+o:0}}}};if(!a.offset.setOffset){a.offset.setOffset=function(b,c){if(/static/.test(a.curCSS(b,"position")))b.style.position="relative";var f=a(b),
k=f.offset(),m=parseInt(a.curCSS(b,"top",true),10)||0,o=parseInt(a.curCSS(b,"left",true),10)||0;k={top:c.top-k.top+m,left:c.left-k.left+o};"using"in c?c.using.call(b,k):f.css(k)};a.fn.offset=function(b){var c=this[0];if(!c||!c.ownerDocument)return null;if(b)return this.each(function(){a.offset.setOffset(this,b)});return j.call(this)}}})(jQuery);
