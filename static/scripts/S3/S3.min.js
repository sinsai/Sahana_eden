...
setTimeout(function(){prep(img);},1);};setTimeout(function(){img.src=href;},1);}else if(href){$loadingBay.load(href,function(data,status,xhr){prep(status==='error'?'Request unsuccessful: '+xhr.statusText:$(this).children());});}};publicMethod.next=function(){if(!active){index=index<$related.length-1?index+1:0;publicMethod.load();}};publicMethod.prev=function(){if(!active){index=index?index-1:$related.length-1;publicMethod.load();}};publicMethod.close=function(){if(open&&!closing){closing=true;open=false;trigger(event_cleanup,settings.onCleanup);$window.unbind('.'+prefix+' .'+event_ie6);$overlay.fadeTo('fast',0);$box.stop().fadeTo('fast',0,function(){trigger(event_purge);$loaded.remove();$box.add($overlay).css({'opacity':1,cursor:'auto'}).hide();setTimeout(function(){closing=false;trigger(event_closed,settings.onClosed);},1);});}};publicMethod.element=function(){return $(element);};publicMethod.settings=defaults;$(publicMethod.init);}(jQuery,this));(function($,undefined){$.extend($.ui,{timepicker:{version:"0.0.1"}});var PROP_NAME='timepicker';var tpuuid=new Date().getTime();function Timepicker(){this.debug=true;this._curInst=null;this._isInline=false;this._disabledInputs=[];this._timepickerShowing=false;this._inDialog=false;this._dialogClass='ui-timepicker-dialog';this._mainDivId='ui-timepicker-div';this._inlineClass='ui-timepicker-inline';this._currentClass='ui-timepicker-current';this._dayOverClass='ui-timepicker-days-cell-over';this.regional=[];this.regional['']={hourText:'Hour',minuteText:'Minute',amPmText:['AM','PM']};this._defaults={showOn:'focus',showAnim:'fadeIn',showOptions:{},appendText:'',onSelect:null,onClose:null,timeSeparator:':',showPeriod:false,showLeadingZero:true,showMinutesLeadingZero:true,altField:'',defaultTime:''};$.extend(this._defaults,this.regional['']);this.tpDiv=$('<div id="'+this._mainDivId+'" class="ui-timepicker ui-widget ui-helper-clearfix ui-corner-all " style="display: none"></div>');}
...
this._get(inst,'hourText')+'</div>'+'<table class="ui-timepicker">';html+='<tr><th rowspan="2" class="periods">'+amPmText[0]+'</th>';for(h=0;h<=5;h++){html+=this._generateHTMLHourCell(inst,h,showPeriod,showLeadingZero);}
...
html+='</tr></table>'+'</td>'+'<td class="ui-timepicker-minutes">'+'<div class="ui-timepicker-title ui-widget-header ui-helper-clearfix ui-corner-all">'+
this._get(inst,'minuteText')+'</div>'+'<table class="ui-timepicker">'+'<tr>';for(m=0;m<15;m+=5){html+=this._generateHTMLMinuteCell(inst,m,(m<10)&&showMinutesLeadingZero?"0"+m.toString():m.toString());}
...
html+='</tr></table>'+'</td></tr></table>';return html;},_generateHTMLHourCell:function(inst,hour,showPeriod,showLeadingZero){var displayHour=hour;if((hour>12)&&showPeriod){displayHour=hour-12;}
...
displayHour.toString()+'</a></td>';return html;},_generateHTMLMinuteCell:function(inst,minute,displayText){var html='<td '+'onclick="TP_jQuery_'+tpuuid+'.timepicker.selectMinutes(\'#'+inst.id+'\', '+minute.toString()+', this ); return false;" '+'ondblclick="TP_jQuery_'+tpuuid+'.timepicker.selectMinutes(\'#'+inst.id+'\', '+minute.toString()+', this, true ); return false;" '+'>'+'<a href="#" class="ui-state-default '+
(minute==inst.minutes?'ui-state-active':'')+'" >'+
displayText+'</a></td>';return html;},_isDisabledTimepicker:function(target){if(!target){return false;}
...
var timeVal=inst.lastVal=timeToParse;var time=this.parseTime(inst,timeVal);inst.hours=time.hours;inst.minutes=time.minutes;},_getCurrentTimeRounded:function(inst){var currentTime=new Date();var timeSeparator=this._get(inst,'timeSeparator');var currentMinutes=currentTime.getMinutes()
...
return retVal;},selectHours:function(id,newHours,td,fromDoubleClick){var target=$(id);var inst=this._getInst(target[0]);$(td).parents('.ui-timepicker-hours:first').find('a').removeClass('ui-state-active');$(td).children('a').addClass('ui-state-active');inst.hours=newHours;this._updateSelectedValue(inst);inst._hoursClicked=true;if((inst._minutesClicked)||(fromDoubleClick)){$.timepicker._hideTimepicker();}},selectMinutes:function(id,newMinutes,td,fromDoubleClick){var target=$(id);var inst=this._getInst(target[0]);$(td).parents('.ui-timepicker-minutes:first').find('a').removeClass('ui-state-active');$(td).children('a').addClass('ui-state-active');inst.minutes=newMinutes;this._updateSelectedValue(inst);inst._minutesClicked=true;if((inst._hoursClicked)||(fromDoubleClick)){$.timepicker._hideTimepicker();}},_updateSelectedValue:function(inst){if((inst.hours<0)||(inst.hours>23)){inst.hours=12;}
...
return $.timepicker['_'+options+'Datepicker'].apply($.timepicker,[this[0]].concat(otherArgs));return this.each(function(){typeof options=='string'?$.timepicker['_'+options+'Datepicker'].apply($.timepicker,[this].concat(otherArgs)):$.timepicker._attachTimepicker(this,options);});};function extendRemove(target,props){$.extend(target,props);for(var name in props)
if(props[name]==null||props[name]==undefined)
target[name]=props[name];return target;};$.timepicker=new Timepicker();$.timepicker.initialized=false;$.timepicker.uuid=new Date().getTime();$.timepicker.version="1.8.6";window['TP_jQuery_'+tpuuid]=$;})(jQuery);;(function($){$.cluetip={version:'1.0.7'};var $cluetip,$cluetipInner,$cluetipOuter,$cluetipTitle,$cluetipArrows,$cluetipWait,$dropShadow,imgCount;$.fn.cluetip=function(js,options){if(typeof js=='object'){options=js;js=null;}
...
if(options&&options.insertionElement){insertionElement=options.insertionElement;}};})(jQuery);if(jQuery)(function($){function renderOption(id,option)
...
var s3_gis_pullNext;function s3_gis_dropdown_select(level,force){s3_gis_pullNext=false;$('#gis_location_lat').val('');$('#gis_location_lon').val('');$('#gis_location_addr_street').val('');$('#gis_location_postcode').val('');var new_id=$('#gis_location_L'+level).val();if(new_id||force){if(level==s3_gis_maxlevel){var this_url=s3_gis_url+'/search.json?filter=%3D&field=level&value=nullnone&parent='+new_id;}else{var this_url=s3_gis_url+'/search.json?filter=%3D&field=level&value=L'+(level+1)+'&parent='+new_id;}
var s3_gis_load_locations=function(data,status){var options;var v='';if(data.length==0){options=s3_gis_empty_set;}else if(data.length==1){options='<option value="'+data[0].id+'">'+data[0].name+'</option>';s3_gis_pullNext=true;}else{options=s3_gis_select_location;for(var i=0;i<data.length;i++){v=data[i].id;options+='<option value="'+data[i].id+'">'+data[i].name+'</option>';}}
if(level==s3_gis_maxlevel){$('#gis_location_').html(options);}else{$('#gis_location_L'+(level+1)).html(options);}}
$.getJSONS3(this_url,s3_gis_load_locations,false,true);if(level==s3_gis_maxlevel){if(options==s3_gis_empty_set){}else{$('#gis_location_').removeClass('hidden').show();$('#gis_location_label_').removeClass('hidden').show();$('#gis_location_details-btn').removeClass('hidden').show();}}else{$('#gis_location_L'+(level+1)).removeClass('hidden').show();$('#gis_location_label_L'+(level+1)).removeClass('hidden').show();$('#gis_location_details-btn').removeClass('hidden').show();}
s3_gis_dropdown_hide(level+2);if(s3_gis_pullNext){new_id=$('#gis_location_L'+(level+1)).val();$('#'+s3_gis_location_id).val(new_id);s3_gis_dropdown_select(level+1);}else{if(''==$('#gis_location_name').val()){$('#'+s3_gis_location_id).val(new_id);}}}else{s3_gis_dropdown_hide(level+1);if((0==level)&&(''==$('#gis_location_name').val())){$('#'+s3_gis_location_id).val('');}}}
function s3_gis_dropdown_hide(level){for(l=level;l<=parseInt(s3_gis_maxlevel);l=l+1){$('#gis_location_L'+l).hide().html(s3_gis_loading_locations);$('#gis_location_label_L'+l).hide();}
if(level<(parseInt(s3_gis_maxlevel)+2)){$('#gis_location_').hide().html(s3_gis_loading_locations);$('#gis_location_label_').hide();$('#gis_location_details-btn').hide();}}
function s3_gis_save_location(name,lat,lon,addr_street,addr_postcode){var _parent=$('#gis_location_L5').val();if(undefined==_parent||''==_parent){_parent=$('#gis_location_L4').val();if(undefined==_parent||''==_parent){_parent=$('#gis_location_L3').val();if(undefined==_parent||''==_parent){_parent=$('#gis_location_L2').val();if(undefined==_parent||''==_parent){_parent=$('#gis_location_L1').val();if(undefined==_parent||''==_parent){_parent=$('#gis_location_L0').val();}}}}}
var url;if(''==S3.gis.uuid){url=s3_gis_url+'/create.url?name='+name;}else{url=s3_gis_url+'/update.url?uuid='+S3.gis.uuid+'&name='+name;}
if(''==lat||''==lon){}else{url=url+'&lat='+lat+'&lon='+lon;}
if(''==addr_street){}else{url=url+'&addr_street='+addr_street.replace(/\n/g,'%0d');}
if(''==addr_postcode){}else{url=url+'&addr_postcode='+addr_postcode;}
if(undefined==_parent||''==_parent){}else{url=url+'&parent='+_parent;}
$.ajax({async:false,url:url,dataType:'json',success:function(data){if((''==S3.gis.uuid)&&(data.status=='success')){var new_id=data.message.split('=')[1];$('#'+s3_gis_location_id).val(new_id);}}});}
function s3_gis_geolocate(position){var lat=position.coords.latitude;var lon=position.coords.longitude;$('#gis_location_lat').val(lat);$('#gis_location_lon').val(lon);}
var s3_gis_calcDone=false;var s3_gis_calcAnswer;function s3_gis_convertDD(){var deg=$('#DDMMSS_deg').val();var min=$('#DDMMSS_min').val();var sec=$('#DDMMSS_sec').val();var invalid=false;if(deg==''){deg=0;}else if(isNaN(deg)||deg>180||deg<-180){alert(s3_gis_degrees_validation_error);invalid=true;}
if(min==''){min=0;}else if(isNaN(min)||min>60||min<0){alert(s3_gis_minutes_validation_error);invalid=true;}
if(sec==''){sec=0;}else if(isNaN(sec)||sec>60||sec<0){alert(s3_gis_seconds_validation_error);invalid=true;}
if(invalid==false){var dec_min=(min*1.0+(sec/60.0));if(deg>0){s3_gis_calcAnswer=deg*1.0+(dec_min/60.0);}else{s3_gis_calcAnswer=deg*1.0-(dec_min/60.0);}
$('#DDMMSS_dec').val(s3_gis_calcAnswer);s3_gis_calcDone=true;}}
function s3_gis_convertGps(){var deg=0;var min=0;deg=$('#gps_deg').val();min=$('#gps_min').val();var invalid=false;if(deg==''){deg=0;}else if(isNaN(deg)||deg>180||deg<-180){alert(s3_gis_degrees_validation_error);invalid=true;}
if(min==''){min=0;}else if(isNaN(min)||min>60||min<0){alert(s3_gis_minutes_validation_error);invalid=true;}
if(invalid==false){if(deg>0){s3_gis_calcAnswer=(deg*1.0)+min/60.0;}else{s3_gis_calcAnswer=(deg*1.0)-min/60.0;}
$('#gps_dec').val(s3_gis_calcAnswer);s3_gis_calcDone=true;}}
function s3_gis_convertFillBack(whereto){if(s3_gis_calcDone==true){if(whereto==1){$('#gis_location_lat').attr('value',s3_gis_calcAnswer);}else{$('#gis_location_lon').attr('value',s3_gis_calcAnswer);}
return true;}else{alert(s3_gis_no_calculations_error);}
return false;}
$(function(){if(typeof(s3_gis_location_id)=="undefined"){}else{$('#gis_location_L0').change(function(){s3_gis_dropdown_select(0);});$('#gis_location_L1').change(function(){s3_gis_dropdown_select(1);});$('#gis_location_L2').change(function(){s3_gis_dropdown_select(2);});$('#gis_location_L3').change(function(){s3_gis_dropdown_select(3);});$('#gis_location_L4').change(function(){s3_gis_dropdown_select(4);});$('#gis_location_').change(function(){var new_id=$(this).val();if(''==$('#gis_location_name').val()){$('#'+s3_gis_location_id).val(new_id);}});$('#gis_location_add-btn').click(function(evt){if(''!=S3.gis.uuid){$('body').data('uuid',S3.gis.uuid);S3.gis.uuid='';}
var lat=$('#gis_location_lat').val();var lon=$('#gis_location_lat').val();if((''!=lat)||(''!=lon)){$('body').data('lat',lat);$('body').data('lon',lon);$('#gis_location_lat').val('');$('#gis_location_lon').val('');}
$('#gis_location_add-btn').hide();$('#gis_location_cancel-btn').removeClass('hidden').show();$('#gis_location_').hide();$('#gis_location_label_').hide();if(navigator.geolocation){$('#gis_location_geolocate-btn').removeClass('hidden').show();}else{}
$('#gis_location_map-btn').removeClass('hidden').show();$('#gis_location_name_label').removeClass('hidden').show();$('#gis_location_name').removeClass('hidden').show();$('#gis_location_addr_street_label').removeClass('hidden').show();$('#gis_location_addr_street_row').removeClass('hidden').show();$('#gis_location_postcode_label').removeClass('hidden').show();$('#gis_location_postcode_row').removeClass('hidden').show();$('#gis_location_advanced_div').removeClass('hidden').show();evt.preventDefault();});$('#gis_location_advanced_checkbox').change(function(evt){if($('#gis_location_advanced_checkbox').is(':checked')){$('#gis_location_lat_label').removeClass('hidden').show();$('#gis_location_lat_row').removeClass('hidden').show();$('#gis_location_lon_label').removeClass('hidden').show();$('#gis_location_lon_row').removeClass('hidden').show();}else{$('#gis_location_lat_label').hide();$('#gis_location_lat_row').hide();$('#gis_location_lon_label').hide();$('#gis_location_lon_row').hide();}
evt.preventDefault();});$('#gis_location_search-btn').click(function(evt){$(this).hide();$('#gis_location_autocomplete_div').removeClass('hidden').show();evt.preventDefault();});$('#gis_location_details-btn').click(function(evt){$('#gis_location_map-btn').removeClass('hidden').show();$('#gis_location_addr_street_label').removeClass('hidden').show();$('#gis_location_addr_street_row').removeClass('hidden').show();$('#gis_location_postcode_label').removeClass('hidden').show();$('#gis_location_postcode_row').removeClass('hidden').show();$('#gis_location_advanced_div').removeClass('hidden').show();evt.preventDefault();});$('#gis_location_cancel-btn').click(function(evt){S3.gis.uuid=$('body').data('uuid');$('#gis_location_lat').val($('body').data('lat'));$('#gis_location_lon').val($('body').data('lon'));$('#gis_location_cancel-btn').hide();$('#gis_location_name').val('');$('#gis_location_add-btn').show();if(""!=$('#gis_location_').val()){$('#gis_location_').show();$('#gis_location_label_').show();$('#gis_location_details-btn').show();}
$('#gis_location_map-btn').hide();$('#gis_location_name_label').hide();$('#gis_location_name').hide();$('#gis_location_addr_street_label').hide();$('#gis_location_addr_street_row').hide();$('#gis_location_postcode_label').hide();$('#gis_location_postcode_row').hide();$('#gis_location_advanced_div').hide();evt.preventDefault();});$('#gis_location_geolocate-btn').click(function(evt){navigator.geolocation.getCurrentPosition(s3_gis_geolocate);evt.preventDefault();});var s3_gis_ac_data={val:$('#gis_location_autocomplete').val(),accept:false};$('#gis_location_autocomplete').autocomplete({source:s3_gis_url+'/search.json?filter=~&field=name&exclude_field=level&exclude_value=XX',minLength:2,focus:function(event,ui){$('#gis_location_autocomplete').val(ui.item.name);return false;},select:function(event,ui){$('#gis_location_autocomplete').val(ui.item.name);$('#'+s3_gis_location_id).val(ui.item.id);s3_gis_ac_data.accept=true;var s3_gis_path=ui.item.path;var s3_gis_id=ui.item.id;var s3_gis_l1,s3_gis_l2,s3_gis_l3,s3_gis_l4,s3_gis_l5;var s3_gis_missing=[];switch(ui.item.level){case'L0':$('#gis_location_L0').val(s3_gis_id);$('#'+s3_gis_location_id).val(s3_gis_id);s3_gis_dropdown_select(0);break;case'L1':if(s3_gis_path){s3_gis_path=s3_gis_path.split('/');if(s3_gis_path.length==2&&s3_gis_path[1]==s3_gis_id){$('#gis_location_L0').val(s3_gis_path[0]);}else{s3_gis_dropdown_select(0,true);$('#gis_location_L1').val(s3_gis_id);$('#'+s3_gis_location_id).val(s3_gis_id);break;}}
s3_gis_dropdown_select(0,true);$('#gis_location_L1').val(s3_gis_id);$('#'+s3_gis_location_id).val(s3_gis_id);s3_gis_dropdown_select(1);break;case'L2':if(s3_gis_path){s3_gis_path=s3_gis_path.split('/');if(s3_gis_path.length==3&&s3_gis_path[2]==s3_gis_id){$('#gis_location_L0').val(s3_gis_path[0]);s3_gis_l1=s3_gis_path[1];}else if(s3_gis_path.length==2&&s3_gis_path[1]==s3_gis_id){if(undefined!=$('#gis_location_L0 option[value='+s3_gis_path[0]+']')[0]){$('#gis_location_L0').val(s3_gis_path[0]);}else{s3_gis_l1=s3_gis_path[0];}}else{s3_gis_dropdown_select(1,true);$('#gis_location_L2').val(s3_gis_id);$('#'+s3_gis_location_id).val(s3_gis_id);break;}}
s3_gis_dropdown_select(0,true);$('#gis_location_L1').val(s3_gis_l1);s3_gis_dropdown_select(1,true);$('#gis_location_L2').val(s3_gis_id);$('#'+s3_gis_location_id).val(s3_gis_id);s3_gis_dropdown_select(2);break;case'L3':if(s3_gis_path){s3_gis_path=s3_gis_path.split('/');if(s3_gis_path.length==4&&s3_gis_path[3]==s3_gis_id){$('#gis_location_L0').val(s3_gis_path[0]);s3_gis_l1=s3_gis_path[1];s3_gis_l2=s3_gis_path[2];}else if(s3_gis_path.length==3&&s3_gis_path[2]==s3_gis_id){if(undefined!=$('#gis_location_L0 option[value='+s3_gis_path[0]+']')[0]){$('#gis_location_L0').val(s3_gis_path[0]);s3_gis_missing=[s3_gis_path[1]];}else{s3_gis_l1=s3_gis_path[0];s3_gis_l2=s3_gis_path[1];}}else if(s3_gis_path.length==2&&s3_gis_path[1]==s3_gis_id){if(undefined!=$('#gis_location_L0 option[value='+s3_gis_path[0]+']')[0]){$('#gis_location_L0').val(s3_gis_path[0]);}else{s3_gis_missing=[s3_gis_path[0]];}}else{s3_gis_dropdown_select(2,true);$('#gis_location_L3').val(s3_gis_id);$('#'+s3_gis_location_id).val(s3_gis_id);break;}}
s3_gis_dropdown_select(0,true);if(s3_gis_missing[0]){if(undefined!=$('#gis_location_L1 option[value='+s3_gis_missing[0]+']')[0]){s3_gis_l1=s3_gis_missing[0];}}
$('#gis_location_L1').val(s3_gis_l1);s3_gis_dropdown_select(1,true);if(s3_gis_missing[0]){if(undefined!=$('#gis_location_L2 option[value='+s3_gis_missing[0]+']')[0]){s3_gis_l2=s3_gis_missing[0];}}
$('#gis_location_L2').val(s3_gis_l2);s3_gis_dropdown_select(2,true);$('#gis_location_L3').val(s3_gis_id);$('#'+s3_gis_location_id).val(s3_gis_id);s3_gis_dropdown_select(3);break;case'L4':if(s3_gis_path){s3_gis_path=s3_gis_path.split('/');if(s3_gis_path.length==5&&s3_gis_path[4]==s3_gis_id){$('#gis_location_L0').val(s3_gis_path[0]);s3_gis_l1=s3_gis_path[1];s3_gis_l2=s3_gis_path[2];s3_gis_l3=s3_gis_path[3];}else if(s3_gis_path.length==4&&s3_gis_path[3]==s3_gis_id){if(undefined!=$('#gis_location_L0 option[value='+s3_gis_path[0]+']')[0]){$('#gis_location_L0').val(s3_gis_path[0]);s3_gis_missing=[s3_gis_path[1],s3_gis_path[2]];}else{s3_gis_missing=[s3_gis_path[0],s3_gis_path[1],s3_gis_path[2]];}}else if(s3_gis_path.length==3&&s3_gis_path[2]==s3_gis_id){if(undefined!=$('#gis_location_L0 option[value='+s3_gis_path[0]+']')[0]){$('#gis_location_L0').val(s3_gis_path[0]);s3_gis_missing=[s3_gis_path[1]];}else{s3_gis_missing=[s3_gis_path[0],s3_gis_path[1]];}}else if(s3_gis_path.length==2&&s3_gis_path[1]==s3_gis_id){if(undefined!=$('#gis_location_L0 option[value='+s3_gis_path[0]+']')[0]){$('#gis_location_L0').val(s3_gis_path[0]);}else{s3_gis_missing=[s3_gis_path[0]];}}else{s3_gis_dropdown_select(3,true);$('#gis_location_L4').val(s3_gis_id);$('#'+s3_gis_location_id).val(s3_gis_id);break;}}
s3_gis_dropdown_select(0,true);if(s3_gis_missing[0]){if(undefined!=$('#gis_location_L1 option[value='+s3_gis_missing[0]+']')[0]){s3_gis_l1=s3_gis_missing[0];}}
$('#gis_location_L1').val(s3_gis_l1);s3_gis_dropdown_select(1,true);if(s3_gis_missing[0]){if(undefined!=$('#gis_location_L2 option[value='+s3_gis_missing[0]+']')[0]){s3_gis_l2=s3_gis_missing[0];}else if(s3_gis_missing[1]){if(undefined!=$('#gis_location_L2 option[value='+s3_gis_missing[1]+']')[0]){s3_gis_l2=s3_gis_missing[1];}}}
$('#gis_location_L2').val(s3_gis_l2);s3_gis_dropdown_select(2,true);if(s3_gis_missing[0]){if(undefined!=$('#gis_location_L3 option[value='+s3_gis_missing[0]+']')[0]){s3_gis_l3=s3_gis_missing[0];}else if(s3_gis_missing[1]){if(undefined!=$('#gis_location_L3 option[value='+s3_gis_missing[1]+']')[0]){s3_gis_l3=s3_gis_missing[1];}else if(s3_gis_missing[2]){if(undefined!=$('#gis_location_L3 option[value='+s3_gis_missing[2]+']')[0]){s3_gis_l3=s3_gis_missing[2];}}}}
$('#gis_location_L3').val(s3_gis_l3);s3_gis_dropdown_select(3,true);$('#gis_location_L4').val(s3_gis_id);$('#'+s3_gis_location_id).val(s3_gis_id);s3_gis_dropdown_select(4);break;case'L5':break;default:if(s3_gis_path){s3_gis_path=s3_gis_path.split('/');if(s3_gis_path.length==6&&s3_gis_path[5]==s3_gis_id){$('#gis_location_L0').val(s3_gis_path[0]);s3_gis_l1=s3_gis_path[1];s3_gis_l2=s3_gis_path[2];s3_gis_l3=s3_gis_path[3];s3_gis_l3=s3_gis_path[4];}else if(s3_gis_path.length==5&&s3_gis_path[4]==s3_gis_id){if(undefined!=$('#gis_location_L0 option[value='+s3_gis_path[0]+']')[0]){$('#gis_location_L0').val(s3_gis_path[0]);s3_gis_missing=[s3_gis_path[1],s3_gis_path[2],s3_gis_path[3]];}else{s3_gis_missing=[s3_gis_path[0],s3_gis_path[1],s3_gis_path[2],s3_gis_path[3]];}}else if(s3_gis_path.length==4&&s3_gis_path[3]==s3_gis_id){if(undefined!=$('#gis_location_L0 option[value='+s3_gis_path[0]+']')[0]){$('#gis_location_L0').val(s3_gis_path[0]);s3_gis_missing=[s3_gis_path[1],s3_gis_path[2]];}else{s3_gis_missing=[s3_gis_path[0],s3_gis_path[1],s3_gis_path[2]];}}else if(s3_gis_path.length==3&&s3_gis_path[2]==s3_gis_id){if(undefined!=$('#gis_location_L0 option[value='+s3_gis_path[0]+']')[0]){$('#gis_location_L0').val(s3_gis_path[0]);s3_gis_missing=[s3_gis_path[1]];}else{s3_gis_missing=[s3_gis_path[0],s3_gis_path[1]];}}else if(s3_gis_path.length==2&&s3_gis_path[1]==s3_gis_id){if(undefined!=$('#gis_location_L0 option[value='+s3_gis_path[0]+']')[0]){$('#gis_location_L0').val(s3_gis_path[0]);}else{s3_gis_missing=[s3_gis_path[0]];}}else{s3_gis_dropdown_select(4,true);$('#gis_location_').val(s3_gis_id);$('#'+s3_gis_location_id).val(s3_gis_id);break;}
s3_gis_dropdown_select(0,true);if(s3_gis_missing[0]){if(undefined!=$('#gis_location_L1 option[value='+s3_gis_missing[0]+']')[0]){s3_gis_l1=s3_gis_missing[0];}}
$('#gis_location_L1').val(s3_gis_l1);s3_gis_dropdown_select(1,true);if(s3_gis_missing[0]){if(undefined!=$('#gis_location_L2 option[value='+s3_gis_missing[0]+']')[0]){s3_gis_l2=s3_gis_missing[0];}else if(s3_gis_missing[1]){if(undefined!=$('#gis_location_L2 option[value='+s3_gis_missing[1]+']')[0]){s3_gis_l2=s3_gis_missing[1];}}}
$('#gis_location_L2').val(s3_gis_l2);s3_gis_dropdown_select(2,true);if(s3_gis_missing[0]){if(undefined!=$('#gis_location_L3 option[value='+s3_gis_missing[0]+']')[0]){s3_gis_l3=s3_gis_missing[0];}else if(s3_gis_missing[1]){if(undefined!=$('#gis_location_L3 option[value='+s3_gis_missing[1]+']')[0]){s3_gis_l3=s3_gis_missing[1];}else if(s3_gis_missing[2]){if(undefined!=$('#gis_location_L3 option[value='+s3_gis_missing[2]+']')[0]){s3_gis_l3=s3_gis_missing[2];}}}}
$('#gis_location_L3').val(s3_gis_l3);s3_gis_dropdown_select(3);if(s3_gis_missing[0]){if(undefined!=$('#gis_location_L4 option[value='+s3_gis_missing[0]+']')[0]){s3_gis_l4=s3_gis_missing[0];}else if(s3_gis_missing[1]){if(undefined!=$('#gis_location_L4 option[value='+s3_gis_missing[1]+']')[0]){s3_gis_l4=s3_gis_missing[1];}else if(s3_gis_missing[2]){if(undefined!=$('#gis_location_L4 option[value='+s3_gis_missing[2]+']')[0]){s3_gis_l4=s3_gis_missing[2];}else if(s3_gis_missing[3]){if(undefined!=$('#gis_location_L4 option[value='+s3_gis_missing[4]+']')[0]){s3_gis_l4=s3_gis_missing[3];}}}}}
$('#gis_location_L4').val(s3_gis_l4);s3_gis_dropdown_select(4,true);$('#gis_location_').val(s3_gis_id);}
$('#'+s3_gis_location_id).val(s3_gis_id);}
$('#gis_location_autocomplete_div').hide();$('#gis_location_search-btn').show();return false;}}).data('autocomplete')._renderItem=function(ul,item){return $('<li></li>').data('item.autocomplete',item).append('<a>'+item.name+'</a>').appendTo(ul);};$('#gis_location_autocomplete').blur(function(){if(!$('#gis_location_autocomplete').val()){$('#'+s3_gis_location_id).val('');s3_gis_ac_data.accept=true;}
if(!s3_gis_ac_data.accept){$('#gis_location_autocomplete').val(s3_gis_ac_data.val);}else{s3_gis_ac_data.val=$('#gis_location_autocomplete').val();}
s3_gis_ac_data.accept=false;});$('form').submit(function(){S3ClearNavigateAwayConfirm();var name=$('#gis_location_name').val();var lat=$('#gis_location_lat').val();var lon=$('#gis_location_lon').val();var addr_street=$('#gis_location_addr_street').val();var addr_postcode=$('#gis_location_postcode').val();if(''==name){if((''==lat||''==lon)&&(''==addr_street)&&(''==addr_postcode)){return true;}else{name=$('#gis_location_ :selected').text();if(s3_gis_loading_locations.search(name)){$('#gis_location_name').after('<div id="type__error" class="error" style="display: block;">Name field is required!</div>');$('#gis_location_name').focus();if(s3_navigate_away_confirm){S3SetNavigateAwayConfirm()}
return false;}else{if((S3.gis.lat==$('#gis_location_lat').val())&&(S3.gis.lon==$('#gis_location_lon').val())&&(S3.gis.addr_street==$('#gis_location_addr_street').val().replace(/\n/g,'%0d'))&&(S3.gis.postcode==$('#gis_location_postcode').val())){}else{s3_gis_save_location(name,lat,lon,addr_street,addr_postcode);}
return true;}}}
s3_gis_save_location(name,lat,lon,addr_street,addr_postcode);return true;});}});Ext.onReady(function(){var converterWin;var converterButton=Ext.get('gis_location_converter-btn');var mapButton=Ext.get('gis_location_map-btn');if(mapButton){mapButton.on('click',function(){mapWin.show(this);});}
if(converterButton){converterButton.on('click',function(){if(!converterWin){converterWin=new Ext.Window({applyTo:'gis-convert-win',layout:'fit',width:400,height:250,closeAction:'hide',plain:true,items:new Ext.TabPanel({applyTo:'gis-convert-tabs',autoTabs:true,activeTab:0,deferredRender:false,border:false}),buttons:[{text:s3_gis_fill_lat,disabled:false,handler:function(){if(s3_gis_convertFillBack(1)){s3_gis_calcDone=false;$('#DDMMSS_dec').val('');$('#gps_dec').val('');}}},{text:s3_gis_fill_lon,handler:function(){if(s3_gis_convertFillBack(2)){s3_gis_calcDone=false;$('#DDMMSS_dec').val('');$('#gps_dec').val('');}}}]});}
converterWin.show(this);});}});var S3MultiselectWidgetFileCounter=0;function S3MultiselectWidgetAddClick(Args)
...
$("#"+WidgetID+"_json").val(JSONInputValue);};(function($,undefined){$.extend($.ui,{datepicker:{version:"1.8.9"}});var PROP_NAME='datepicker';var dpuuid=new Date().getTime();function Datepicker(){this.debug=false;this._curInst=null;this._keyEvent=false;this._disabledInputs=[];this._datepickerShowing=false;this._inDialog=false;this._mainDivId='ui-datepicker-div';this._inlineClass='ui-datepicker-inline';this._appendClass='ui-datepicker-append';this._triggerClass='ui-datepicker-trigger';this._dialogClass='ui-datepicker-dialog';this._disableClass='ui-datepicker-disabled';this._unselectableClass='ui-datepicker-unselectable';this._currentClass='ui-datepicker-current-day';this._dayOverClass='ui-datepicker-days-cell-over';this.regional=[];this.regional['']={closeText:'Done',prevText:'Prev',nextText:'Next',currentText:'Today',monthNames:['January','February','March','April','May','June','July','August','September','October','November','December'],monthNamesShort:['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'],dayNames:['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'],dayNamesShort:['Sun','Mon','Tue','Wed','Thu','Fri','Sat'],dayNamesMin:['Su','Mo','Tu','We','Th','Fr','Sa'],weekHeader:'Wk',dateFormat:'mm/dd/yy',firstDay:0,isRTL:false,showMonthAfterYear:false,yearSuffix:''};this._defaults={showOn:'focus',showAnim:'fadeIn',showOptions:{},defaultDate:null,appendText:'',buttonText:'...',buttonImage:'',buttonImageOnly:false,hideIfNoPrevNext:false,navigationAsDateFormat:false,gotoCurrent:false,changeMonth:false,changeYear:false,yearRange:'c-10:c+10',showOtherMonths:false,selectOtherMonths:false,showWeek:false,calculateWeek:this.iso8601Week,shortYearCutoff:'+10',minDate:null,maxDate:null,duration:'fast',beforeShowDay:null,beforeShow:null,onSelect:null,onChangeMonthYear:null,onClose:null,numberOfMonths:1,showCurrentAtPos:0,stepMonths:1,stepBigMonths:12,altField:'',altFormat:'',constrainInput:true,showButtonPanel:false,autoSize:false};$.extend(this._defaults,this.regional['']);this.dpDiv=$('<div id="'+this._mainDivId+'" class="ui-datepicker ui-widget ui-widget-content ui-helper-clearfix ui-corner-all"></div>');}
...
return $.datepicker['_'+options+'Datepicker'].apply($.datepicker,[this[0]].concat(otherArgs));return this.each(function(){typeof options=='string'?$.datepicker['_'+options+'Datepicker'].apply($.datepicker,[this].concat(otherArgs)):$.datepicker._attachDatepicker(this,options);});};$.datepicker=new Datepicker();$.datepicker.initialized=false;$.datepicker.uuid=new Date().getTime();$.datepicker.version="1.8.9";window['DP_jQuery_'+dpuuid]=$;})(jQuery);(function($,undefined){if($.cleanData){var _cleanData=$.cleanData;$.cleanData=function(elems){for(var i=0,elem;(elem=elems[i])!=null;i++){$(elem).triggerHandler("remove");}
...
{return jQuery(this).length>0;};})(jQuery);(function($)
{$.fn.formElementType=function()
{if(jQuery(this).elementExists())
return jQuery(this).attr("type");else
return"";};})(jQuery);(function($)
{$.fn.isTextBox=function()
{return(jQuery(this).formElementType()==="text");};})(jQuery);(function($)
{$.fn.isTextArea=function()
{return(jQuery(this).formElementType()==="textarea");};})(jQuery);(function($)
{$.fn.isPassword=function()
{return(jQuery(this).formElementType()==="password");};})(jQuery);(function($)
{$.fn.isHiddenInput=function()
{return(jQuery(this).formElementType()==="hidden");};})(jQuery);(function($)
{$.fn.isCheckBox=function()
{return(jQuery(this).formElementType()==="checkbox");};})(jQuery);(function($)
{$.fn.isRadioBox=function()
{return(jQuery(this).formElementType()==="radio");};})(jQuery);(function($)
{$.fn.isButton=function()
{return(jQuery(this).formElementType()==="button");};})(jQuery);(function($)
{$.fn.isSubmitButton=function()
{return(jQuery(this).formElementType()==="submit");};})(jQuery);(function($)
{$.fn.isResetButton=function()
{return(jQuery(this).formElementType()==="reset");};})(jQuery);(function($)
{$.fn.isSelectBox=function()
{var type=jQuery(this).formElementType();var size=(this).attr("size");if(type!=="select-one")
{return false;}
else
{if(typeof size==="undefined")
return false;else
return(parseInt(size)>1);}};})(jQuery);(function($)
{$.fn.isMultiSelectBox=function()
{return(jQuery(this).formElementType()==="select-multiple");};})(jQuery);(function($)
...
this.activate(event,result);}else{this.activate(event,this.element.children(".ui-menu-item").filter(!this.active||this.first()?":last":":first"));}},hasScroll:function(){return this.element.height()<this.element.attr("scrollHeight");},select:function(event){this._trigger("selected",event,{item:this.active});}});}(jQuery));$(document).ready(function(){if(S3.rtl){$('div.label label[id!="delete_record__label"]').prepend(':');}else{$('div.label label[id!="delete_record__label"]').append(':');}});var S3=Object();S3.gis=Object();var popupWin=null;function openPopup(url){if(!popupWin||popupWin.closed){popupWin=window.open(url,'popupWin','width=640, height=480');}else popupWin.focus();}
$(document).ready(function(){try{$('.zoom').fancyZoom({scaleImg:true,closeOnClick:true,directory:S3.Ap.concat("/static/media")});}catch(e){};$('input.date').datepicker({changeMonth:true,changeYear:true,showOn:'both',buttonImage:S3.Ap.concat('/static/img/jquery-ui/calendar.gif'),buttonImageOnly:true,dateFormat:'yy-mm-dd',isRTL:S3.rtl});$('.error').hide().slideDown('slow')
...
$('.confirmation').click(function(){$(this).fadeOut('slow');return false;});$('.hoverable').hover(function(){$(this).addClass('hovered');},function(){$(this).removeClass('hovered');});$('#modulenav li').hover(function(){var header_width=$(this).width();var popup_width=$('ul',this).width();if(popup_width!=null){if(popup_width<header_width){$('ul',this).css({'width':header_width.toString()+'px'});}}
$('ul',this).css('display','block');},function(){$('ul',this).css('display','none');});$('#subnav li').hover(function(){var popup_width=$(this).width()-2;$('ul',this).css({'display':'block','width':popup_width.toString()+'px'});},function(){$('ul',this).css('display','none');});$('a.colorbox').attr('href',function(index,attr){var caller='';try{caller=$(this).parents('tr').attr('id').replace(/__row/,'');}catch(e){if(caller=='')return attr;}
var url_out=attr;if(attr.indexOf('&caller=')==-1){url_out=attr+'&caller='+caller;}
return url_out;});$('.colorbox').click(function(){$.fn.colorbox({iframe:true,width:'99%',height:'99%',href:this.href,title:this.title});return false;});$('.tooltip').cluetip({activation:'hover',sticky:false,splitTitle:'|'});var tipCloseText='<img src="'+S3.Ap.concat('/static/img/cross2.png')+'" alt="close" />';$('.stickytip').cluetip({activation:'hover',sticky:true,closePosition:'title',closeText:tipCloseText,splitTitle:'|'});$('.ajaxtip').cluetip({activation:'click',sticky:true,closePosition:'title',closeText:tipCloseText,width:380});now=new Date();$('form').append("<input type='hidden' value="+now.getTimezoneOffset()+" name='_utc_offset'/>");});function s3_tb_remove(){$.fn.colorbox.close();}
(function($){$.ajaxS3=function(s){var options=$.extend({},$.ajaxS3Settings,s);options.tryCount=0;if(s.message){s3_showStatus(S3.i18n.ajax_get+' '+(s.message?s.message:S3.i18n.ajax_fmd)+'...',this.ajaxS3Settings.msgTimeout);}
options.success=function(data,status){s3_hideStatus();if(s.success)
s.success(data,status);}
options.error=function(xhr,textStatus,errorThrown){if(textStatus=='timeout'){this.tryCount++;if(this.tryCount<=this.retryLimit){s3_showStatus(S3.i18n.ajax_get+' '+(s.message?s.message:S3.i18n.ajax_fmd)+'... '+S3.i18n.ajax_rtr+' '+this.tryCount,$.ajaxS3Settings.msgTimeout);$.ajax(this);return;}
s3_showStatus(S3.i18n.ajax_wht+' '+(this.retryLimit+1)+' '+S3.i18n.ajax_gvn,$.ajaxS3Settings.msgTimeout,false,true);return;}
if(xhr.status==500){s3_showStatus(S3.i18n.ajax_500,$.ajaxS3Settings.msgTimeout,false,true);}else{s3_showStatus(S3.i18n.ajax_dwn,$.ajaxS3Settings.msgTimeout,false,true);}};$.ajax(options);};$.postS3=function(url,data,callback,type){return $.ajaxS3({type:"POST",url:url,data:data,success:callback,dataType:type});};$.getS3=function(url,data,callback,type,message,sync){if($.isFunction(data)){sync=message;message=type;type=callback;callback=data;data=null;}
if(sync){var async=false;}
return $.ajaxS3({type:'GET',url:url,async:async,data:data,success:callback,dataType:type,message:message});};$.getJSONS3=function(url,data,callback,message,sync){if($.isFunction(data)){sync=message;message=callback;callback=data;data=null;}
if(!sync){var sync=false;}
return $.getS3(url,data,callback,'json',message,sync);};$.ajaxS3Settings={timeout:10000,msgTimeout:2000,retryLimit:10,dataType:'json',async:true,type:'GET'};$.ajaxS3Setup=function(settings){$.extend($.ajaxS3Settings,settings);};})($);function S3StatusBar(sel,options){var _I=this;var _sb=null;this.elementId='_showstatus';this.prependMultiline=true;this.showCloseButton=false;this.afterTimeoutText=null;this.cssClass='statusbar';this.highlightClass='statusbarhighlight';this.errorClass='statusbarerror';this.closeButtonClass='statusbarclose';this.additive=false;$.extend(this,options);if(sel){_sb=$(sel);}
if(!_sb){_sb=$("<div id='_statusbar' class='"+_I.cssClass+"'>"+"<div class='"+_I.closeButtonClass+"'>"+
(_I.showCloseButton?' X </div></div>':'')).appendTo(document.body).show();}
$('.'+_I.cssClass).click(function(e){$(_sb).hide();});this.show=function(message,timeout,additive,isError){if(additive||((additive==undefined)&&_I.additive)){var html="<div style='margin-bottom: 2px;' >"+message+'</div>';if(_I.prependMultiline){_sb.prepend(html);}else{_sb.append(html);}}
else{if(!_I.showCloseButton){_sb.text(message);}else{var t=_sb.find('div.statusbarclose');_sb.text(message).prepend(t);}}
_sb.show();if(timeout){if(isError){_sb.addClass(_I.errorClass);}else{_sb.addClass(_I.highlightClass);}
setTimeout(function(){_sb.removeClass(_I.highlightClass);if(_I.afterTimeoutText){_I.show(_I.afterTimeoutText);}},timeout);}}
this.release=function(){if(_statusbar){$('#_statusbar').remove();_statusbar=undefined;}}}
var _statusbar=null;function s3_showStatus(message,timeout,additive,isError){if(!_statusbar){_statusbar=new S3StatusBar();}
_statusbar.show(message,timeout,additive,isError);}
function s3_hideStatus(){if(_statusbar){_statusbar.release();}}
function S3SetNavigateAwayConfirm(){window.onbeforeunload=function(){return S3.i18n.unsaved_changes;};};function S3ClearNavigateAwayConfirm(){window.onbeforeunload=function(){};};function S3EnableNavigateAwayConfirm(){$(document).ready(function(){if($('[class=error]').length>0){S3SetNavigateAwayConfirm();}
$(':input:not(input[id=gis_location_advanced_checkbox])').keypress(S3SetNavigateAwayConfirm);$(':input:not(input[id=gis_location_advanced_checkbox])').change(S3SetNavigateAwayConfirm);$('form').submit(S3ClearNavigateAwayConfirm);});};function S3ConfirmClick(ElementID,Message){$(ElementID).click(function(event){if(confirm(Message)){return true;}else{event.preventDefault();return false;}});};function s3_viewMap(feature_id){var url=S3.Ap.concat('/gis/display_feature/')+feature_id;var oldhtml=$('#map').html();var iframe="<iframe width='640' height='480' src='"+url+"'></iframe>";var closelink=$('<a href=\"#\">'+S3.i18n.close_map+'</a>');closelink.bind("click",function(evt){$('#map').html(oldhtml);evt.preventDefault();});$('#map').html(iframe);$('#map').append($("<div style='margin-bottom: 10px' />").append(closelink));}
function s3_viewMapMulti(module,resource,instance,jresource){var url=S3.Ap.concat('/gis/display_feature//?module=')+module+'&resource='+resource+'&instance='+instance+'&jresource='+jresource;var oldhtml=$('#map').html();var iframe='<iframe width="640" height="480" src="'+url+'"></iframe>';var closelink=$('<a href=\"#\">'+S3.i18n.close_map+'</a>');closelink.bind('click',function(evt){$('#map').html(oldhtml);evt.preventDefault();});$('#map').html(iframe);$('#map').append($("<div style='margin-bottom: 10px' />").append(closelink));}