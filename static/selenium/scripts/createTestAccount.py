...
class CreateTestAccount(unittest.TestCase):
    ...
    def test_create_test_account(self):
        ...
        # This script creates a test account with the user name of testing@example.com the account is given administrator rights and will be used throughout the testing suite.
        # The user testing@example.com will be deleted by the "Delete Test Account" test case.
        sel.open("/eden/admin/user")
        self.failUnless(sel.is_element_present("show-add-btn"))
        sel.click("show-add-btn")
        sel.type("auth_user_first_name", "Testing")
        sel.type("auth_user_last_name", "User")
        sel.select("auth_user_language", "label=English")
        sel.type("auth_user_email", "testing@example.com")
        ...
        sel.run_script("oTable = $('#list').dataTable();  oTable.fnFilter( 'testing@example.com' );")
        for i in range(60):
            try:
        ...
        sel.wait_for_page_to_load("30000")
        # Log out from the current "Administrator" user and log back in for the remaining test cases as testing@example.com
